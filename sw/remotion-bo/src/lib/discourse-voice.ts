/**
 * 가상 담화(Discourse) 음성 파일명·경로 규칙 — BO 측 정의.
 *
 * ⚠ 동기화 대상: sw/remotion/src/compositions/Discourse/voice-names.ts 가 SSoT 다.
 *   워크스페이스 경계상 import 불가라 복제한다. 한쪽을 바꾸면 반드시 다른 쪽도 함께 바꾼다.
 *   vnTurn·vnCastEpithet·vnTimingKey·vnVerify 는 규칙이 100% 일치해야 한다.
 *
 ...
 *   번호만 쓰면 밀림이 소리로만 드러나지만, 번호와 slug 를 함께 쓰면
 *   T03-musk.wav 가 알트만 자리에 놓인 순간 눈으로 잡힌다. 검증 함수(vnVerify)가 이 대조를 자동으로 한다.
 */

import type { DiscourseScript, Speaker, Turn } from './discourse-types'
