 ...
 * 2) 힌트가 없으면 각 계정에 GET /v1/voices/{voiceId} 를 병렬 조회해
 *    이 음성을 가진(=소유하거나 접근 가능한) 첫 계정을 고른다.
 *    공용·기본(premade) 음성은 여러 계정에서 접근되므로 앞선(기본) 계정이 우선된다.
...
export async function resolveEleAccountForVoice(
  ...
  const hinted = getEleAccountById(accountId)
