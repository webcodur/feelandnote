...
test('그룹과 장면은 같은 시퀀스 카드 골격과 세력-순서 번호를 쓴다', () => {
  const group = renderToStaticMarkup(
    <FactionSequenceCard id="group" numberLabel="2-1" type="cluster" title="귀향자">
      첫 본문
    </FactionSequenceCard>,
  )
  const scene = renderToStaticMarkup(
    <FactionSequenceCard id="scene" numberLabel="2-2" type="scene" title="활의 시험">
      둘째 본문
    </FactionSequenceCard>,
  )

  assert.match(group, /data-faction-sequence-card="cluster"/)
  assert.match(scene, /data-faction-sequence-card="scene"/)
  ...
  assert.match(group, /bg-amber-400/)
  assert.match(scene, /bg-cyan-400/)
...
test('시퀀스 카드는 내부 fixed 모듈의 뷰포트 기준 배치를 가두지 않는다', () => {
  const markup = renderToStaticMarkup(
    <FactionSequenceCard id="voice-host" numberLabel="2-1" type="cluster" title="오디세우스">
