  ...
  return (
    <div className="bg-bg-secondary/50 border border-border rounded-lg p-4 space-y-3">
      ...
      <div className="space-y-2">
        <label htmlFor="ai-description" className="block text-sm font-medium text-text-secondary">인물 설명 (선택)</label>
        <textarea id="ai-description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="예: 테슬라 CEO, 스페이스X 창업자..." rows={2} className="w-full px-4 py-2 bg-bg-secondary border border-border rounded-lg text-text-primary placeholder-text-secondary focus:border-accent focus:outline-none resize-none" />
        <p className="text-xs text-text-secondary">AI가 프로필과 7개 항목 영향력을 함께 생성합니다.</p>
      </div>
      ...
      {!result && (
        <Button type="button" variant="secondary" onClick={handleGenerate} disabled={generating || !nickname.trim()}>
          {generating ? <><Loader2 className="w-4 h-4 animate-spin" />생성 중...</> : <><Sparkles className="w-4 h-4" />AI 프로필 생성</>}
        </Button>
      )}
      ...
      {result && (
        <div className="bg-bg-secondary rounded-lg p-4 space-y-4">
          ...
          <div className="space-y-2 text-sm">
            <div><span className="text-text-secondary">직군:</span><span className="ml-2 text-text-primary">{getCelebProfessionLabel(result.profession)}</span></div>
            <div><span className="text-text-secondary">소개:</span><p className="mt-1 text-text-primary">{result.bio}</p></div>
            {result.avatarUrl && (
              <div className="flex items-center gap-2">
                <span className="text-text-secondary">이미지:</span>
                <div className="relative w-10 h-10">
                  <Image src={result.avatarUrl} alt="" fill unoptimized className="rounded-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none' }} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-border pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`px-2 py-0.5 rounded text-xs font-bold ${RANK_COLORS[result.influence.rank]}`}>{result.influence.rank}등급 ({result.influence.totalScore}/100)</span>
            </div>
                ...
                if (typeof field === 'object' && 'score' in field) {
                  ...
                  return (
                    <div key={key} className="space-y-1">
                      ...
                      <div className="h-1.5 bg-bg-main rounded-full overflow-hidden"><div className="h-full bg-accent rounded-full" style={{ width: `${percent}%` }} /></div>
                      {field.exp && <p className="text-xs text-text-secondary pl-1">{field.exp}</p>}
                    </div>
                  )
                }
                return null
          ...
          <div className="flex gap-2 pt-2">
            <Button type="button" size="sm" onClick={handleApply}><Check className="w-4 h-4" />적용</Button>
            <Button type="button" size="sm" variant="ghost" onClick={() => setResult(null)}><X className="w-4 h-4" />취소</Button>
          </div>
        </div>
      )}
    </div>
