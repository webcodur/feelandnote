...
### H. 소장·선물 — 등록 불가 (본인 구입은 예외)

**서재에 꽂혀 있었다는 사실은 감상 근거가 아니다.** 장서 목록·서재 사진·"소장한 책들"이라는 보도는 그 책을 읽었다는 진술이 아니며, 우연히 꽂혀 있었을 수 있다. 선물로 받았다는 기록도 마찬가지다.

- ❌ "다큐멘터리에 비친 서재 300여 권 가운데 이 책이 꽂혀 있었다" → 소장
- ❌ "환송회에서 전집을 선물했다" → 선물
- ⭕ "입단을 앞두고 이 교본으로 공부했다" → 행위가 명시됨, A급

**단, 본인이 직접 샀다는 사실이 확인되면 등록한다.** 구입은 그 사람이 고른 행위다. 우연히 꽂혀 있는 소장이나 남이 준 선물과 다르다. 아직 읽지 않았어도 본인이 사서 읽겠다고 밝힌 책은 받는다.

- ⭕ "연휴에 읽을 책"이라며 자기가 산 세 권을 찍어 올림 → 본인 구입, 등록
- ❌ 산 사람이 누구인지 알 수 없거나 협찬·증정본 → 기각

같은 출처 안에서도 책마다 강도가 다르다. **한 기사에 나온 목록이라고 통째로 등록하지 말고 책별로 서술을 확인한다.**

> 박지성 7권을 한 기사에서 등록했다가 6권을 걷어냈다. 기사가 쓴 단어는 "소장한 책들"이었고, "팬들이 보내준 책들을 섭렵했다"는 독서 서술은 있었지만 어느 책인지 연결되지 않았다. 행위가 붙은 것은 「표준 일본어 교본」 한 권뿐이었다.

### 현대 인물의 제3자 서술 — 필자가 무엇을 걸었나로 가른다

본인 진술만 근거가 되는 것은 아니다. 제3자의 서술도 받되, 가르는 선은 **글쓴이가 무엇을 걸고 무엇을 말했는가**다.

| 판정 | 정의 | 예시 |
|---|---|---|
| **받는다** | 필자가 실명·직함을 걸고 **사실로 진술** | 블로터 발행인이 칼럼에 "『한비자』는 이건희 회장이 삼성 임원들에게 필독을 권했던 책"이라고 씀 |
| **버린다** | 필자가 **스스로 추측임을 밝힌** 것 | 『이건희의 서재』(안상헌)는 경영 스타일에서 "짐작해볼 만한 책", "서재에 꽂힐 만한 책"을 고른 기획이다 |
| **버린다** | 인물과 책이 한 지면에 있을 뿐 **연결이 없는** 것 | 어록 옆에 편집자가 곁들인 추천 도서 |

**해설서에서 기각한 책도 개별로 다시 검색하면 살아난다.** 위 28권은 해설서 단독으로는 전량 기각이지만, 따로 찾아 「한비자」를 등록했고 그 목록에 없던 「정관정요」("정관정요에서 인사를 배웠다" — 본인 발언)까지 얻었다.

### 재큐레이션 사이트 — 단독 근거 불가

`radicalreads.com`·`bookauthority.org`·`favobooks.com`·`alux.com`·`polarbooks`·`namu.wiki`·`wikitree`는 남의 목록을 모아 놓은 곳이거나 위키다. 후보 탐색에는 쓰되 `source_url`로 박지 않는다.

- 이 사이트들이 표기한 **원 출처(NYT·오프라 매거진·본인 웹사이트 등)를 추적해 그것으로 등록한다.**
- 원문을 열면 **재큐레이션이 옮기며 붙인 인용문이 원문에 없는 경우가 나온다.** 실제로 그렇게 발견된 것들이 있다.
- 폐쇄된 페이지는 웨이백 스냅샷에 원 출처 표기가 남아 있는 경우가 많다.
...
- **이 판정은 위로 올리지 않는다.** 기각하고 사유만 보고한다. 실측에서 반복해 올라온 유형은 이렇다 —
  17~19세기 마술서, 프랑스 고전 요리서, 전래 찬송가, 성경 낱권 본문. 증거가 1차 사료여도 결과는 같다.
  **현행 리프린트가 유통 중이면 그것으로 등록할 수 있다.** 후디니가 인용한 로베르 우댕 회고록이 그렇게 살았다.
...
- `amazon`을 BOOK 메타데이터나 `contents.external_source`로 사용하지 않는다(공식 API 부재·접근권 제한). 다만 OpenLibrary로 확인한 실제 영문판의 Amazon 구매·제휴 링크는 `content_locales.affiliate_url`에 둘 수 있으며, fiction 원전은 [`celeb-pipeline.md`](celeb-pipeline.md)의 「fiction 원전·등장 도서 연결」을 따른다.
