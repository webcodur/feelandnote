...
> 당시 §6-6의 「`profiles` 유지」 결론은 26.08.09 재조사에서 다시 폐기됐다. 최종 구조와
> 완료 검증은
> `docs/archive/data/profile-celeb-account-separation-improvement-2026-08-09.md`가 쥔다.
> 운영 DB에는 `member_profiles`·`celebs`와 관계별 전용 테이블만 남았고, 새 앱 배포 뒤
> 레거시 혼합 테이블과 `profiles`도 제거했다. 이 문서는 완료 이력으로 보존한다.
