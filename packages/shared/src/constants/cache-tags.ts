 ...
 *
 * 도메인을 추가·변경하면 사본 하나를 함께 맞춘다.
 * - web의 /api/revalidate 는 이 상수를 import하므로 자동으로 따라온다.
 * - DB 트리거의 허용 목록 web_revalidate_allowed_domains() 는 따라오지 않는다.
 *   빠뜨리면 DB가 그 도메인의 태그를 조용히 버려 캐시가 비워지지 않는다.
 *   `pnpm check:cache-tags` 가 불일치를 잡고 붙여 넣을 마이그레이션을 출력하며,
 *   오라클 배포가 빌드 전에 같은 검사를 강제한다.
 */
