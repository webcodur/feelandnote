...
export async function getCelebForModal(celebId: string): Promise<CelebProfile | null> {
  ...
  return {
    ...
    quotes: (dialogueResult.data?.lines as Record<string, any> | null)?.quote ?? profile.quotes ?? null,
    quotes_en: (dialogueResult.data?.lines_en as Record<string, any> | null)?.quote ?? (profile as any).quotes_en ?? null,
