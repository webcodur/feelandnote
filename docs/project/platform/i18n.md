...
| 테이블 | 추가 컬럼 |
|--------|----------|
| `celebs` | `title_en`, `bio_en`, `consumption_philosophy_en`, `virtual_monologue_en`. `cultural_journey_en`은 generated 읽기 별칭이며 `quotes_en` 컬럼은 없음 |
...
| celebs (현재 `consumption_philosophy_en`) | 867명 | ✅ |
...
→ **완료**(26.03). naver_book 2,364건 전량 verified, 한글/CJK 잔존 0건. 진단·수정 이력은 **`docs/archive/data/en-book-data-quality.md`** 참조.
...
- `sw/web-bo/scripts/translate-celeb-readings.ts`: 읽어보기 3필드의 해시 가드·재개 가능한 2단계 번역
...
레거시 음악 후보를 iTunes 최종 콘텐츠로 바꾸며 생긴 `celeb_contents` 255행에 한국어
...
추가 MUSIC 4행은 감상문만 번역하지 않고 Spotify 레거시를 iTunes 원곡으로 전환해
