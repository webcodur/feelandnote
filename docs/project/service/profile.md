...
`[userId]` 라우트는 `member_profiles`의 회원만 렌더한다. 인물 링크 생성은
`sw/web/src/lib/url.ts`의 `getCelebProfileUrl`이 맡고, 제목·레이아웃·본문의 주소 해석과
없는 대상 판정은 `sw/web/src/lib/profile-route.ts`가 맡는다. 조회 장애는 없는 대상으로
처리하지 않는다.

인물 정본은 `/celeb/{slug}`다. slug가 없는 인물 링크는 `/celeb/{id}`로 들어와 정본으로
이동하며, 외부에 남은 옛 `/{id}` 인물 주소도 정본으로 보낸다. 최근 방문 기록은 slug를
함께 저장하고 같은 링크 생성기를 쓰므로 기존의 ID만 있는 저장 기록도 계속 열린다.
`/celeb/{id}`는 middleware가 `celeb/id/[id]`의 동적 이동 페이지로 내부 재작성한다.
이 페이지도 같은 주소 해석기를 쓰며, 정본 본문의 ISR과 UUID 이동 응답을 분리한다.
