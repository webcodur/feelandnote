...
> **최종 실측 체크: 26.09.09** — 09-04 기준 Search Console 색인 목록 82 URL을 URL Inspection API로 전수 재검사했다. 81개는 `크롤링됨 - 현재 색인이 생성되지 않음`, 1개(`/library/era`)는 리디렉션이며 색인 PASS는 0이다. 이는 그 목록 전체의 최신 검사 결과이지 도메인 모든 URL의 색인 수는 아니다. 입력 목록·URL별 검사 결과는 [조사 데이터](../../../data/seo-index-inspection-20260909.json)에 있다. 사이트맵 API의 `indexed: 0`과 지연된 페이지 보고서의 82개를 현재 색인 수로 해석하지 않는다.
>
> 홈은 09-05까지 노출·클릭이 있었지만 현재 API·UI 모두 미색인이다. 09-09 02:58 KST 실제 URL 테스트는 재시도에서 색인 가능·HTTP 200을 통과했고, Google이 렌더링한 모바일 화면의 본문도 확인했다. 실패 리소스 3개는 외부 광고 주소이며 콘솔 메시지 1개는 검사 환경의 서비스 워커 등록 거부다. 홈·영문 홈·탐색·머스크 상세의 현재 Googlebot 응답에도 본문·index/follow가 있다. 직접 조치·보안 문제·최근 6개월 임시 삭제 요청은 UI에서 없음으로 재확인했다. 실시간 테스트 통과는 색인 복구가 아니다.
>
> **제외 원인은 미확정이다.** 전체 검색 노출은 3월 674 → 4월 195 → 5월 17로 이미 감소했고, 최근 30일(08-08~09-06)은 49회·4클릭이다. 최근 서버 OOM·502 재발과 fetch·목록 캐시의 수정·검증 및 09-09 운영 반영은 [Oracle 사용자 웹 운영](../platform/external-services.md#oracle-사용자-웹-운영)에 기록했다. 해당 장애 때 Googlebot 실패는 확인하지 못했으며, 현재 자료만으로 서버 장애·외부 신뢰도·콘텐츠 평가 중 하나를 원인으로 확정하지 않는다. 인물 수 축소나 사이트맵 반복 제출의 효과도 입증되지 않았다.

09-09 배포 전 이력 대조: 당시 운영 활성 슬롯은 `3da95ac7`(09-08 05:41:41 UTC 메인 시작), 직전 슬롯은 `fd4407e4`다. 홈페이지 마지막 크롤은 09-04 16:48:13 UTC이며, canary·메인 시작 기록상 `80e22780` 전환 뒤 다음 전환 전이다. `80e22780`→`3da95ac7`에서 전역 robots·middleware·locale layout·홈 page·canonical·봇 렌더 분기·Next 설정 변경은 없었다. 09-06 홈 로고·탐색 UI 변경은 이 크롤보다 뒤다. 당시 HTML·CDN 기록은 없어 실제 전달 본문까지 특정하지는 못했다. 이후 `8dd8f2b6`를 09-09 10:05 KST 운영에 반영하고 Cloudflare HTML 캐시를 비웠다. 공개 URL 7곳에서 새 배포 ID·HTTP 200·canonical·index/follow를 확인했다. 이 검증은 Google 색인 복구 확인과는 별개다.
...
본 도메인이 색인을 못 받는 동안 사이트에 글을 더 써도 그 글도 색인되지 않는다. 그래서 티스토리에
「필앤노트 시네마」를 열었다. 영화 감상 3,208건은 네이버 책 블로그가 다룰 수 없는 덩어리이고,
티스토리는 색인이 빨리 붙는다. 목적은 링크 가중치가 아니라 **검색 결과에서 자리를 차지해 사람을
사이트로 보내는 것**이다. 운영 규칙은 [`tistory-cinema.md`](../../continuous/tistory-cinema.md)가 쥔다.
