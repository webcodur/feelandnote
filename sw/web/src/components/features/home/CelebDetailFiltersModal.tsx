...
type DetailFilter = "profession" | "nationality" | "contentType" | "gender" | "birthYear";
...
export default function CelebDetailFiltersModal({ filters, onClose, onInteraction }: Props) {
  ...
  const [active, setActive] = useState<DetailFilter>("profession");
  ...
  const renderOption = (option: { value: string; label: string; count?: number }) => {
    ...
    const selected = filters[active] === option.value;
    return (
