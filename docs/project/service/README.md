...
작품을 선택하면 제목·표지·감상배경·리뷰·출처와 `책 소개`·`게임 소개` 같은 구획명을 즉시 바꾼다. 아직 도착하지 않은 작품 소개와 메타데이터 자리만 로딩 상태로 남기며, 이전 작품의 본문을 대신 보여 주지 않는다. 인접 작품은 미리 요청하지 않는다. 음악은 저장된 iTunes metadata가 있으면 외부를 다시 조회하지 않는다. 소개에는 `책 소개` 또는 `작품 소개` 라벨을 붙이고, 문자열로 넘어온 HTML 엔티티를
