/**
 * youtube-lineup.ts — YouTube 편성표 + 메타 생성
 *
 * 정식 원천: remotion-bo/src/lib/youtube-utils.ts
 * 이 파일의 buildTitle, buildDescription, calcChapterTimestamps는
 * BO youtube-utils.ts와 동일하게 유지해야 한다.
 */
...
export const lineup: Record<string, EpisodeMeta> = lineupData
...
/** 에피소드 JSON에서 챕터 타임스탬프 계산 (프레임 기반, render-all.ts와 동일) */
export function calcChapterTimestamps(ep: any, lang: 'ko' | 'en' = 'ko'): { time: string; label: string }[] {
  const n = ep.narrator
  const h = ep.host
  const books = ep.books as any[]
  if (!n?.serviceGreetingDuration || !h?.voiceDuration || !books?.length) return []
  ...
  const fqRaw = (h.featuredQuoteDuration ?? 0) > 0 ? toF(h.featuredQuoteDuration) : 0
...
export function buildDescription(
  celebName: string,
  books: { title: string; creator: string; stats?: { publishYear?: string } }[],
  lang: 'ko' | 'en',
  isShorts: boolean,
