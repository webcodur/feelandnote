...
| `docs/todo/game-wave2-contract.md` | **실험 게임 7종 SSoT** — 교차 격자·넷씩 넷·근접도·경로 잇기·어느 쪽·상위 다섯·가림 해제. 검증된 실존 데일리게임 포맷을 이 서비스 데이터로 옮긴 것. 병렬 구현 경계 규정, 체험 표본 규칙, 마감 결정 기록(쉼터 미등록 근거·문구 전송량 실측). 개별 규격은 `docs/todo/game-<키>-order.md` 7종. **현재 `/lab/games`에서만 열린다 — 공개 쉼터에 붙이지 않았다** |
...
| 실험 게임 7종 공개 여부 판정 | `docs/todo/game-wave2-contract.md` · `docs/todo/game-<키>-order.md` | **구현·검수 완료·공개 판정 대기(26.07.31)** | 검증된 실존 데일리게임 포맷을 이 서비스 데이터로 옮긴 7종(교차 격자·넷씩 넷·근접도·경로 잇기·어느 쪽·상위 다섯·가림 해제). 실 DB로 8개 라우트 200·폴백 0·`tsc` 0·관련 `eslint` 0·`pnpm build:web` 성공.<br>**현재 `/lab/games`에서만 열린다** — `/lab`은 robots 차단 실험 구역이라 공개 쉼터에는 붙이지 않았다(통합 단계가 넣은 쉼터 카드는 되돌렸다).<br>**다음**: 만져보고 남길 것을 골라 쉼터로 승격(링크가 아니라 기존 4종처럼 전체화면 임베드), 버릴 것은 폴더·서버 조회·문구 파일·`i18n/request.ts` 네임스페이스 등록까지 함께 제거. 비공개 3종(유랑·기억궁·초상) 재공개 여부도 같은 자리에서 정한다.<br>**미검증**: 브라우저 손조작 완주(주소 응답만 확인), 320px 실기기, 하루 시드 게임의 날짜 경계 동작. |
| 기관 선정 | `docs/project/service/curated-lists.md` | **완료·가동 중(26.08.01)** | 대학·언론·시상기관이 뽑은 작품 목록. 실측 25기관·25목록·2,529항목·표지 85%, 실서비스 반영 확인. 전용 백오피스 화면은 **예정 없음** — 조사 JSON을 `import-curated-list` → `match-curated-items` → `register-curated-books` 순으로 돌리고 `/api/revalidate`에 `curated` 태그를 던지는 것이 운영 절차다.<br>**남은 것**: 미연결 314건은 대부분 국내 미출간작(아쿠타가와 로마자 표기 132)과 논문·발췌(세인트존스 86)라 서지 조회로는 한계. 표지만 없는 72건은 ISBN 보유가 6건뿐이라 자동 보강 열쇠가 없다 — **둘 다 그대로 두기로 결정**(화면은 제목 기반 생성 표지로 채워진다). |
...
| remotion-bo 프로젝트 | `docs/project/remotion-bo-plan.md` | **폐기 완료(26.07.29)** | 팩션·가상 담화·서재 탐방을 모두 web-bo로 옮긴 뒤 앱 디렉터리와 실행 배선을 제거했다. |
| 포트 정비 | — | **완료** | 현행 개발 서버 포트는 상단 앱 표와 루트 명령을 따른다. |
...
| 인물 생애 행적 | `docs/project/celeb-journey.md` | **완료(26.07.27)** | 영향력 50점 이상 **205명 3,507건** 적재·가동 중(좌표 3,354 · 장소 1,386곳 · 근거 누락 0). 서술은 한·영 모두 GPT 재작문. 백오피스 편집 화면 `/celebs/timeline` 신설. 남은 것: ① **천도 지구본을 `WorldGlobe`로 이관** — 핵심 완주 흐름과 별개인 후속 과제 ② 40점대 이하 확장은 미정.<br>🔴 **재작문 적재는 id 로 하지 마라** — 조사 재적재가 id 를 갈아치워 2,382건이 조용히 누락된 적이 있다(문서 함정 7) |
...
| 음성 파일 매니저 | `docs/archive/voice-file-manager.md` | **완료·web-bo 이관(26.07.29)** | `features/book-recommend/lib/server-utils.ts`의 저장소 함수와 `/api/book-recommend/voice/storage`, `VoiceStorage.tsx`, 제작 현황 탭이 web-bo에서 동작한다. audio-bo 흡수도 R2 의존도 아니다. |
