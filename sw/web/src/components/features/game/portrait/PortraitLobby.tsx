...
import { Eye, ScanFace, Sparkles, Timer, Trophy } from "lucide-react";
...
export default function PortraitLobby({ canStart, bestScore, onStart }: Props) {
  ...
  return (
    <div className="m-auto w-full max-w-3xl py-3 text-center sm:py-6">
      <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full border border-accent/35 bg-accent/10 text-accent shadow-[0_0_40px_-14px_rgba(212,175,55,0.8)] sm:mb-5 sm:h-16 sm:w-16">
        <ScanFace className="h-6 w-6 sm:h-7 sm:w-7" aria-hidden />
      </div>
      <p className="mb-1.5 font-cinzel text-[9px] font-bold tracking-[0.25em] text-accent/70 sm:mb-2 sm:text-[10px] sm:tracking-[0.28em]">
        PORTRAITS IN TIME
      </p>
      <h1 className="font-serif text-2xl font-black text-text-primary sm:text-4xl">{t("label")}</h1>
      ...
      <button
        ...
        disabled={!canStart}
        onClick={onStart}
        className="mt-4 inline-flex min-h-12 min-w-52 items-center justify-center gap-2 rounded-xl border border-accent/45 bg-accent/15 px-7 py-3 font-serif text-base font-black text-accent shadow-[0_12px_32px_-18px_rgba(212,175,55,0.8)] hover:border-accent hover:bg-accent/25 active:scale-[0.98] disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-white/5 disabled:text-text-secondary sm:mt-6 sm:py-3.5"
      >
        <Sparkles className="h-4 w-4" aria-hidden />
        {t("startGame")}
      </button>
      {!canStart && <p className="mt-3 text-xs text-red-300">{t("notEnoughFigures")}</p>}
      <p className="mt-3 hidden text-[10px] text-text-secondary/70 sm:block">{t("keyboardHelp")}</p>

      <div className="mx-auto mt-7 hidden max-w-2xl grid-cols-3 gap-3 text-left sm:grid">
        <RuleCard icon={<Eye />} title={t("rules.revealTitle")} body={t("rules.revealBody")} />
        <RuleCard icon={<Timer />} title={t("rules.speedTitle")} body={t("rules.speedBody")} />
