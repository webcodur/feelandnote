...
> **최종 실측 체크: 26.08.14** — 08-10 교정 후에도 회복이 없어 재조사했다. 3월 노출 674 → 5월 이후 3.5개월 55로 무너진 시점이 4월 장애와 일치하고, 인물·작품 상세는 「발견됨 - 색인 생성 안 됨」에 묶여 있다. 크롤 통로(홈 → `/explore/directory` → 인물 링크 1,749개)는 온전하므로 병목은 크롤 예산으로 판정하고, 제출량의 79%를 차지하던 작품 상세 14,386 URL을 사이트맵에서 제외했다(18,434 → 4,048). 별개로 08-13 배포 이후 `full` 인물 1,508명의 상세가 500이며 교정 코드는 미배포 상태다. 상세는 「색인 회복 실패 재조사」절이 쥔다.
>
...
2. **작품 상세를 사이트맵에서 제외**(이 문서 「사이트맵」절 반영 완료) — 제출량 18,434 → 4,048.
...
- **URL 구성**(2026-08-14 반영 후): 정적·기관 선정 `core.xml` 332 URL + 인물 1,858명 3,716 URL = 총 **4,048개**. 각 경로가 ko·en 2 URL로 나간다. DB 증가에 따라 바뀌므로 규약값이 아니라 시각을 붙인 스냅샷이다.
