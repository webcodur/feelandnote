 ...
 *
 * 실행:
 *   pnpm figure-books:batch -- --file ../../data/celeb/fiction/<work>.json
 *   pnpm figure-books:batch -- --file ../../data/celeb/fiction/<work>.json --apply
 */
...
async function loadSourceBatchSnapshot(
  ...
  return {
    ...
    sourceDesignated: Boolean(sourceResult.data as FigureBookRow | null),
    rows: (rowsResult.data ?? []) as FigureBookCharacterRow[],
  }
}
...
async function resolveCharacters(
  db: DatabaseClient,
  manifest: FigureBookBatchManifest,
): Promise<ResolvedFigureBookCharacter[]> {
...
async function verifySourceDesignation(db: DatabaseClient, contentId: string): Promise<void> {
  ...
  if (!data || (data as FigureBookRow).content_id !== contentId) {
...
async function applySourceBatch(
  ...
  assertExactFigureBookReadback(expectedRows, await loadReadbackRows(db, contentId))
}
...
async function main(): Promise<void> {
  ...
  const manifest = parseFigureBookBatchManifest(
  ...
  const plan = buildFigureBookBatchPlan(manifest.contentId, characters, snapshot.rows)
