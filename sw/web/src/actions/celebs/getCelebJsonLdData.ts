...
async function fetchJsonLdContents(celebId: string): Promise<JsonLdContentRow[]> {
  ...
  throwOnQueryError('getCelebJsonLdContents', error)
  return (data ?? []).map(row => {
...
const getCelebJsonLdContentsCached = (celebId: string) =>
  cachedDetail(CACHE_TAGS.CELEBS, celebId, ['celeb-jsonld-contents-v2-query-guards', celebId], () => fetchJsonLdContents(celebId), {
...
export async function getCelebJsonLdContents(celebId: string): Promise<JsonLdContentRow[]> {
  return withQueryFallback(
    'getCelebJsonLdContents',
    () => getCelebJsonLdContentsCached(celebId),
    [],
  )
}
