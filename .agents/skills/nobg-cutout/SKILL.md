...
`download`·`all` 명령은 현행 운영 DB·저장소와 연결되지 않은 옛 흐름이다. 로컬 이미지를 처리할 땐 `rembg`만 쓴다.

## 셀럽 아바타 파이프라인에서의 자리

```
개인샷 → crop-faces(얼굴 정사각) → codex 고해상도 재생성 → [여기서 누끼] → avatar/upload.ts
```

- 크롭 → 재생성 → 누끼 순서로 해도 된다. 재생성본은 인물 형태가 온전해 목 단면을 배경으로 오인하지 않는다.
- `photo/crop-faces.ts` 에는 누끼 기능이 없다. 넣지 마라.
