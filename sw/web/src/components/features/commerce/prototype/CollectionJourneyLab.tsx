...
export default function CollectionJourneyLab() {
  ...
  const [id, setId] = useState<JourneyId>("low-end-theory");
  const [counts, setCounts] = useState({ preview: 0, open: 0, choice: 0, outbound: 0 });
  ...
  return <div className="mx-auto max-w-5xl pb-12">
    <header className="mb-8 border-b border-white/10 pb-7">
      ...
      <p className="mt-4 max-w-2xl text-sm leading-relaxed text-text-secondary">이야기를 읽고, 감상할 형태를 고른 뒤, 실제 판매처로 이동합니다. 세 작품으로 음반·게임·미술의 서로 다른 선택을 눌러보세요.</p>
    ...
    <div aria-label="모형 작품 선택" className="mb-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
      {Object.values(JOURNEY_WORKS).map((item) => <button key={item.id} type="button" aria-pressed={id === item.id} onClick={() => setId(item.id)}
        ...
        <span className="min-w-0"><span className="block text-[10px] text-accent">{item.kind === "music" ? "음반 · 매체와 품절" : item.kind === "game" ? "게임 · 기종과 보유 여부" : "미술 · 소재와 크기"}</span><span className="mt-1 block text-sm font-medium leading-snug">{item.title}</span></span>
      </button>)}
    </div>
    <article key={id}>
    ...
    <aside className="mt-8 border-t border-white/10 pt-5">
      ...
      <p role="status" className="mt-6 text-xs leading-relaxed text-text-tertiary">이 화면에서 누른 횟수 · 미리듣기 재생 {counts.preview} · 안내 열기 {counts.open} · 선택 변경 {counts.choice} · 판매처/공식 안내 이동 {counts.outbound}<br />모형 확인용이며 GA로 전송하지 않습니다. 외부 이동은 구매 완료가 아닙니다.</p>
    </aside>
  </div>;
}
