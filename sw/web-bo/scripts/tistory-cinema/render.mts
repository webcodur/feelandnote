 ...
 */
const P_STYLE = 'margin:0 0 20px;line-height:1.9;'
const para = (html: string) => `<p style="${P_STYLE}">${html}</p>`
const h2 = (id: string, text: string) =>
  `<h2 id="${id}" style="margin:44px 0 18px;padding-top:8px;font-size:21px;line-height:1.45;border-top:1px solid #eee;">${text}</h2>`
const h3 = (text: string) => `<h3 style="margin:30px 0 12px;font-size:17px;">${text}</h3>`
...
const revHtml = (t: string) =>
  esc(t).split(/\n{2,}/).map((x) => x.trim()).filter(Boolean).join('</p><p style="margin:14px 0 0;">')
...
export function renderWork(m: Material): { title: string; html: string; tags: string[] } {
  ...
  if (m.work.poster) {
    p(`<figure style="margin:0 0 32px;text-align:center;">`)
    p(`<img src="${m.work.poster}" alt="${esc(t)} 포스터" style="max-width:330px;width:100%;border:1px solid #e3e3e3;border-radius:2px;" />`)
    p(`<figcaption style="margin-top:10px;font-size:13px;color:#888;">${esc(t)}${year ? ` (${year})` : ''}${m.work.creator ? ` · ${esc(m.work.creator)}` : ''}</figcaption>`)
    p(`</figure>`)
  }
  ...
  p(para(`${year ? `${year}년에 나온 ` : ''}『${esc(t)}』입니다.${m.tmdb.director?.length ? ` ${esc(m.tmdb.director[0])}가 연출했습니다.` : ''} 이 영화를 자기 인생작으로 꼽은 사람이 <b>${m.total}명</b> 있습니다.${pair.length === 2 ? ` 감독과 배우만이 아니라 ${pair[0]}도 ${pair[1]}도 이 영화를 이야기했습니다.` : ''}`))
  p(para(`이 글은 세 가지를 차례로 봅니다. 먼저 『${esc(t)}』${ga(t)} 어떤 영화인지 기본 정보와 줄거리, 예고편으로 한 번에 정리합니다. 그다음 이 영화를 꼽은 사람들이 <b>어디서 무슨 말을 했는지</b> 그대로 옮깁니다. 끝으로 그 기록을 모아 놓고 보이는 것을 적습니다.`))

  p(`<div style="margin:32px 0;padding:20px 24px;background:#f7f7f8;border-radius:6px;">`)
  ...
  p(`<table style="width:100%;border-collapse:collapse;font-size:15px;margin:0 0 24px;">`)
  ...
  if (m.tmdb.trailer) {
    p(h3('예고편'))
    p(`<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;margin:0 0 8px;">`)
  ...
  m.picked.forEach((r, i) => {
    ...
    p(`<div id="${anchor(i)}" style="margin:0 0 26px;padding:20px 22px;border-left:3px solid #222;background:#fafafa;">`)
  ...
  p(`<div style="margin:38px 0 12px;padding:28px 22px;text-align:center;background:#111;border-radius:8px;">`)
  p(`<img src="https://feelandnote.com/icon.png" alt="필앤노트" style="width:52px;height:52px;border-radius:12px;margin-bottom:14px;" />`)
...
export function renderPerson(m: PersonMaterial): { title: string; html: string; tags: string[] } {
  ...
  p(para(`${esc(who)}${intro ? `${eun(who)} ${esc(intro)}로 알려져 있습니다. 그가` : ga(who)} 인터뷰와 방송에서 직접 말한 영화가 <b>${m.total}편</b> 있습니다.`))
  p(para(`아래는 그 가운데 널리 알려진 ${m.picked.length}편입니다. 발언은 <b>출처가 있는 것만</b> 골라 고치지 않고 옮겼습니다.`))
...
export function renderList(m: ListMaterial): { title: string; html: string; tags: string[] } {
  ...
  p(para(`<b>${esc(name)}</b>${m.curator ? ` — ${esc(m.curator.name)}가 고른 목록입니다.` : '입니다.'} 아래에 <b>${m.totalItems}편 전체</b>를 실었습니다.`))
  ...
  p(para(`그리고 이 목록이 다른 곳과 갈리는 지점이 하나 있습니다. ${m.totalItems}편 가운데 <b>${m.withVoice}편</b>은 필앤노트에 기록이 있는 감독·배우·작가가 인터뷰에서 자기 인생 영화로 꼽은 작품입니다. 누가 어디서 무슨 말을 했는지 함께 적었습니다.`))
