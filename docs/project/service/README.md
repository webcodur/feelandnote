...
> **확인 중 걸린 것 — 인기 작품 구역이 통째로 사라져 있었다.** 처음에는 "개발 환경의 캐시가 굳은 것이라 코드 결함이 아니다"로 판단했으나 **틀렸다.** 조회가 실패해도 빈 목록을 정상 결과처럼 돌려주고 있었고, 그 빈 값이 7일짜리 캐시에 박히는 구조였다. 같은 사고가 셀럽 목록에서 먼저 났지만 재발 방지가 이 조회에는 적용되지 않았다. 26.08.07에 조회 실패를 캐시하지 않도록 고치고 캐시를 비워 복구했다. 최초 사례와 현행 규칙은 `docs/project/platform/tooling-gotchas.md`의 「실패를 캐시에 박지 마라」.
