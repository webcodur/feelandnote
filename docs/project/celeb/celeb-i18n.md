...
| **full** | 필수 |
| **light** | 필수 |
| **fiction** | 번역 트랙 생략 · 신설 필수 영문은 생성 단계에서 작성 |

fiction은 영향력 설명과 21개 영문 대사를 만드는 일괄 번역 트랙을 실행하지 않는다. 다만 인물 신설 시
`celebs`의 영문 기본 정보, 읽어보기 영문 본문, 연대기 영문 제목·설명, 작품 배정 영문 설명은
fiction 공개 준비 데이터이므로 각 생성 단계에서 함께 작성한다. 티어 정의는 `celeb-pipeline.md` §티어를 따른다.
