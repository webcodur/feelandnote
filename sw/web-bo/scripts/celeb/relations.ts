 ...
 *
 * [방향 규약 — 이 규약이 전부다]
 *   관계 사실 하나는 한 행만 쓴다. rel_type은 "to_id가 from_id에게 무엇인가"를 뜻한다.
 *   방향 관계는 자녀·영향을 받거나 배운 사람을 from_id에, 부모·영향을 주거나 가르친 사람을
 *   to_id에 둔다. 대칭 관계는 두 id를 정렬한다. 화면은 양끝을 조회해 현재 인물 기준 라벨을 만든다.
