...
> **이 문서가 [`docs/archive/remotion/faction-db-sync.md`](../../../archive/remotion/faction-db-sync.md)(다리 방식)를 대체한다.** 다리(양방향 동기화)는 폐기 개념이고, 목표는 집 하나다. faction-sync 코드는 이관·이미지 배관 도구로 개조되어 살아남는다.
