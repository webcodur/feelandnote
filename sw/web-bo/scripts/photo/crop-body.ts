/**
 * crop-body — 단체샷에서 인물별 "전신 크롭"을 뽑는다(얼굴 크롭이 아니다).
 *
 * 얼굴은 위치를 잡는 기준일 뿐이고, 실제 산출물은 머리 위 여백부터 무릎까지 담은
 * 900x1400(4.5:7) 세로 이미지다. 단독 화보 발주의 REF 재료로 쓴다.
 *
 * 사용법: pnpm exec tsx scripts/photo/crop-body.ts <_group.png 경로> [출력 폴더]
 * 산출물: <원본명>_body_<번호>.jpg (얼굴 면적 내림차순) + <원본명>_rename.txt
 *
 * 아바타용 얼굴 정사각 크롭은 photo/crop-faces.ts를 쓴다.
 */
...
async function main() {
  ...
  if (!imagePath) {
    console.error('사용법: pnpm exec tsx photo/crop-body.ts <이미지경로> [출력디렉토리]')
    process.exit(1)
  }
  ...
  for (let i = 0; i < faces.length; i++) {
    ...
    const outPath = resolve(outDir, `${baseName}_body_${i + 1}.jpg`)
  ...
  let txtContent = `[전신 크롭 — 얼굴 크기순 매핑 (1번이 가장 큼)]\n# 아래 '이름' 부분을 실제 인물명으로 수정하세요.\n`
  for (let i = 0; i < faces.length; i++) {
