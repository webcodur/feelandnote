      ...
      <div className="grid grid-cols-[minmax(0,1fr)_300px] gap-8 overflow-hidden">
        ...
        >
          ...
          >
            ...
            >
              {/* 텍스트 영역 */}
              <div className="relative z-20 flex-1 p-8 flex flex-col justify-center bg-gradient-to-r from-black via-black/90 to-transparent">
                <div className="flex flex-col gap-1 mb-4">
                  {heroCeleb?.title && (
                    <span className="text-sm text-accent font-serif font-bold tracking-wider">{heroCeleb.title}</span>
                  )}
                  <h3 className="text-2xl font-serif font-black text-white leading-tight">{heroCeleb?.nickname}</h3>
                </div>
                {heroCeleb?.short_desc && (
                  <p className="text-lg text-white/90 font-serif italic leading-relaxed mb-3">"{heroCeleb.short_desc}"</p>
                )}
                {heroCeleb?.long_desc && (
