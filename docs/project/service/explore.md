...
검색·정렬·필터는 중앙의 공통 패널로 묶고 PC에서는 한 줄로 배치한다. 좁은 화면에서는 검색창 아래에 선택 버튼을 둔다. 작품의 매체 선택과 인물의 트렌드 안내도 이 패널 안에 두며, 트렌드 안내는 강한 배지 없이 보조 문구로 표시한다. 두 모드 모두 페이지 번호를 바꿀 때 현재 스크롤 위치를 유지한다. 기관명은 두 줄 높이를 확보하고 한 줄일 때도 세로 가운데에 놓는다. 인물·기관 카드는 금빛 테두리·그림자·이미지 하단 빛·이름 칸 강조와 내부 이미지 확대를 `ExploreCard.styles.ts`로 공유한다.
