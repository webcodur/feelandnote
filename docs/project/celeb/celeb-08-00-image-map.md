...
| 세력도감 개인화보 | `celeb_tag_assignments.faction_image_url` | R2 `faction/{tagId}/celeb-{celebId}.webp`; 화면은 `faction_atlas_members` 뷰에서 읽고 원본 비율로 표시 | `faction-image` 스킬 |
| 세력도감 단체화보 | `celeb_tags.team_images` | R2 `faction/{tagId}/team/…`; 태그별 묶음(이름·구성원)과 단체 사진. 세력 화면은 사진을 띄우지 않고, 세력이 하나뿐인 테마를 잘게 나누는 묶음 정의로만 쓴다. 세력이 둘 이상이면 세력(뷰의 `group_label`)으로 묶는다 | `faction-image` 스킬 |
...
세력도감 화보는 web-bo 테마 편집 화면(`/factions/<테마>`)에서 올린다([`web-bo.md`](../apps/web-bo.md) 「세력도감」).
...
- 세력도감 개인화보의 운영 읽기 창구는 `faction_atlas_members`다.
- 세력도감 페이지(`/explore/faction`)는 단체화보도 출연진 판도 띄우지 않고 탐색과 같은 인물 카드로 진영별 격자를 세우고, 인물 카드를 누르면 뜨는 인물 소개 모달도 개인화보 대신 인물 상세와 같은 아바타 모듈(확대 보기·인사 음성)을 쓴다(26.09.15). 구성원 아바타를 한 무대에 겹쳐 세운 출연진 판(`FactionMemberLineup`)은 인물 상세 세력 구획에만 남는다. 판은 아바타가 배경을 지운 누끼라는 전제에 선다 — 불투명 아바타를 들이면 판에서 사각형이 튄다. 무리가 크면 영향력 점수(`celeb_influence.total_score`)로 가른 핵심만 무대에 세우고 나머지는 발치에 작은 얼굴로 둔다. 핵심·마이너를 적는 별도 칸은 두지 않는다. 인원 기준은 `FactionMemberLineup.tsx`의 상수가 쥔다.
