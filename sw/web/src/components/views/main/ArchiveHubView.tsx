          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Avatar
                ...
                size="xl"
                className="group-hover:ring-accent/40 ring-4 ring-accent/20"
              />
            </Button>
            ...
            <div className="flex-1 min-w-0">
              <Button
                ...
                className="group"
              >
                <h1 className="text-lg font-bold text-text-primary group-hover:text-accent transition-colors text-left">
                  {profile.nickname}
                </h1>
              </Button>
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <Button
                ...
                className="flex items-center gap-1 px-2.5 py-1.5 bg-accent hover:bg-accent-hover text-white text-xs font-medium rounded-lg"
              >
                <Plus size={14} />
              ...
              <Button
                unstyled
                onClick={onExplore}
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-sm font-medium text-text-secondary hover:text-accent transition-colors rounded-lg hover:bg-background"
              >
                <Compass size={14} />
    ...
    <>
      ...
      <header className="bg-surface rounded-2xl p-4">
        <div className="flex items-center h-12">
          <Button
            unstyled
            onClick={onBack}
            className="w-12 h-12 flex items-center justify-center -ml-2 rounded-lg hover:bg-background transition-colors text-text-secondary hover:text-text-primary"
          >
            <ArrowLeft size={20} />
          </Button>
          <div>
        ...
        {friends.length > 0 ? (
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-4">
            {friends.slice(0, 8).map((friend) => (
              <UserCard
                key={friend.id}
                user={{ ...friend, type: "friend" }}
                onClick={() => onSelectUser({ ...friend, type: "friend" })}
              />
            ))}
          </div>
        ) : (
        ...
        {celebs.length > 0 ? (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
            {celebs.map((celeb) => (
              <UserCard
                key={celeb.id}
                user={{ ...celeb, type: "celeb" }}
                onClick={() => onSelectUser({ ...celeb, type: "celeb" })}
                showCategory
              />
            ))}
          </div>
      ...
      <section className="bg-surface rounded-2xl p-5">
        ...
        <EmptyState
          icon={<Star size={32} />}
          title="추천 기능 준비 중"
          description="취향 분석을 통해 비슷한 유저를 추천해드릴게요"
        />
      </section>
    ...
    <div className="relative">
      {url ? (
        <img
          src={url}
          alt={name}
          className={`${sizeClasses[size]} rounded-full object-cover ring-2 ring-accent/20 ${className}`}
        />
      ) : (
        <div
          className={`${sizeClasses[size]} rounded-full flex items-center justify-center font-bold text-white ring-2 ring-accent/20 ${className}`}
      ...
      {verified && (
        <CheckCircle
          size={size === "xl" ? 20 : size === "lg" ? 18 : 14}
          className="absolute -bottom-0.5 -right-0.5 text-accent bg-surface rounded-full"
        />
      )}
    </div>
  );
  ...
  return (
    ...
    >
      <Avatar
        ...
        size="md"
        verified={user.is_verified}
        className="group-hover:ring-accent transition-all"
      />
      <span className="mt-2 text-xs font-medium text-text-secondary group-hover:text-accent transition-colors truncate max-w-full">
        {user.nickname}
      </span>
      {showCategory && user.category && (
        <span className="text-[10px] text-text-tertiary">{user.category}</span>
      )}
    </Button>
  );
}
