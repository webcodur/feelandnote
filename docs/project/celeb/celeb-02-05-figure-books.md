...
- **화면 구획 이름은 관계명 + 작품**으로만 만든다: 연관 작품 Related Works · 창작 작품 Created Works. 인물 화면은 등장을 단정하지 않기로 해(26.09.07) `appearance`·`related`를 「연관 작품」 한 구획에 함께 보여 주고, 관계 유형 라벨(등장 작품·연관 작품)은 백오피스와 목록 표기에만 쓴다. 인물 화면 아래 구매 구획은 연관 작품의 상품에 추천 도서(읽은 책·직군·인기)를 이어 붙인 구획으로 「참고도서 Reference Books」라 부른다.
...
앙솔로지(열전·평전 모음)를 책 단위로 훑어 등록 인물을 찾는 **작품 기준 조사는 폐기했다.** 후속 세션은 이 방식을 되살리지 않는다.

- 이유: 브라우저 자동화로 책 수백 권을 순회하면 유명한 책과 마이너한 재야사학서가 구분 없이 섞여 들어간다. 소출도 얕다(실측: 개인 전기가 압도적이라 인물 기준이 주력).
- **대안**: 새 인물이 추가될 때마다 그 인물 기준으로 `appearance-muse-candidates`를 돌린다. 그 인물이 앙솔로지에 나온다면 인물 기준 조사에서도 걸린다 — 별도의 책 단위 순회가 없어도 된다.
- 조사 스크립트(`appearance-by-work.mjs`, `by-work-*.mjs`, `by-work-targets.mjs`)는 지우지 않고 남겨 둔다. 다만 새로 실행하지 않는다.

## 건국신화 팩션(FICTION)의 연관 도서 — 팩션 단위 일괄 적용

각국 건국신화·전승(`docs/todo/founding-myth.md`)의 시조·전설 인물은 위 「작품 기준 폐기」와 다른
문제를 겪는다 — 개인 전기가 원천적으로 없는 부류라 인물 기준을 아무리 돌려도 대부분 "없음"으로
끝난다(26.09.08 실측: 279명 개별 조사 시 정상 도서 발견 43명뿐). 이 인물들은 **팩션(전승) 하나에
묶인 신화·서사시 자체가 이미 원전**이므로, 팩션 대표 하나(전승에서 가장 유명한 인물)를 조사해
그 원전이 실제로 있는지 확인하고, **있으면 같은 팩션 전원에게 `related`로 일괄 적용**한다.
사기(중국 상고 신화)·샤나메(페르시아)·마하바라타(인도 왕조)·에린 침략의 서(켈트·브리튼)·빌헬름
텔(게르만)·몽골 비사·마나스 서사시(초원)·아즈텍 신화(아메리카)·하우사·요루바 전설(아프리카)·
베트남의 신화와 전설(동남아)·마우이 관련서(오세아니아)가 그렇게 붙었다.

**표본 순서를 조심한다.** 팩션 대표를 나중에 확인하면 "이 팩션은 발견 0건"이라는 오판이 난다 —
페르시아·초원·게르만·오세아니아가 전부 그렇게 잘못 판정될 뻔했다. **가장 유명한 인물(전승표의
`fame: A`급)부터 먼저 확인**한다. 그래도 대표가 없으면(예: 켈트·브리튼의 브루투스 계보처럼
정식 번역본이 없는 갈래) 그 갈래만 다른 팩션 도서를 붙이거나 폐기 후보로 남긴다 — 팩션 전체를
폐기하는 것은 대표 확인까지 마친 뒤에만 판단한다.
...
- 검증된 관계는 판매 판본이나 제휴링크가 없다는 이유로 삭제하지 않는다. 한국어 인물 도서 화면은 저장된 판본을 모두 선택할 수 있고, 같은 판본의 기존 쿠팡 링크를 보존한다. 선택 판본의 예스24 구매 연결은 [제휴 판매 운영](../operations/affiliate-commerce.md)이 쥔다. 영어는 활성 상품이 있으면 그 상품의 판본을, 없으면 요청 언어의 판본을 보여 준다. 노출 판정은 `sw/web/src/actions/figure-books/getFigureBooks.ts`가 쥔다.
...
- **OL 언어 태그 하나만 믿지 않는다.** 이탈리아어 「Un Natale di Maigret」(Adelphi)·스페인어 「San Camilo 1936」(Alianza)·러시아어 오디오북 「Barbarians at the Gate」가 OL에서 `eng`로 찍혀 있었다. 국가군이 978-0·978-1·979-8 밖이면 판본 제목이 영어 문장인지, 출판사가 영어권 출판사(Springer·Tuttle·Kodansha International·인도·싱가포르·홍콩 영문 출판사 등)인지 함께 본다. 둘 다 아니면 영문판으로 인정하지 않는다.
- **언어판을 확인하지 못한 카드는 지우지 않고 표시용 제목 행으로 바꾼다.** 규칙은 [`celeb-02-02-content-registration.md`](celeb-02-02-content-registration.md)「한국어판 확인」·「영문판과 표지」가 쥔다. `en` 카드 제목이 영어가 아니면(프랑스어·독일어 원제, 한국어) 그 저작의 통용 영어 제목으로 바꾸고, 한국·중국·일본 저작은 로마자 표기를 쓴다(`sources.title='romanized'`). 판본 값(ISBN·출판사·표지·소개)은 비우고 그 locale의 판본 행은 지운다. 실제 판본이 확인되면 공식 값으로 덮는다. `sw/web-bo/scripts/figure-books/locale-display-title.mjs`가 이 전환을 한다.
