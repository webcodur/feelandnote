/**
 * BookRecommend TTS 생성 (마스터 대본 v6 — Studio 음성)
 * 사용법: node scripts/generate-voice.mjs
 *
 * Google Cloud TTS Studio 음성 (최고 품질) + SSML 호흡 제어.
...
// Neural2: 한국어 최고 품질 (Studio는 ko 미지원). WaveNet 대비 자연스러운 억양·호흡.
const MALE_VOICE = { languageCode: 'ko-KR', name: 'ko-KR-Neural2-C', ssmlGender: 'MALE' }
const FEMALE_VOICE = { languageCode: 'ko-KR', name: 'ko-KR-Neural2-A', ssmlGender: 'FEMALE' }
...
// --- SSML 래퍼: 문장 단위 <s> + 문장 간 break ---
const ssml = (text) => {
  // 문장 분리 (마침표/물음표/느낌표 + 공백 기준)
  const sentences = text.split(/(?<=[.?!])\s+/).filter(Boolean)
  const body = sentences
    .map((s) => `<s>${s}</s>`)
    .join('<break time="500ms"/>')
  return `<speak><prosody rate="92%">${body}</prosody></speak>`
}
