/*
  ...
  책임: 원장의 rel_type을 화면이 읽는 다섯 묶음으로 옮긴다. 조회는 호출처가 하고
        여기서는 나누고 줄 세우기만 한다 — relatedFigures.ts와 같은 규약이다.

        방향 규약은 원장과 같다: rel_type은 "to_id가 from_id에게 무엇인가"를 뜻한다.
        중심 인물을 from_id로 놓고 조회하므로, 여기 오는 rel_type은 전부
        "상대가 중심에게 무엇인가"로 읽으면 된다.
*/
