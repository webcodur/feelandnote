...
// 위 guard 의 좁힘은 함수 경계를 넘지 못한다. main() 안에서 쓸 확정 경로를 여기서 받는다.
const filePath = file
...
async function main() {
  ...
  if (celeb.slug !== slug) throw new Error(`slug 불일치 — DB "${celeb.slug}" vs 인자 "${slug}"`)
  ...
  const smallBuf = await sharp(buf)
  ...
  const key = `celebs/${celebId}/avatar.webp`
  await put(key, buf)
  await put(`celebs/${celebId}/${CELEB_AVATAR_SMALL.smallFile}`, smallBuf)
