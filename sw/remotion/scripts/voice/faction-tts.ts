 ...
 *
 * public/factions/<에피소드>/faction-data.json 의 각 인물 대사(quote)를 Gemini TTS 로 합성하고,
 * public/factions/<에피소드>/voice/<파일>.wav 로 저장한 뒤, 각 wav 길이를 측정해
 * faction-data.json 의 해당 인물 quoteDuration 에 기록한다.
 *
 ...
 *
 * 파일명 규칙(렌더와 동일):
 *   세력 i, 그룹 c, 인물 j → F{i+1}C{c+1}P{j+1}-quote.wav  (예 F02C01P03-quote.wav)
 *   ※ 모든 세력이 그룹(clusters)을 가지므로 파일명은 항상 FxxCxxPxx 다(solo 세력 포함).
 *
 * 사용법:
