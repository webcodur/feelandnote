 ...
 * 에피소드 상태 변경.
 *
 * 신구조는 현재 위치와 그룹을 보존하고 `_status.json`만 갱신한다.
 * `_status.json`이 없는 옛 구조만 legacy status 폴더로 물리 이동한다.
 */
export async function moveEpisode(person: string, toStatus: EpisodeStatus): Promise<void> {
  ...
  if (isNewLayout(found.dir) || existsSync(path.join(found.dir, STATUS_FILE))) {
    await writeFile(
      path.join(found.dir, STATUS_FILE),
      `${JSON.stringify({ status: toStatus }, null, 2)}\n`,
      'utf-8',
    )
    return
  }
...
export async function getVoiceStorageStatus(): Promise<{
  ...
  const totalLoaded = { count: 0, sizeBytes: 0 }
  const totalArchived = { count: 0, sizeBytes: 0 }
