        ...
        {groupCounts.map(({ group, count }) => {
          ...
          return (
            <button key={id} onClick={() => setTab({ kind: 'group', group })}
              className={`px-3 py-1.5 font-semibold transition-colors ${active ? 'bg-accent text-bg-main' : 'text-text-secondary hover:text-text-primary'}`}>
              {groupLabel(group)} {count}
            </button>
          )
        })}
        <button onClick={() => setTab({ kind: 'candidate-pool' })}
          className={`px-3 py-1.5 font-semibold transition-colors ${tabId === 'candidate-pool' ? 'bg-zinc-500 text-white' : 'text-text-secondary hover:text-text-primary'}`}>
          Draft {filteredCandidates.length}
        </button>
        <button onClick={() => setTab({ kind: 'storage' })}
          className={`px-3 py-1.5 font-semibold transition-colors ${tabId === 'storage' ? 'bg-purple-500 text-white' : 'text-text-secondary hover:text-text-primary'}`}>
          Storage
        </button>
      </div>
