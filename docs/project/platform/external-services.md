...
- 서가 인기 작품은 `pnpm sync:bestsellers`와 `.github/workflows/sync-bestsellers.yml`이 주간 수집해 공개 JSON을 갱신한다. 웹은 `src/lib/library/bestsellerFeed.ts`의 게시 주소·캐시 주기로 새 목록을 읽으며 목록 갱신에는 웹 재배포가 필요 없다. 분류별 수집일을 표시하고, 부분 실패는 해당 목록·날짜를 보존하면서 Actions를 실패 처리한다. 수집 기준과 검증은 `src/lib/library/bestsellerPolicy.mjs`를 공유한다. Actions에는 `KAKAO_REST_API_KEY`·`TMDB_API_KEY`를 등록한다.
