...
> **최종 실측 체크: 26.08.28** — 카드 자산 6장과 `/rest` 등록 코드를 대조했다. 기억은 공개,
> 시대의 초상·유랑은 개발자 모드다.

쉼터(`/rest`) 허브용 게임 카드 상징 이미지 6장을 관리한다. 공개 게임 5종이 각 자산을 쓰고,
개발자 모드인 시대의 초상은 기억의 `memory-card.webp`를 함께 쓴다.
...
**원본 캔버스**: 숯빛 석재와 낡은 황동으로 지은 어두운 기록 회랑. 뒷면만 보이는 직사각형 초상 카드가 황동 레일에 줄지어 걸리고 소실점을 향해 멀어진다. 같은 두 장에 빛이 들던 기존 기억 게임 그림을 그대로 사용한다.
...
`memory-card.webp`는 기억 게임용으로 납품된 파일명을 바꾸거나 복제하지 않고 시대의 초상 카드와 전체화면 배경에서 재사용한다.
...
3. 6장이 나란히 놓였을 때 **한 세트로 보이는지** 확인한다. 톤은 각기 다르되 어둠의 깊이와 빛의 세기가 비슷해야 카드 6장이 형제로 읽힌다.
...
- **유랑 카드 납품(26.07.31)**: §3-6 프롬프트로 3안(평원 길·능선 조망·폐허 아치)을 뽑아 카드에 얹은 상태로 비교하고 폐허 아치안을 채택했다. 1536×768 WebP 18KB. 평균 밝기 17.7로 기억(16.3)·미궁(24.5)과 같은 대역이라 여섯 장이 한 세트로 읽힌다.
- **노출 상태(26.08.28)**: 기억은 `dev: false`로 공개하며 자료도 항상 조회한다. 유랑·시대의 초상은 `dev: true`라 로컬 개발 서버나 `?dev=1`에서만 카드와 자료를 연다.
...
- **호버(현행)**: 테두리 금색 강조·제목 금색 전환은 `transition` 없이 즉시. 배경 이미지는 곁들이는 연출로 분류되어 `group-hover:scale-105`가 `duration-700`으로 천천히 확대된다 (`docs/project/platform/code-rules.md` 상호작용 절 — 즉각 반응축과 연출축의 구분).
