/*
  파일명: actions/game/getTimelineCelebContents.ts
  기능: 여러 셀럽의 감상 콘텐츠 배치 조회
  책임: 타임라인 게임 결과에서 셀럽별 콘텐츠 표시용 데이터 반환
*/
"use server";

import { createClient } from "@/lib/supabase/server";

export interface TimelineContent {
  contentId: string;
  title: string;
  creator: string | null;
  thumbnailUrl: string | null;
  type: string;
  review: string | null;
  sourceUrl: string | null;
}

export async function getTimelineCelebContents(
  celebIds: string[]
): Promise<Record<string, TimelineContent[]>> {
  if (celebIds.length === 0) return {};

  const supabase = await createClient();

  // 단일 쿼리: user_contents JOIN contents (셀럽당 최대 4개는 클라이언트에서 제한)
  const { data, error } = await supabase
    .from("user_contents")
    .select(
      "user_id, content_id, review, source_url, contents!inner(id, title, creator, thumbnail_url, type)"
    )
    .in("user_id", celebIds)
    .eq("visibility", "public");

  if (error || !data) return {};

  const result: Record<string, TimelineContent[]> = {};

  for (const row of data) {
    const c = row.contents as unknown as {
      id: string;
      title: string | null;
      creator: string | null;
      thumbnail_url: string | null;
      type: string | null;
    };

    if (!result[row.user_id]) {
      result[row.user_id] = [];
    }

    // 셀럽당 최대 4개
    if (result[row.user_id].length >= 4) continue;

    result[row.user_id].push({
      contentId: c.id,
      title: c.title ?? "",
      creator: c.creator,
      thumbnailUrl: c.thumbnail_url,
      type: c.type ?? "BOOK",
      review: (row as any).review ?? null,
      sourceUrl: (row as any).source_url ?? null,
    });
  }

  return result;
}
