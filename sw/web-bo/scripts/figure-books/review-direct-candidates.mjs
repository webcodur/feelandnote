...
같은 직군이라는 사실 하나만으로는 고르지 않는다. 그러나 관련성이 그 사람에게만 고유할 필요는 없다. 프로필에 드러난 구체적인 하위 맥락과 책 주제가 바로 맞으면 여러 인물이 같은 책을 공유해도 된다. 예를 들어 축구선수와 축구 전술서, 영화배우와 스크린 연기서, AI 기업가와 AI 개론서, 컴퓨터 설계자와 컴퓨터과학서는 모두 유효한 후보다. 일반 성공론·리더십·자기계발서는 제외한다.
