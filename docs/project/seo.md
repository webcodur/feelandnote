...
- 26.08.10 11:08 KST 비개인화 검색(`pws=0`, 한국)에서 `feelandnote`는 홈페이지가 첫 웹 결과로 복귀했고 새 소개 문구가 스니펫에 노출됐다. `필앤노트`는 아직 자사 도메인이 없고 YouTube 채널만 노출된다. 영문 도메인명 검색은 회복 신호가 확인됐지만 한글 브랜드 검색 복구는 진행 중이다.

### Bing·네이버 콘솔 교차 감사 (2026-08-10 13:07 KST)
...
| Site Scan | 기존 스캔 이력 0. `Feelandnote SEO audit 2026-08-10`으로 최대 1,000페이지 스캔을 새로 시작했고 12:52 KST 현재 `Processing`이다. |
...
| 주요 유입 | 앤드류 후버만·다리오 아모데이·이재용·젠슨 황 등 인물+작품 질의가 주력이다. `/celeb/matt-damon`은 노출 6,434·클릭 12·CTR 0.2%였고, 통용 표기와 다른 DB 이름 `멿 데이먼`을 `맷 데이먼`으로 교정했다. |
...
- 네이버 단일 페이지 최대 노출이며 CTR 0.2%인 `matt-damon`을 추가 감사했다. DB `celebs.nickname`의 `멿 데이먼`은 Wikidata `Q175535`의 한국어 라벨·한국어 위키백과 문서명과 다른 표기였다. `맷 데이먼`으로 교정하자 `updated_at`이 갱신됐고, 해당 인물 캐시만 무효화한 뒤 프로덕션 title·description에 즉시 반영됐다. slug·영문 `Matt Damon`·hreflang·canonical은 불변이다. 한·영 URL 2개는 Bing·네이버 IndexNow에 증분 통지해 둘 다 HTTP 200을 받았다.
...
3. 같은 URL을 반복 제출해도 대기열 위치·우선순위가 바뀌지 않으므로 재요청을 연속으로 보내지 않는다. 28일 뒤에도 한글 브랜드 검색이 회복되지 않을 때 원인 보고서와 페이지별 노출을 다시 감사한다.
