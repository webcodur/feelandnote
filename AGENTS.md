...
| 인물 생애 행적 | `docs/project/celeb-journey.md` | **완료(26.07.27)** | 영향력 50점 이상 **205명 3,507건** 적재·가동 중(좌표 3,354 · 장소 1,386곳 · 근거 누락 0). 서술은 한·영 모두 GPT 재작문. 백오피스 편집 화면 `/celebs/timeline` 신설. 남은 것: ① **천도 지구본을 `WorldGlobe`로 이관** — 게임 비활성이라 보류 ② 40점대 이하 확장은 미정.<br>🔴 **재작문 적재는 id 로 하지 마라** — 조사 재적재가 id 를 갈아치워 2,382건이 조용히 누락된 적이 있다(문서 함정 7) |
