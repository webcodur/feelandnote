 ...
 * 가로세로비가 화면과 다른 이미지의 좌우·상하 검정 여백을 없앤다.
 *
 * 영상은 CutMedia 가 비율을 유지한 채 화면을 빈틈없이 채우므로(objectFit: cover) 흐린 배경이 가려져 보이지 않는다.
    ...
    <AbsoluteFill style={{ overflow: 'hidden' }}>
      ...
      {!isVid && imgFit !== 'cover' && (
        <div style={{ position: 'absolute', top: '-6%', left: '-6%', width: '112%', height: '112%' }}>
          <CutMedia src={src} startFrame={startFrame} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'blur(60px) brightness(0.7)' }} />
        </div>
      )}
      {/* 본 이미지 레이어 — 이미지는 fit(기본 contain), 영상은 비율 유지+채움(cover, CutMedia 가 덮어씀) */}
      {/* 사진 맞춤(crop): objPos·transformOrigin 으로 보일 위치를, scale 로 확대를 잡는다 */}
      <AbsoluteFill>
        <CutMedia src={src} startFrame={startFrame} fit={fit} onError={onError} style={{ width: '100%', height: '100%', objectFit: imgFit, objectPosition: objPos, transform: `scale(${scale}) translate(${tx}%, ${ty}%)`, transformOrigin }} />
      </AbsoluteFill>
    </AbsoluteFill>
  )
}
