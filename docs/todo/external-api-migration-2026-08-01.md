...
## 5. 감상 여정 60명 (그대로 둔다 — 26.08.01 결정)

26.07.31 등록 인물 중 60명에게 `consumption_philosophy`/`cultural_journey`가 새로 작성됐다.
이 항목은 폐기 예정이라 신규 작성 금지인데(`celeb-pipeline.md` §0 첫 줄) 그 방침이 지켜지지 않았다.
같은 글이 두 칸에 중복 저장돼 있고 분량은 평균 129자다.

**지우지 않고 둔다.** 어차피 폐기 예정 항목이라 운영 로직이 이 값을 읽지 않고,
지운다고 얻는 것도 없다. 다만 **새 인물에게 또 채우지는 마라.**
