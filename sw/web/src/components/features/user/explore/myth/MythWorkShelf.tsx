...
export default function MythWorkShelf({ works, selectedPersonId }: Props) {
  ...
  const visible = [...selectedWorks, ...works.filter((work) => !selectedIds.has(work.id))].slice(0, 10);
    ...
    <section aria-labelledby="myth-works-title" className="pt-1">
      ...
      <div className="scrollbar-hide -mx-1 flex snap-x gap-3 overflow-x-auto px-1 pb-2 md:gap-4">
        {visible.map((work) => {
          ...
          const cardBody = (
            <>
              ...
              <div className="p-3">
                ...
                {work.creator && <p className="mt-1 truncate text-sm text-text-secondary">{work.creator}</p>}
                {purchaseUrl ? (
                  <p className="mt-2 text-sm font-bold text-[#ff776a]">{t("buyOnCoupang")}</p>
                ) : (
                  <p className="mt-2 text-sm font-semibold text-accent">{t("castCount", { count: work.personIds.length })}</p>
                )}
              </div>
            </>
          );
          ...
          return (
            <div key={work.id} className="w-36 shrink-0 snap-start md:w-44">
              {purchaseUrl ? (
                <a
                  href={purchaseUrl}
                  target="_blank"
                  rel="noopener noreferrer nofollow sponsored"
                  className={`group block overflow-hidden rounded-2xl border bg-bg-card hover:border-[#E44232]/70 ${selected ? "border-[#E44232]/60" : "border-[#E44232]/35"}`}
                  title={`${work.title} · ${t("buyOnCoupang")}`}
                >
                  {cardBody}
                </a>
              ) : (
                <Link href={workHref} className={`group block overflow-hidden rounded-2xl border bg-bg-card hover:border-accent/70 ${selected ? "border-accent/60" : "border-stone-heavy"}`}>
                  {cardBody}
                </Link>
              )}
              {purchaseUrl && (
