...
### Bing·네이버 콘솔 교차 감사 (2026-08-10 12:30 KST)
...
| 사이트맵 | 알려진 사이트맵 1, 오류 0, 경고 0, 발견 URL 17.4K. `/sitemap.xml`을 재제출해 `Last submit 8/10/2026`, `Processing`으로 바뀌었다. 직전 최종 크롤은 8/7이었다. |
...
| Site Scan | 기존 스캔 이력 0. `Feelandnote SEO audit 2026-08-10`으로 최대 1,000페이지 스캔을 새로 시작했고 현재 `Queued`다. |
...
| 주요 유입 | 앤드류 후버만·다리오 아모데이·이재용·젠슨 황 등 인물+작품 질의가 주력이다. `/celeb/matt-damon`은 노출 6,434·클릭 12·CTR 0.2%로 후속 스니펫 개선 후보다. |
...
| Bing Webmaster Tools | ✅ 사이트맵 재제출·스캔 대기·IndexNow 확인 | Google SC 연동 | 사이트맵 | 26.08.10 `/sitemap.xml` 재제출 `Processing`·발견 17.4K, 1,000페이지 Site Scan `Queued`. 최근 3개월 노출 992·클릭 27, AI 인용 35 |
