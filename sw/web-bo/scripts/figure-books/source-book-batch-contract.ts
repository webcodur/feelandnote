...
export type FigureBookManifest = {
  work: {
    identity: string
    title: string
    creator: string
...
export type FigureBookPlan = {
  ...
  duplicateMatchers: {
    workIdentity: string
    editionKind: FigureBookEditionKind
    textScope: string
    isbns: string[]
...
export function parseFigureBookManifest(input: unknown): FigureBookManifest {
  ...
  return {
    ...
    edition: { kind: kind as FigureBookEditionKind, scope },
...
function figureBookIdentity(content: StoredContentRow): {
  workIdentity?: string
  editionKind?: string
  textScope?: string
} {
  const source = content.metadata?.figureBook
...
function explicitReuseMetadata(
  ...
  const desiredSource = desired.figureBook
  ...
  if (currentSource !== undefined && currentSource !== null
    ...
    return {
      metadata,
      changed: false,
      conflicts: ['contents.metadata.figureBook is non-null and not an object'],
    }
  }
  ...
  for (const [key, value] of Object.entries(desiredSource as Record<string, unknown>)) {
    ...
    if (current === undefined || current === null || current === '') {
      ...
      conflicts.push(`contents.metadata.figureBook.${key} conflicts with explicit reuse`)
    }
  }
  if (conflicts.length === 0 && changed) metadata.figureBook = mergedSource
...
function reviewedDeterministicTargetConflicts(
  ...
  const conflicts: string[] = []
  const identity = figureBookIdentity(target)
  if (target.type !== 'BOOK'
      || identity.workIdentity !== manifest.work.identity) {
    conflicts.push(`deterministic target ${target.id} does not have the requested figureBook work identity`)
  }
...
export function buildFigureBookPlan(
  manifest: FigureBookManifest,
  resolved: ResolvedSourceBookRegistration,
  ...
  if (manifest.reuseContentId) {
    ...
    for (const contentId of reviewedDistinctContentIds.filter((id) => candidateSet.has(id))) {
      ...
      if (reasons.has('work_identity')) {
        conflicts.push(`reviewed candidate ${contentId} has the same figureBook work identity`)
      }
  ...
  if (selectedId) {
    ...
    if (selected) {
      const identity = figureBookIdentity(selected)
  ...
  let contentUpdate: FigureBookPlan['contentUpdate'] = null
...
function snapshotMaterial(snapshot: ExactContentSnapshot): FigureBookPlan['expectedAfterMaterial'] {
...
export function planSha256(plan: FigureBookPlan): string {
...
export function buildAtomicSourceBookApplySql(plan: FigureBookPlan): string {
...
WHERE content.type = 'BOOK'
  AND (
    ...
    OR
    content.metadata #>> '{figureBook,workIdentity}' = batch.payload #>> '{duplicateMatchers,workIdentity}'
...
SELECT coalesce(jsonb_agg(jsonb_build_object(
  'id', content.id,
  'external_id', content.external_id,
  'figureBook', jsonb_build_object(
    'workIdentity', content.metadata #>> '{figureBook,workIdentity}',
    'editionKind', content.metadata #>> '{figureBook,editionKind}',
    'textScope', content.metadata #>> '{figureBook,textScope}'
  ),
  'locales', coalesce((
