-- Follow-up to 20260925003000: living figures store death_date as '' as well
-- as NULL, so the living check must use nullif(death_date,'') is null.
-- Signature, filters, and result columns are unchanged.
begin;
...
begin
  ...
  order by
    ...
    case when p_sort_by = 'daily_recommend' then
      ...
      + 0.08 * (case when nullif(candidate.death_date, '') is null and candidate.computed_birth_year >= 1900 then 1 else 0 end)
