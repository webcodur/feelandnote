...
기본 상품 링크에는 제휴 수익이 없다. [애드온](https://developers.yes24.com/docs/addon)에 별도로 동의한 계정은 API가 반환한 `addOnLink`(`apis.yes24.com/a/<userKey>/goods/<상품번호>`)를 사용한다. 구매 영역의 공통 `구매 안내` 모달에 예스24 또는 쿠팡으로부터 수수료를 받을 수 있다는 고지 한 문장만 둔다. 목록은 구획당 안내 버튼 하나를 쓰며, 상세에서도 판매처별 안내를 반복하지 않는다. 애드온 동의는 즉시 활성화되고 해제할 수 없으므로 별도 명시적 승인을 받는다. API 키 발급·애드온 가입만으로 상업 이용 조건 확인을 대신하지 않는다. 서비스 공개 전 [공식 FAQ](https://developers.yes24.com/support/faq)에 따라 데이터 표시·캐시·다른 판매처 병기 방식을 문의한다.

애드온 정산 조건(2026-09 계정 확인): 주문 금액의 **3% 예치금** 또는 **3.3% YES포인트** 중 선택, 실적인정 **24시간**, 배송 완료 후 확정되어 **익월 10일** 지급. 예치금은 마이페이지 > 예치금 > 환불요청으로 계좌 출금이 되고, 포인트는 예스24 전용이다. 정산·실적 화면은 마이페이지 > 애드온 정산 관리(`/Member/FTMyAddon.aspx`)다 — 클릭수나 상품별 클릭 통계는 나오지 않고 출고 뒤 적립·정산만 보인다. **같은 예스24 링크에 링크프라이스를 섞으면 외부 프로그램이 우선되어 애드온 실적이 죽는다** — yes24 링크를 `linkmoa.kr` 등으로 감싸지 않는다.
...
한국어 추가 구매처로 쓰던 링크는 엉뚱한 책에 연결된 사례가 많아 전량 수거했다 — `figure_book_products`의 활성 쿠팡 상품은 모두 비활성이고 `content_locales.affiliate_url`의 쿠팡 항목은 모두 제거했다. 수거한 원본은 `data/coupang/removed-links-2026-09-16.json`에 있고, 도서분·비도서분의 후속 처리는 [`docs/todo/coupang-links.md`](../../todo/coupang-links.md)가 쥔다. 재편 전까지 화면에 쿠팡 단추가 나오지 않으며, 구매 안내 모달은 재편 중임을 고지한다. 다시 붙일 때는 아래 선정 기준으로 검증한 상품만 등록한다.
...
코드는 승인을 기다리는 상태로 이미 들어가 있다 — `coupangBookLink`(ISBN·제목 검색 딥링크), `resolveCoupangPurchaseRedirect`, `seller=coupang` 경유, 구매 모듈의 쿠팡 단추까지 만들어 뒀고 `bookPurchaseRedirect.ts`의 `LINKPRICE_COUPANG_APPROVED` 상수가 전부를 눌러두고 있다. 승인이 나면 이 상수를 `true`로 바꾸는 것만으로 교보와 같은 경로로 동작한다. 미승인 링크는 게이트웨이가 「승인되지 않은 링크입니다」 에러로내므로 상수를 켜기 전 화면에 세우면 안 된다.
