...
`celeb_explanations` 2,670명의 기존 글을 새 기준으로 전량 검수한다. 좋은 글은 본문을 보존하고
`review_status = 'ai_reviewed'`만 기록한다. 부족한 글은 자료 조사부터 한영 재작성·검수·DB
반영까지 마친 뒤 같은 상태를 기록한다.
...
| 공개 | 1,732 |
| 비공개 | 938 |
| `review_status IS NULL` | 2,667 |
| `ai_reviewed` | 3 |
| `human_reviewed` | 0 |
| 공개 상태 불일치 | 0 |

AI 검수 완료 3명은 시험 대상이다.
...
강제 중단 때문에 `run.lock`은 남아 있다. 기록된 PID `64936`은 종료됐으며 현행
`acquireRunLock()`은 다음 실행 때 생존 여부를 확인하고 stale lock을 자동 제거한다. 캐시
디렉터리는 삭제하지 않는다.
...
판정 캐시에만 있고 DB 상태는 `NULL`인 `pass` 1명과 `rewrite` 1명을 골라 시험한다.

확인할 것:

1. `pass` 캐시 인물은 즉시 상태 반영되고 본문·`updated_at`이 불필요하게 바뀌지 않는다.
2. `rewrite` 캐시 인물은 판정 단계에서 멈추지 않고 같은 레인 로그에 조사·생성·DB 반영까지 나온다.
...
- `ai_reviewed + human_reviewed = 2,670`
- 한국어·영어 여섯 필드 누락 0
- 활성 1,732명 공개, 비활성 713명과 정지 225명 비공개
- 공개 상태 불일치 0
