...
> 「저승 술집(hell-bar)」은 폐기됐다(2026-07-16). 삼국지 인물 페어 매치 전용 시리즈로 기획됐으나 구현된 적이 없다(컴포지션·데이터 전무). 기획서는 저장소에서 지웠다. 인물끼리 사상을 부딪치는 자리는 「가상 담화」([`discourse/`](discourse/README.md))가 대신하며, 담화는 2026-07-16 기준 삼국지 인물을 아직 쓰지 않는다(`qin-shi-huang`·`musk-altman` 2편).
...
> 마속·위연·장비·조운 등은 DB 미등록이라 명단에서 제외. 등록 시점에 추가. (폐기된 hell-bar 기획서가 페어 후보로 적어 두었던 인물들이다.)
