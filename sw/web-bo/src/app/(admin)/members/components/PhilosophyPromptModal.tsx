...
import { PHILOSOPHY_PROMPT } from '@/constants/celebPrompts'

interface PhilosophyPromptModalProps {
  isOpen: boolean
  onClose: () => void
  guessedName: string
}

export default function PhilosophyPromptModal({ isOpen, onClose, guessedName }: PhilosophyPromptModalProps) {
  const [copied, setCopied] = useState(false)

  const projectRulePrompt = guessedName
    ? PHILOSOPHY_PROMPT.replace('[입력된 인물]', guessedName)
    : PHILOSOPHY_PROMPT
    ...
    <div className="fixed inset-0 z-modal flex items-center justify-center">
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] bg-bg-card border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <h2 className="text-xl font-bold text-text-primary">감상 철학 프로젝트 룰</h2>
