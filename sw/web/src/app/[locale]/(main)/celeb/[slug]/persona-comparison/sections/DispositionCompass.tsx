...
export default function DispositionCompass({
  ...
  const formatValue = (
    ...
    if (Math.abs(value) <= 10) {
      return `${t("personaMatchModalNeutral")} ${signedValue}`;
    }
    return `${tl(value < 0 ? endpoints[0] : endpoints[1])} ${signedValue}`;
  };

  return (
    <ChartFrame title={title} hint={t("personaMatchGraphicDispositionHint")}>
      <div
        className={cn(
            ...
            >
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
                ...
                >
                  {t(
                    opposite
                      ? "personaMatchModalMirrorGap"
                      : "personaMatchModalGap",
                    { value: gap },
                  )}
                </span>
