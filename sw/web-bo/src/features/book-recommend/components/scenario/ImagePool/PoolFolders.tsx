...
export function PoolFolders({
  ...
  const renderFolder = (folder: string): React.ReactNode => {
    ...
    return (
      <div
        key={folder}
        className={`border rounded overflow-hidden ${dropActive ? 'border-accent ring-1 ring-accent/30 bg-accent/10' : 'border-border'}`}
  ...
  return (
    <div className="space-y-1 border border-border rounded p-1.5 bg-bg-main/20">
      ...
      {subFolders.length > 0 && (
        <div
          className={`mt-1 px-2 py-2 text-sm font-bold text-center rounded border-2 border-dashed ${
