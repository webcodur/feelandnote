...
아바타의 신원·구도·검수는 `docs/project/celeb/celeb-08-01-avatar.md`, 등록 실행은 `celeb-avatar-register` 스킬이 쥔다. 이 규격은 팩션 개인샷에는 적용하지 않는다.
...
실제 얼굴을 알 수 없는 인물은 얼굴을 추정하지 않고 은폐된 정체성을 보존한다. 익명 화보와 REF 설계는 `faction-image`의 「신원 비공개 인물」, 아바타 후보 판정과 등록은 `celeb-avatar-register`를 따른다.

`_refs`는 생성 재료이지 업로드 완성본이 아니다. 해당 인물의 완성 개인샷을 직접 확인하고, 배경 제거가 필요하면 `nobg-cutout`을 거친 뒤 등록한다. 등록 후에는 R2 재다운로드본과 서비스의 원형·확대 표시를 확인하고 팩션 출간 패널의 진단과 dry-run을 다시 실행한다.

## 수동 REST 폴백 (파이프라인 장애 시에만)

데이터 CRUD는 DB REST(PostgREST + `DB_SECRET_KEY`)로 가능하다. DDL·SQL은 Oracle DB VM에 SSH로 접속해 `supabase-db` PostgreSQL 컨테이너에서 실행한다.
```
