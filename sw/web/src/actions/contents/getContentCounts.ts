...
// 특정 사용자의 공개 WANT 콘텐츠 타입별 개수 반환
export async function getUserWantContentCounts(userId: string): Promise<ContentTypeCounts> {
  const supabase = await createClient()
  ...
  if (error) {
    console.error('사용자 관심 콘텐츠 개수 조회 에러:', error)
    return { BOOK: 0, VIDEO: 0, GAME: 0, MUSIC: 0, CERTIFICATE: 0 }
  }
