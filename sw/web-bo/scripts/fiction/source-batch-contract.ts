...
export type FictionSourceBatchCharacter = {
  slug?: string
  celebId?: string
...
export type FictionSourceBatchManifest = {
  contentId: string
  title?: string
  characters: FictionSourceBatchCharacter[]
}

export type ResolvedFictionSourceCharacter = Omit<
  FictionSourceBatchCharacter,
  'slug' | 'celebId'
> & {
  celebId: string
  slug: string
...
export type FictionSourceCharacterRow = {
  content_id: string
  celeb_id: string
...
export type FictionSourceBatchChange = {
  kind: 'insert' | 'update' | 'unchanged'
  slug: string
  before: FictionSourceCharacterRow | null
  after: FictionSourceCharacterRow
}

export type FictionSourceBatchPlan = {
  expectedRows: FictionSourceCharacterRow[]
  changes: FictionSourceBatchChange[]
...
export function parseFictionSourceBatchManifest(input: unknown): FictionSourceBatchManifest {
  ...
  const characters = raw.characters.map((value, index): FictionSourceBatchCharacter => {
...
function normalizedRow(row: FictionSourceCharacterRow): FictionSourceCharacterRow {
  return {
...
function rowKey(row: FictionSourceCharacterRow): string {
  return `${row.content_id}\u0000${row.celeb_id}`
}

function sortRows(rows: FictionSourceCharacterRow[]): FictionSourceCharacterRow[] {
  return rows
    .map(normalizedRow)
    .sort((left, right) => (
...
function sameRow(
  left: FictionSourceCharacterRow,
  right: FictionSourceCharacterRow,
): boolean {
 ...
 */
export function buildFictionSourceBatchPlan(
  contentId: string,
  characters: ResolvedFictionSourceCharacter[],
  currentRows: FictionSourceCharacterRow[],
): FictionSourceBatchPlan {
  const existingByCelebId = new Map<string, FictionSourceCharacterRow>()
  ...
  for (const character of characters) {
    ...
    const sortOrder = character.sortOrder
      ?? before?.sort_order
      ?? nextSortOrder++
    const after: FictionSourceCharacterRow = {
...
export function assertExactFictionSourceReadback(
  expectedRows: FictionSourceCharacterRow[],
  actualRows: FictionSourceCharacterRow[],
): void {
