...
표시값은 **조사 상태 하나만** 본다. 등급도 노출 여부도 개입하지 않는다.
...
신규 인물은 `open`으로 시작하므로 **비공개로 만들어도 표시값은 `0`이다.**

**조사를 마친 인물은 결과가 둘로 갈릴 뿐 둘 다 재조사 대상이 아니다.** 콘텐츠를
찾았으면 `full`(표시값 양수), 없었으면 `confirmed_empty`(표시값 `-1`). 그래서
조사 장부 모집단에서 `full`이 빠진다 — 이미 조사를 거쳤기 때문이다. `fiction`은
허구 인물이라 콘텐츠 개념 자체가 없고, `suspended`는 서비스에서 내린 인물이다.

**운영은 단순하다 — 조사 장부 화면에서 `0`으로 보이면 그게 조사 대상이다.** 화면이
