  ...
  return (
    <Fragment>
      ...
      <div className="relative group/del" style={accentStyle(pair.quoteSpeaker)}>
        ...
        />
        <button onClick={() => removeQuotePair(i, pi)} className="absolute top-1 right-6 text-sm font-bold text-red-400 hover:text-red-300 opacity-0 group-hover/del:opacity-100">삭제</button>
      </div>
        ...
        return parts.map((partText: string, ap: number) => {
          ...
          return (
            <div key={key} className="relative group/del" style={accentStyle(spk)}>
                  ...
                  <>
                    {ap > 0 && (
                      <button
                        ...
                        className="px-2 py-0.5 text-xs font-bold text-text-secondary border border-border/40 rounded hover:border-accent/40 hover:text-accent"
              ...
              />
              {first && (
                <button onClick={() => updateAfterSplit(i, pi, [])} className="absolute top-1 right-6 text-sm font-bold text-red-400 hover:text-red-300 opacity-0 group-hover/del:opacity-100">삭제</button>
              )}
            </div>
          )
        })
