...
const activeProfile: FictionProfileSnapshot = {
...
function candidate(): FictionCandidate {
  return {
...
test('legacy active candidate remains valid without profile metadata', () => {
  const result = validateFictionCandidate(candidate())
...
test('inactive candidate requires matching profile status and fingerprint metadata', () => {
  ...
  assert.ok(validateFictionCandidate(missingFingerprint).errors.some(
  ...
  const complete = {
    ...
    profile_fingerprint: fingerprintFictionProfile(inactiveProfile),
  }
  assert.deepEqual(validateFictionCandidate(complete).errors, [])
})
...
test('fiction candidate rejects a profile without a connected source work', () => {
  ...
  assert.ok(validateFictionCandidate(unlinked).errors.some(
...
test('profile fingerprint changes when publication status changes', () => {
  assert.notEqual(
    fingerprintFictionProfile(activeProfile),
    fingerprintFictionProfile({ ...activeProfile, publication_status: 'inactive' }),
  )
})
