...
export default function ScripturesBanner() {
  const pathname = usePathname();
  const router = useRouter();
  const tNav = useTranslations("nav");
  const tHome = useTranslations("home");
  const tAcademy = useTranslations("scriptures.academy");

  const hubTitle = tNav("scriptures");
  const hubEnglish = tHome("scriptures.englishTitle");

  // segments: ["scriptures", "academy", "video", "composition"]
