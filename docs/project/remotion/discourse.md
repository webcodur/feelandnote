...
> 통합 설계·현황: [discourse-unification.md](./discourse-unification.md)
...
초기 원천은 DB에 보존된 **가상 독백**(`celebs.virtual_monologue`)이다. 서비스 화면 노출과 신규 작성은 중단됐지만 기존 값은 담화 제작 재료로 보존한다. 영상에서는 이 재료를 그대로 낭독하지 않고 다른 인물과 주고받는 대사로 재작성한다.
...
| 항목 | 현황 |
|------|------|
| 저장 | `celebs.virtual_monologue`(한국어 원문) · `virtual_monologue_en`(영문본) |
| 보유 | 1,692명 (물리 분리 전 2026-07-21 실측 스냅샷) |
...
| 생성 | 중단. 과거 생성 도구와 운영 규칙은 `docs/project/celeb/retire/virtual-monologue.md`에 보존 |
| 노출 | 중단. 기존 DB 값은 담화·읽어보기 제작 재료로만 사용 |

**영상에 그대로 못 쓴다.** 800~1,200자 산문은 낭독 2분 분량이고, 문어체라 대담 발화로 어색하다. 보존 독백은 기존에 검토한 제작 재료로만 쓰고, 영상 대사는 원전·실제 저작·발언과 함께 대조해 재작성한다.

- 영상 대사가 보존 독백과 충돌하면 원전·실제 저작·발언을 우선해 교정한다.
- 영상 작업에서 확보한 내용을 가상 독백 칼럼에 백필하지 않는다. 담화 원고와 출처 자료에 남긴다.
...
4. **사상 근거** — 모든 대사는 서비스 가상 독백 또는 실제 저작·발언에 근거해야 한다. 인물이 하지 않았을 말을 시키지 않는다.
...
**담화는 두 사람이 마주 앉은 자리다.** 그런데 서비스 가상 독백(`celebs.virtual_monologue`)에서 사상을 길어 오다 보면, 인물이 이 논제와 무관한 자기소개를 허공에 대고 읊는 것으로 첫 컷이 채워진다. musk-altman 초고가 정확히 그랬다 — 머스크가 문제 푸는 방식(물리 법칙 우선)을 15초 동안 말한 뒤에야 논제가 등장했고, 알트만은 논제를 말하는데 머스크는 방법론을 말하는 **서로 딴소리 상태**로 시작했다. 대담이 아니라 연설 두 개였다.
...
「담화」는 혼자 하는 말과 주고받는 말을 함께 덮는 낱말이라 **독백편·대담편을 한 이름 아래 둘 수 있다**. 앞머리 「가상」이 본서비스의 「가상 독백」과 이어져 같은 계열임이 읽힌다.
