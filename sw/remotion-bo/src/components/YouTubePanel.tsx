...
export function YouTubePanel({ series, name, post }: Props) {
  ...
  const handleUpload = async (lang?: string, type?: string, shortsIndex?: number) => {
    // shortsIndex 1-based. 단일 쇼츠(1)이면 레이블 숫자 생략.
    const idxLabel = (type === 'shorts' && shortsIndex != null && shortsIndex > 1) ? ` #${shortsIndex}` : ''
    const label = [lang?.toUpperCase(), type].filter(Boolean).join(' ') + idxLabel || '전체'
    ...
    const res = await fetch(`/api/${series}/youtube/upload`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ episode: baseName, lang, type, shortsIndex, dry: dryRun }),
    })
    if (!res.ok) {
