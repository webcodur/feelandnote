/** Apply explicitly reviewed ko→en translations; this script never generates or guesses a translation.
 * node --env-file=.env --import tsx scripts/contents/apply-book-introduction-translations-koen.ts --plan FILE [--apply]
 */
...
import { planIntroductionTranslationKoEn, type ReviewedIntroductionTranslation } from './book-introduction-translation-contract'
...
async function main() {
  ...
  const planned = inputs.map(input => ({ input, change: planIntroductionTranslationKoEn(input) }))
  const db = createClient(apiUrl, secret, { auth: { persistSession: false, autoRefreshToken: false } })
  const run = `translation-ko-en-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}`
  ...
  try {
    for (const { input, change } of planned) {
      ...
      const { data: current, error } = await db.from('content_locales').select('description').eq('content_id', id).eq('locale', 'en').single()
      ...
      if (values.apply) {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          const { data: row, error: readError } = await db.from('content_locales').select('description,isbn,sources').eq('content_id', id).eq('locale', 'en').single()
          if (!readError && row.description === change.description && row.isbn === change.before.isbn
            ...
            && row.sources?.description_source_locale === 'ko') { confirmed = true; break }
          await delay(500)
        }
