...
description: 셀럽의 DB 대사 22종을 ElevenLabs 또는 Gemini로 합성하고 검증해 R2에 등록할 때 사용한다. “대사 음원 생성”, “보이스 22개 박기”, “한국어·영문 보이스 생성과 업로드”, “끝부분 절단이나 후행 웅얼거림 복구” 요청에 적용한다. Remotion 내레이션에는 사용하지 않는다.
---

# 셀럽 대사 음성 생성·등록

셀럽 slug와 locale별로 `대사 22종 조회 → 직접 합성 → 검증 → R2 등록`을 수행한다. 기본 생성은 DB 대사를 그대로 보내고, 합성 직후 들숨 무음 처리와 쉼 정리만 자동으로 거친다. 말줄임표·안전 문구·후행 절단은 아래 복구 경로에서만 쓴다.
