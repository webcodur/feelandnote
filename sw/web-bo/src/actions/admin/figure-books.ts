...
export interface FigureBookProductAdminItem {
  id: number
...
export interface FigureBookEditionAdminItem {
  id: number
  contentId: string
  locale: 'ko' | 'en'
  title: string
...
export interface FigureBookAdminItem extends FigureBookContentSummary {
  characterIds: string[]
...
export interface FigureBookCharacterOption {
  id: string
  slug: string
  nickname: string
...
export interface FigureBookAdminData {
  sources: FigureBookAdminItem[]
...
async function loadContentSummaries(
  contentIds: string[],
): Promise<FigureBookContentSummary[]> {
...
export async function getFigureBookAdminData(): Promise<FigureBookAdminData> {
  await requireAdmin()
  ...
  const assignmentsByContent = new Map<string, FigureBookCharacterAssignment[]>()
  const productsByEdition = new Map<number, FigureBookProductAdminItem[]>()
  ...
  const editionsByContent = new Map<string, FigureBookEditionAdminItem[]>()
  ...
  const sources = sourceRows.flatMap((row): FigureBookAdminItem[] => {
  ...
  const characters: FigureBookCharacterOption[] = characterRows.map((row) => ({
    id: row.id,
    slug: row.slug,
...
export async function searchFigureBookCandidates(
  rawQuery: string,
): Promise<FigureBookContentSummary[]> {
  await requireAdmin()
...
export async function saveFigureBook(input: {
  contentId: string
  relations: Array<{
    celebId: string
    relationType: FigureBookRelationType
  ...
  await revalidateWebItems(
    ...
    [CACHE_TAGS.FIGURE_BOOKS, CACHE_TAGS.CELEBS, CACHE_TAGS.CONTENTS],
  )
}

export async function saveFigureBookCharacterDescription(input: {
  contentId: string
  celebId: string
  description: string
...
async function revalidateSourceCatalog(contentId: string) {
  ...
  await revalidateWebItems(
    [{ domain: CACHE_TAGS.CONTENTS, id: contentId }],
    [CACHE_TAGS.FIGURE_BOOKS],
  )
}

export async function saveFigureBookEdition(input: {
  editionId?: number
  contentId: string
  locale: string
  title: string
...
export async function deactivateFigureBookProduct(input: {
  editionId: number
  platform: string
}): Promise<void> {
  await requireAdmin()
...
export async function removeFigureBook(contentId: string): Promise<void> {
  await requireAdmin()
