...
//
// 모델: gemini-3.1-flash-tts-preview (2026-04-15 공개)
// - 200+ audio tag 지원: [whispering], [excited], [sad], [pause: 0.5s] 등 텍스트 inline 제어
// - 70+ 언어, Elo 1211 (Artificial Analysis TTS 리더보드)
// - 단가: $1.00/1M input, $20.00/1M output (2.5 대비 2배)
//
...
export async function POST(req: Request, { params }: { params: Promise<{ series: string }> }) {
  ...
  while (true) {
    try {
      ...
      const wav = await cleanVoiceBuffer(wrapPcmAsWav(pcm, 24000, 1, 16), 'wav', 'reading')
