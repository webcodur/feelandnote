...
그리드 아래에는 분야별 챔피언·스펙트럼·신화의 세계·세력도감을 그림과 설명이 있는 2×2 카드로 배치한다. 연대기·디렉토리는 보조 카드다. 주요 카드와 푸터의 인물 메뉴는 `EXPLORE_FEATURED_LINKS`를 공유한다. 인물 피드·오늘의 인물은 탐색 메뉴에서 제외하고 기존 주소는 유지한다.
...
- **인물** — 세력에 배정된 `celebs` 행. 대표 인물(`lead_person_ids`)은 세력 타이틀 아트에 세우는 3인으로 배열 순서가 세우는 순서다.
