/**
 * 상황 대사 21줄의 영문판을 muse-spark(opencode)로 만든다. 생성만 하고 DB는 건드리지 않는다.
 *
 ...
 * 기본은 OpenCode Go 라인(opencode-go/muse-spark-1.3-contributor). --line free 로 무료 라인을 쓴다.
 * 무료 라인은 소모가 빠르므로 동시 수를 낮춘다(1~2). 라인 상태 진단은 opencode-muse 스킬이 쥔다.
 *
 * 실행 (sw/web-bo 에서):
 *   node scripts/celeb/muse-lines-en.mjs --list
 *   node scripts/celeb/muse-lines-en.mjs --conc 8 --group 3
 *   node scripts/celeb/muse-lines-en.mjs --slugs rurik,rollo
 *   node scripts/celeb/muse-lines-en.mjs --line free --conc 1
 */
...
const LINE = arg('--line', 'go')
const MODEL = LINE === 'free' ? MUSE_FREE : MUSE_GO
const CONC = Number(arg('--conc', LINE === 'free' ? 1 : 8))
...
Do not use web search or any tools. Translate directly from the Korean lines given. Do not run scripts to count characters.
...
async function main() {
  ...
  console.log(`대상 ${todo.length}명 / 완료 ${done.size}명 / 묶음 ${groups.length}개 (묶음당 ${GROUP}명, 동시 ${CONC}, 라인 ${LINE})`)
  if (LIST_ONLY || !groups.length) return

  let made = 0
  let failed = 0
  let consecutiveFail = 0
  for (let i = 0; i < groups.length && consecutiveFail < 5; i += CONC) {
    await Promise.all(groups.slice(i, i + CONC).map(async (g) => {
      const label = g.map((r) => r.name).join('·')
      try {
        const r = await museCall(buildPrompt(g), { model: MODEL, timeoutMs: 300000 })
        if (!r.text?.trim()) throw new Error('muse 빈 출력')
        const { ok, issues } = validate(r.text, g)
        for (const o of ok) {
        ...
        if (!ok.length) {
          ...
          consecutiveFail = 0
          console.log(`OK   ${label} → ${ok.length}/${g.length}명${issues.length ? `\n     ${issues.join(' | ')}` : ''}`)
        }
      } catch (e) {
        failed++; consecutiveFail++
  ...
  if (consecutiveFail >= 5) {
    console.log(`중단: 연속 실패 5회 — 라인 상태를 먼저 확인한다. node ../../.agents/skills/opencode-muse/scripts/muse-call.mjs "3 곱하기 7은?"`)
  }
