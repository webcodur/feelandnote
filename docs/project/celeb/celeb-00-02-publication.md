...
허용 티어와 목록·검색·색인 게이트는 `packages/shared/src/constants/celeb-tiers.ts`가 유일한 SSoT다. 현재 full·light는 기본 목록에, fiction은 상단 인물 검색과 직접 상세 접근에 포함된다. 노출 범위를 바꿀 때는 문서 표를 고치는 대신 그 코드 상수와 소비처를 함께 수정한다.
