...
### 🔴 실제 상태 — 전역 퍼지가 이미 활성이다 (태그 국소화가 최우선)

**egress 감사 문서의 "`CRON_SECRET` 미설정" 기록은 낡았다. 실측 결과 프로덕션에 설정돼 있다.**
...
- 의미: **BO 저장 1회마다 `revalidateTag('celebs', {expire:0})`가 실제로 실행된다.** 그런데 egress 감사 ⑤(태그 국소화)가 미착수라 캐시 약 70곳이 전부 단일 태그 `['celebs']`다 → **저장 1회가 7일 캐시를 포함한 전 캐시를 즉시 전멸**시킨다. 호출부는 `web-bo/src/actions/admin/`(celebs·contents·dialogues)에 **42곳**이라 셀럽 대량 작업 시 저장마다 반복된다.
...
**미완**: GSC 사이트맵 재제출은 API 권한 부족(403)으로 실패했다. 사이트맵 자체는 등록돼 있고 오류 0이나 `lastDownloaded`가 2026-03-26이라 오래됐다 → **유저가 GSC에서 직접 재제출**해야 즉시 재크롤이 걸린다.
...
2. **사이트맵 재제출** (GSC) — **유저 액션 필요**. URL이 2,196 → 15,884로 늘어난다.
