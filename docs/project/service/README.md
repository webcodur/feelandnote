...
기록물의 감상 목록은 **목록 ↔ 펼침** 두 보기를 오간다. 펼침은 목록에서 고른 한 건을
...
작품 소개·정보는 화면에 뜬 한 건만 `getContentBrief`가 바깥(카카오·TMDB·IGDB·iTunes)에서 받아
채우고 앞뒤 한 건을 미리 챙긴다. 음악은 저장된 iTunes metadata가 있으면 외부를 다시 조회하지 않는다. 소개에는 `책 소개` 또는 `작품 소개` 라벨을 붙이고, 문자열로 넘어온 HTML 엔티티를
