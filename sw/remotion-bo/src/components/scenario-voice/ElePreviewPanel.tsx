...
export function ElePreviewPanel({ blobUrl, duration, onSave, saving, onClose, label = 'ELE preview', tone = 'purple', autoPlay, onRegenerate, regenerating, playbackRate, gainDb }: {
  ...
  /** 재생 배속 — 파형 플레이어·자체 재생 버튼에 반영. 미지정이면 1배속. */
  playbackRate?: number
  /** 음량 dB 게인 — 자체 재생 버튼 audio.volume 에 반영. 미지정이면 0dB. */
  gainDb?: number
}) {
  ...
  const togglePlay = () => {
    ...
    audioRef.current = a
    if (playbackRate && Number.isFinite(playbackRate) && playbackRate > 0) a.playbackRate = playbackRate
    a.volume = dbToLinear(gainDb)
