...
async function setPostDeleted(id: string, deleted: boolean) {
  ...
  const { error } = await db.from('free_posts').update({ is_deleted: deleted }).eq('id', id)
...
async function setCommentDeleted(id: string, deleted: boolean) {
  const db = createAdminClient()
  const { error } = await db.from('free_post_comments').update({ is_deleted: deleted }).eq('id', id)
  if (error) throw new Error(error.message)
  revalidatePath('/free-board')
}
