...
- 이번 증분 처리: 19명
- 완료 상태: R2 재업로드, `profiles.avatar_url` 갱신, 800×800 RGBA WebP 원격 재검증, 임시 이미지 삭제 완료
...
1. DB `profiles`에서 `profile_type='CELEB'`이고 `avatar_url`이 있는 행을 조회한다.
2. `profiles`에는 `updated_at`이 없으므로 `avatar_url`의 `?v=<밀리초 시각>`가 위 완료 시점보다 최신인 행을 1차 후보로 잡는다.
