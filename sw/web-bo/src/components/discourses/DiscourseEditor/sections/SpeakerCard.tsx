...
import { ChevronUp, ChevronDown, Trash2, Eye, EyeOff } from '@feelandnote/shared/bo/icons'
import { CelebBadge } from '../../shared/CelebBadge'
import { imageSrc } from '../../shared/timing'
import { MediaThumb, ImageLightbox, ImageSlot, DISCOURSE_IMAGE_DND } from '@feelandnote/shared/bo/media'
      ...
      <div className="flex items-center gap-2 p-2.5">
        ...
        <button onClick={() => setOpen(v => !v)} className="min-w-0 flex-1 text-start hover:text-accent">
          <span className="flex items-center gap-1.5">
            ...
            {speaker.living && (
              <span title="생존 인물 — 고지·편성 판단이 따로 필요합니다(§3 고지 원칙)" className="shrink-0 rounded bg-info/15 px-1 text-[10px] font-bold text-info-text">생존</span>
            )}
            {speaker.disabled && <span className="shrink-0 rounded bg-danger/15 px-1 text-[10px] font-bold text-danger-text">영상 제외</span>}
          </span>
        ...
        <span className="flex shrink-0 items-center gap-0.5">
          ...
          <button onClick={onDelete} className="rounded p-1 text-danger-text hover:bg-danger/20" title="인물 삭제">
            <Trash2 size={14} />
          </button>
