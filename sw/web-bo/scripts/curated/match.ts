...
/**
 * 같은 사람의 다른 표기를 묶은 대응표(lib/author-aliases.json).
...
async function main() {
  ...
  loadAliases(scriptsPath('curated', 'lib', 'author-aliases.json'))
  ...
  const reportPath = scriptsPath('curated', '.tmp', 'match-report.json')
  writeFileSync(
    reportPath,
    JSON.stringify(
