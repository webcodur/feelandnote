...
export function YouTubePanel({ series, name, post }: Props) {
  ...
  const autoGenerated = useMemo(() => {
    ...
    if (!status?.lineup) return result
    const lineup = status.lineup
    ...
    for (const key of VARIANT_KEYS) {
      ...
      const title = buildTitle(lineup, celebName, lang, isShorts)
  ...
  const disabled = !lineup || (!auth.ko.authenticated && !auth.en.authenticated)
