...
- 페르소나(`celeb_persona`)가 있으면 스탯·Grade 산정에 활용
...
### 페르소나 스탯 (`celeb_persona` 기반, 16종) — 주 체계

`dbToCharacter()`가 페르소나 16값을 그대로 캐릭터 스탯으로 쓴다.
...
`celeb_persona`가 없는 인물에만 쓴다. `celeb_influence` 값을 스케일해 페르소나 자리에 채운다. 인물 대부분이 페르소나를 갖고 있으므로 실사용 빈도는 낮다.
...
> **대부분 미구현이다.** 아래는 페르소나 값을 내정 전반에 연결하는 설계이고, 현재 코드는 훨씬 단순한 규칙을 쓴다. 각 항목에 실제 구현을 병기한다.
