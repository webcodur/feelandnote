 ...
 *
 * pnpm exec tsx scripts/celeb/timeline/apply-narrative-candidates.ts --root <root> --slugs=achilles --dry
 * pnpm exec tsx scripts/celeb/timeline/apply-narrative-candidates.ts --root <root> --slugs=achilles --apply
 * pnpm exec tsx scripts/celeb/timeline/apply-narrative-candidates.ts --root <root> --slugs=absyrtus --allow-inactive --dry
 * pnpm exec tsx scripts/celeb/timeline/apply-narrative-candidates.ts --root <root> --all-pending --recover
 */
...
function loadCandidate(root: string, slug: string): NarrativeCandidate {
  const file = candidateFile(root, slug)
  if (!existsSync(file)) throw new Error(`${slug}: 후보 파일이 없다: ${file}`)
  const parsed: unknown = JSON.parse(readFileSync(file, 'utf8'))
  const validation = validateNarrativeCandidate(parsed)
...
function loadJournal(file: string): ApplyJournal {
  ...
  if (journal.schema_version !== NARRATIVE_CANDIDATE_SCHEMA_VERSION
...
function ensureBackup(
  ...
  currentSourceSnapshot: FigureBookSnapshot,
) {
...
function buildJournal(candidate: NarrativeCandidate, current: StoredTimelineEvent[]): ApplyJournal {
...
async function validateProfileState(input: {
  ...
  if (input.profileFingerprint
    && fingerprintNarrativeProfile(profile) !== input.profileFingerprint) {
    throw new Error(`${input.slug}: 후보 작성 뒤 인물 프로필 지문이 바뀜`)
  }
  return profile
}

async function validateLiveState(candidate: NarrativeCandidate, allowInactive: boolean) {
  ...
  const [current, currentSourceSnapshot] = await Promise.all([
    ...
    loadFigureBookSnapshot(db, candidate.celeb_id),
  ])
