...
- 서가 도서 순위는 [예스24 일별 베스트셀러 API](https://developers.yes24.com/api-doc/category-bestseller-daily)와 [미국 Apple Books 유료 차트](https://rss.marketingtools.apple.com/)를 서버에서 읽는다. 예스24 상업 이용·보관·노출 방식은 [공식 FAQ](https://developers.yes24.com/support/faq)에 따라 출시 전에 문의하고 확인된 조건으로 활성화한다. Apple 차트는 원본 작품 링크를 유지하며 미국 전체 판매순위로 표현하지 않는다. 조회·검증은 `src/lib/library/bestsellerFeed.ts`, 캐시는 `src/actions/library/bestsellers.ts`, 키와 활성화 설정은 [환경변수](env-vars.md)가 담당한다. 외부 페이지를 긁어 공개 JSON을 커밋하던 주간 수집 작업은 폐기했다.
