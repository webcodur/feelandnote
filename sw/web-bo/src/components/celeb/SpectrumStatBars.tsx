...
import type { StatKey, TendencyKey } from '@/actions/admin/spectrum'
import type { SpectrumJsonb } from '@/actions/admin/members'
...
// #region Types
export interface SpectrumStatEntry {
  id: string
  nickname: string
  spectrum?: SpectrumJsonb | null
  [key: string]: string | number | SpectrumJsonb | null | undefined
}

interface SpectrumStatBarsProps {
  spectra: SpectrumStatEntry[]
  showLegend?: boolean
  compact?: boolean
}
// #endregion
...
function getReason(jsonb: SpectrumJsonb | null | undefined, key: string): string | undefined {
...
function CompareStatRow({ label, spectra, statKey }: { label: string; spectra: SpectrumStatEntry[]; statKey: string }) {
  return (
...
function CompareTendencyRow({ labels, spectra, statKey }: { labels: [string, string]; spectra: SpectrumStatEntry[]; statKey: string }) {
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-base text-text-secondary">
        <span>{labels[0]}</span>
...
export default function SpectrumStatBars({ spectra, showLegend = true, compact = false }: SpectrumStatBarsProps) {
  const [tab, setTab] = useState<TabType>('ability')

  if (spectra.length === 0) return null

  const isSingle = spectra.length === 1
  const p0 = spectra[0]
  const color0 = COLORS[0]

  const jsonb = isSingle ? p0.spectrum : undefined
  ...
  const content = {
    ability: isSingle ? (
      ...
      <div className="space-y-3">
        {ABILITY_KEYS.map((key) => (
          <CompareStatRow key={key} label={STAT_LABELS[key]} spectra={spectra} statKey={key} />
        ))}
      </div>
    ),
    virtue: isSingle ? (
    ...
    tendency: isSingle ? (
      ...
      <div className="space-y-4">
        {TENDENCY_KEYS.map((key) => (
          <CompareTendencyRow key={key} labels={TENDENCY_LABELS[key]} spectra={spectra} statKey={key} />
        ))}
      </div>
    ),
  }
