 ...
 *
 * 조사(그록)와 무관한 순수 조회 작업이다. 이미 확인된 장소명에 좌표만 붙이며
 * 사건의 사실관계는 건드리지 않는다.
 *
 * 실행:
...
const SETTLEMENT = /(도시|수도|행정 ?구역|자치|특별시|광역시|[가-힣]+시\b|[가-힣]+군\b|마을|읍|면|city|town|capital|municipality|commune|village|settlement|prefecture|county)/i
...
function queryForms(name: string): string[] {
  ...
  // 「경기도 수원시」처럼 광역+기초가 붙은 경우 마지막 토큰이 실제 도시다.
...
async function lookup() {
  ...
  console.log(`조회 대상 ${targets.length}개 이름 / 사건 ${targets.reduce((s, [, n]) => s + n, 0)}건`)

  const hits: Hit[] = []
  let miss = 0
  for (const [place, events] of targets) {
    const got = await lookupOne(place)
    if (got) hits.push({ place, events, ...got })
    else miss++
    if ((hits.length + miss) % 50 === 0) console.log(`  진행 ${hits.length + miss}/${targets.length} (성공 ${hits.length})`)
    await sleep(120)
  }
