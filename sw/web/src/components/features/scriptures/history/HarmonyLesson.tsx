/*
  파일명: /components/features/scriptures/history/HarmonyLesson.tsx
  기능: 화성학 레슨 뷰
...
import type { LessonSection, SheetExample } from "@/constants/scripturesHistory";
...
function DifficultyBadge({ difficulty }: { difficulty: keyof typeof DIFFICULTY_STYLES }) {
  const t = useTranslations("scriptures.history.lesson.difficulty");
...
function LessonCard({ lesson, index }: { lesson: LessonSection; index: number }) {
  ...
  const t = useTranslations("scriptures.history.lesson");
