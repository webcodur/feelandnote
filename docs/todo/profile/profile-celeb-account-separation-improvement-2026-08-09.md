...
> **최종 실측 체크: 26.08.09** — 운영 DB의 행 분포, 외래키, 트리거, RLS, RPC,
> 함수 권한을 대조했다. 추가형 분리 마이그레이션과 사용자 웹·백오피스 빌드도 확인했다.
>
> 상태: **보안 선처리·2단계 분리·3단계 새 부모 FK 추가 완료 · 기존 쓰기 전환 전**
...
현재 `profiles`가 유일한 쓰기 원천이다. 기존 53개 외래키는 PostgREST 조인 보존을 위해
새 부모 외래키와 함께 유지한다. `user_contents`, `follows`, 백오피스 셀럽 쓰기는 아직
기존 구조를 사용한다. 관계 조인이 필요한 조회도 기존 FK를 제거할 때까지 `profiles`를
유지한다. 원격 전체 스키마의 빈 DB 기준선 정리는 별도 미완료다.

### `profile_type` 확산

26.08.09 작업본에서 `profile_type`은 숨김 자동화 폴더를 제외하고 154개 파일에 362회
...
한 번에 전체를 바꾸지 않는다. 아래 세로 묶음마다 추가→변경 포착→백필→읽기 전환→쓰기
전환→관찰→기존 구조 제거 순서를 반복한다.
...
이행 기간에는 기존 테이블을 유일한 쓰기 원천으로 유지한다. 새 코드는 먼저 새 테이블을
읽되 쓰기는 기존 창구를 사용한다. 모든 구버전 인스턴스 종료를 확인한 뒤 짧게 쓰기를
중지하고 마지막 차이를 대조한 다음 새 쓰기로 전환한다. 양방향 트리거는 사용하지 않는다.
...
- 양방향 트리거를 동시에 두어 순환 갱신을 만들지 않는다.
