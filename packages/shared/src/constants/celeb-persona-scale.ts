// 인물 16축 스펙트럼(celeb_persona.persona) 채점 척도 — Single Source of Truth
//
// 이 파일이 유일한 척도다. 문서·백오피스 화면·채점 작업 지시문은 모두 여기를 참조한다.
// 수치를 다른 곳에 옮겨 적지 마라. 26.08.08 통합 이전에는 척도가 두 벌이었고 서로 어긋나 있었다.
//   - docs/project/celeb/celeb-5-persona.md : 실존 인물 기준 (무력 한 축만)
//   - sw/web-bo/src/constants/personaReferencePoints.ts : KOEI 삼국지 기준 (능력 네 축)
...
export type PersonaAxis = AbilityKey | InnerVirtueKey | OuterVirtueKey | DispositionKey
...
export const PERSONA_AXES: readonly PersonaAxis[] = [
  ...ABILITY_KEYS,
  ...INNER_VIRTUE_KEYS,
  ...OUTER_VIRTUE_KEYS,
  ...DISPOSITION_KEYS,
...
export const AXIS_LABELS: Record<PersonaAxis, string> = {
  command: '통솔',
  martial: '무력',
  intellect: '지력',
  charm: '매력',
...
export const AXIS_DEFINITIONS: Record<PersonaAxis, string> = {
...
export function axisRange(axis: PersonaAxis): { min: number; max: number } {
  return (DISPOSITION_KEYS as readonly string[]).includes(axis) ? DISPOSITION_RANGE : ABILITY_RANGE
}

export function isScoreInRange(axis: PersonaAxis, score: number): boolean {
  const { min, max } = axisRange(axis)
  return Number.isInteger(score) && score >= min && score <= max
}

/** 같은 배치에서 두 인물에게 같은 점수를 주지 않는다. 최소 간격. */
export const MIN_SCORE_GAP = 3
...
const COMMAND: readonly PersonaAnchor[] = [
...
const MARTIAL: readonly PersonaAnchor[] = [
...
const INTELLECT: readonly PersonaAnchor[] = [
...
const CHARM: readonly PersonaAnchor[] = [
...
const TEMPERANCE: readonly PersonaAnchor[] = [
...
const DILIGENCE: readonly PersonaAnchor[] = [
...
const REFLECTION: readonly PersonaAnchor[] = [
...
const COURAGE: readonly PersonaAnchor[] = [
...
const LOYALTY: readonly PersonaAnchor[] = [
...
const BENEVOLENCE: readonly PersonaAnchor[] = [
...
const FAIRNESS: readonly PersonaAnchor[] = [
...
const HUMILITY: readonly PersonaAnchor[] = [
...
const PESSIMISM_OPTIMISM: readonly PersonaAnchor[] = [
...
const CONSERVATIVE_PROGRESSIVE: readonly PersonaAnchor[] = [
...
const INDIVIDUAL_SOCIAL: readonly PersonaAnchor[] = [
...
const CAUTIOUS_BOLD: readonly PersonaAnchor[] = [
...
export const PERSONA_ANCHORS: Record<PersonaAxis, readonly PersonaAnchor[]> = {
  command: COMMAND,
  martial: MARTIAL,
  intellect: INTELLECT,
  charm: CHARM,
 ...
 */
export const THREE_KINGDOMS_ANCHORS: Partial<Record<PersonaAxis, readonly PersonaAnchor[]>> = {
  martial: [
...
export const SCORING_PRINCIPLES: readonly string[] = [
  '직군은 참고일 뿐이다. 실제 행적이 점수를 결정한다. 행적 정보가 전혀 없을 때만 직군 기본값으로 내려간다.',
  '같은 배치 안에서 같은 점수를 주지 않는다. 최소 3점 차를 둔다. 티어 하한값에 기계적으로 찍지 마라.',
  '고점이든 저점이든 근거가 필요하다. 15점에도 95점에도 행적을 명시한다.',
  '명성·흥행력·산업 영향력을 통솔로 환산하지 마라. 통솔은 사람과 조직을 실제로 굴린 기록에서만 나온다.',
  '"싸운 적 없음"을 "몸을 못 씀"으로 바꿔 읽지 마라. 무력 잔질 구간은 실제 장애·질환 기록이 있을 때만이다.',
  '유명하다는 이유로 지력을 올리지 마라. 26.08.08 실측에서 배우·대중가수의 지력이 12~18점 부풀려져 있었다.',
]
