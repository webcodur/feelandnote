...
티어는 실존 여부를 가르고 신화적 속성의 양을 재지 않는다.

- 동시대 기록·고고학·학계 정설로 실존 코어가 확인되면 실존 트랙이다. 예수는 세례와 처형의 역사적 코어로 실존 트랙이며, 동정녀 탄생·기적·부활 같은 신화적 속성이 있어도 밀려나지 않는다. 직군은 종교 지도자이므로 `leader`다.
- 건국신화 등 전승 자체가 본체이고 실존 코어가 없으면 fiction이다. 테세우스·단군왕검·주몽이 여기에 해당하며, 건국 기록(영웅전·삼국사기·삼국유사)은 원전·등장 작품 관계로 연결한다.
- 실존 논란 인물(역사적 실체설과 전승설이 맞서는 경우)은 학계 정설을 따르고, 정설이 없으면 사용자가 정한다.
