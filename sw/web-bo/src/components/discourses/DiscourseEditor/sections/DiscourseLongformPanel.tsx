 ...
 * 롱폼 배치 — 편 경계·장 표지·기준점.
 *
 * 담화는 발언 순서가 곧 영상 순서라 블록을 재배치할 일이 없다. 여기서 정하는 것은 두 가지다.
...
import { ChevronUp, ChevronDown, Trash2, Plus } from '@feelandnote/shared/bo/icons'
            ...
            <div key={i} className="flex items-center gap-2 rounded-md border border-border bg-bg-card p-2">
              ...
              {'era' in it && (
                <span className="flex min-w-0 flex-1 items-center gap-1.5">
                  <span className="shrink-0 rounded bg-warning/15 px-1.5 py-0.5 text-[10px] font-semibold text-warning-text" title={`${at}에 문구 카드 한 장이 끼워집니다`}>장 표지</span>
                  {editLang !== 'en' && (
                    <textarea
              ...
              <span className="flex shrink-0 items-center gap-0.5">
                ...
                <button onClick={() => remove(i)} className="rounded p-1 text-danger-text hover:bg-danger/20" title="이 칸 지우기">
                  <Trash2 size={13} />
                </button>
              </span>
            </div>
