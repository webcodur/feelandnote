...
| `faction_members` | celeb_id, lv2_id, hidden | 세력 소속 (조건 축 "tag") |
| `faction_lv2` | id, name, name_en, slug, is_featured | 세력 라벨 표시 |
...
1. **`getGridGameData()` 서버 액션** (캐시 7일, CELEBS 태그)
   ...
   - faction_members: `hidden=false` → limit 5000
   - faction_lv2: `is_featured=true` → limit 100
