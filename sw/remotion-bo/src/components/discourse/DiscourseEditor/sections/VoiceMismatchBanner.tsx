...
export function VoiceMismatchBanner({ issues, script, voiceCount, onReload }: Props) {
  ...
  if (issues.length === 0) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-success-text/40 bg-success p-2.5 text-[11px] font-semibold text-success-text">
        <span>음원 {voiceCount}개가 모두 제자리에 있습니다. 발언 순서와 음성이 맞습니다.</span>
        <button onClick={onReload} className="ms-auto rounded border border-success-text/40 px-2 py-0.5 hover:bg-success/60">
          다시 확인
        </button>
      </div>
    )
  ...
  return (
    <div className="space-y-2 rounded-lg border-2 border-danger-text/50 bg-danger p-3">
      <div className="flex items-center gap-2">
        <span className="text-sm font-bold text-danger-text">
          음성이 발언과 맞지 않습니다 — {issues.length}곳
        </span>
        <button onClick={onReload} className="ms-auto rounded border border-danger-text/40 px-2 py-0.5 text-[11px] text-danger-text hover:bg-danger/60">
          다시 확인
        </button>
      </div>
