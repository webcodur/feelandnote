/*
  파일명: /components/features/library/academy/AcademyLessonView.tsx
...
import { ACADEMY_CATEGORY_IDS, ACADEMY_CONTENT_FILTERS, getLibraryData } from "@/constants/libraryMuseum";
...
function CourseTabs({
  ...
  const t = useTranslations(`library.academy.course.${categoryId}`);
      ...
      >
        {courses.map((course) => {
          ...
          return (
            <Link
              key={course.id}
              href={`/explore/works/academy/${categoryId}/${course.id}`}
              className={`
...
// #region 유틸
function getLessonSource(categoryId: string, data: ReturnType<typeof getLibraryData>) {
