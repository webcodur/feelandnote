...
>
> 🔴 **이로써 이 앱에 남은 시리즈는 서재 탐방 하나다.** 아래 본문의 「시리즈 확장」 얼개는 지금 한 명뿐인 표가 됐다 — 아래 절 "단일 시리즈가 된 뒤" 참조.

## 정체성

- **web-bo** = 서비스 운영 (셀럽 CRUD, 콘텐츠, 사용자) + **세력도 영상 제작**(26.07.25 이관)
- **remotion-bo** = 영상 제작 (시나리오, 음성, 렌더링, 편성) — **서재 탐방 1종**(26.07.26 기준)
...
가상 담화는 세력도와 같은 이유·같은 방식으로 remotion-bo에서 **전량 폐기**됐다. 새 자리는 web-bo `/discourses`(포트 3001, 사이드바 「가상 담화」)이고, 텍스트·구성의 단일 원천은 Supabase `discourse_*` 3테이블이다. 렌더 엔진 `sw/remotion`은 무수정으로 세 파일(`discourse-data.json`·`cast.json`·`turns.json`)을 계속 읽는다.

착수 근거가 세력도와 달랐다. 담화는 **음성 파이프라인이 아직 없었다** — wav 0개, 발화 시각 파일 0개, 렌더·SRT·유튜브 CLI 전무. 팩션 통합에서 가장 비쌌던 위험(이미 만든 음원 443개와 위치 기반 파일명의 결박)이 담화에는 **아직 생기지 않은 상태**였고, 음성에 손대기 전에 끝내면 그 위험 자체가 발생하지 않는다. 그래서 시리즈가 미완성인 채로 먼저 옮겼다.

- 새 화면·창구·저장 규칙: [web-bo.md](./web-bo.md) 「가상 담화」 절
- 시리즈 SSoT: [discourse.md](./remotion/discourse.md) · 통합 설계: [discourse-unification.md](./remotion/discourse-unification.md)
...
`series-registry.ts`의 `SeriesDataModel` 유니온에서 `'discourse'`를 빼자 타입 검사기가 나머지를 전부 지목했다 — 세력도 폐기 때 쓴 방법 그대로다.
...
즉 추상화가 값을 만들어 내지 못하고 읽는 사람의 품만 늘린다. 남은 선택은 둘이다.

1. **껍데기를 걷어낸다** — `[series]` 세그먼트와 등록표를 없애고 서재 탐방 전용 앱으로 되돌린다. 이 앱을 계속 쓸 거라면 이쪽.
2. **서재 탐방도 web-bo로 옮기고 이 앱을 소멸시킨다** — 세력도·담화가 간 길을 마저 간다. 그러면 영상 제작 화면이 한 앱에 모이고 remotion-bo는 사라진다.

둘 다 이번 통합의 범위 밖이라 **판단만 적어 둔다.** 어느 쪽이든 새 시리즈를 이 앱에 얹기 전에 정해야 한다 — 안 정한 채로 얹으면 멤버 2개짜리 추상화를 다시 유지하게 된다.

### 북리커맨드 통합 방침 (26.07.26 유저 확정 — 착수 전 형태 고민 필수)

**통합은 한다. 단 지금 형태 그대로 옮기지 않는다.** 유저가 정한 두 축:

1. **팩션식 완전 동기화 구조를 지향한다** — DB 단일 원천 + web-bo 편집 + 본서비스 투영(출간). 북리커맨드의 감상배경(review)·도서 목록은 이미 "DB ↔ Remotion 100% 일치" 원칙(AGENTS.md)이 있으므로, 통합하면 지금의 수동 백필(`/remo-db-review-backfill`)이 팩션 출간처럼 구조화될 수 있다.
2. **롱폼을 폐지하고 솔로–쇼츠 2형태로 점진 전환한다.** 롱폼–솔로–쇼츠 3형태 중 롱폼은 없애는 방향. **따라서 통합 스키마·이관·편집기 설계를 롱폼 전제로 잡으면 안 된다** — 형태 전환과 통합을 함께 설계해야 두 번 공사를 피한다(옮긴 뒤 형태를 바꾸면 스키마·검증·편집기를 다시 뜯는다).

착수 전에 정할 것: 롱폼 폐지의 시점·기존 롱폼 산출물(업로드된 영상·데이터)의 처리, 솔로–쇼츠 체제에서의 데이터 모델(책 단위 SSoT는 유지), 음성 파이프라인 결합(팩션보다 깊음 — 5단계 파이프라인·voiceTimings)의 이관 순서. **설계 정찰(팩션·담화와 같은 읽기/쓰기 주체 전수)부터 시작하되, 형태 결정 없이는 스키마를 확정하지 않는다.**
