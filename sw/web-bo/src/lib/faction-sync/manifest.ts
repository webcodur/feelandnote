 ...
 *
 * 같은 사진을 두 번 올리지 않기 위한 유일한 근거다. 사진은 DB 로 옮기지 않고 렌더 저장소에
 * 그대로 두므로(문서 §0), 무엇을 이미 올렸는지는 그 폴더 옆에 기록할 수밖에 없다.
 * 로컬 파일 해시가 기록과 같으면 업로드를 건너뛴다. 미리보기(dryRun)는 이 파일을 절대 쓰지 않는다.
 */
...
export function manifestPath(folder: string): string {
  return path.join(factionEpisodeDir(folder), MANIFEST_FILE)
}
...
export async function readManifest(folder: string): Promise<FactionSyncManifest> {
  try {
    const parsed: unknown = JSON.parse(await readFile(manifestPath(folder), 'utf-8'))
...
export async function writeManifest(folder: string, manifest: FactionSyncManifest): Promise<void> {
  const sorted: FactionSyncManifest = {}
  for (const key of Object.keys(manifest).sort()) sorted[key] = manifest[key]
  await writeFile(manifestPath(folder), JSON.stringify(sorted, null, 2) + '\n', 'utf-8')
}
