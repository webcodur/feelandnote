 ...
 *
 * 실행 예:
 *   pnpm exec tsx scripts/celeb/readings.ts --slugs=hegel,werner-heisenberg --plan
 *   pnpm exec tsx scripts/celeb/readings.ts --slugs=hegel,werner-heisenberg --generate
 *   pnpm exec tsx scripts/celeb/readings.ts --slugs=hegel,werner-heisenberg --apply --resume
 *   pnpm exec tsx scripts/celeb/readings.ts --all --generate --apply --resume
 */
...
인물 탐구는 안내를 다시 요약하지 않는다. 확인된 사건이나 사상 요소 가운데 둘을 연결해 선택 방식, 작동 원리, 긴장 하나를 제목과 본문으로 읽는다. 연표가 없어도 bio에 서로 연결할 수 있는 개념·저술·비판 대상이 있으면 탐구를 쓴다. "A가 아니라 B다" 식 반전으로 한쪽을 지우지 말고, A이면서 B인 두 성질이 서로 돕거나 충돌하는 지점을 제공된 사실에 붙인다. 보통 220~500자다. 탐구에서 깊게 다룰 작품·사건·개념을 골랐다면, 안내만으로 정체를 설명할 수 있는 한 안내에서는 그것을 예고하거나 요약하지 않는다.

재료는 기본 프로필뿐이다. 이름·직군·직함·국적·생몰·bio와, 재작성일 때만 기존 읽어보기 본문. 독자는 자료 조사 과정에 관심이 없다. 공식 프로필·기록·출처가 신원을 확인한다거나 동명이인을 구별한다는 조사 메모를 본문으로 쓰지 않고, 생일 자체를 탐구 주제로 삼지 않는다. 신화·서사처럼 고유명사가 많은 인물은 독자가 원전을 모른다고 보고 누가 무엇을 했는지 먼저 풀어 쓴 뒤 의미를 붙인다. 재료에 없는 연도·숫자·인용·사건·동기·감정을 만들지 않는다. 재료가 짧아도 보류하지 말고 확인된 범위만큼 짧게 쓴다. 탐구에서는 관계, 역할과 결과, 두 특징의 결합 가운데 재료가 허용하는 하나를 고른다. 칭송, 단죄, 교훈, 번역투, 위키식 업적 나열을 피한다. 인물마다 문장 첫머리와 끝맺음을 새로 잡는다.
...
재료에서 확인할 수 없는 초안의 사실·연도·숫자·인용·동기·감정은 삭제한다. 재료는 기본 프로필뿐이다. 공식 프로필·기록·출처가 신원을 확인한다거나 동명이인을 구별한다는 조사 메모와 생일 중심의 탐구는 실제 행동·작품·관계로 교체한다. 신화·서사의 고유명사는 독자가 사건을 이해할 만큼 역할과 행동을 풀어 쓴다. 연표가 없어도 bio의 개념·저술·비판 대상 가운데 둘을 연결할 수 있으면 탐구를 살린다. 초안이 비어 있거나 작성을 거절했더라도 재료를 다시 읽고 두 글을 완성한다.
...
function validateReading(reading: Reading, material: Material): string[] {
  ...
  const yearFromDate = (value: string | null) => {
  ...
  const birthYear = yearFromDate(material.profile.birth_date)
  const deathYear = yearFromDate(material.profile.death_date)
  const contradictingYears = [...new Set(combined.match(/\b(?:1\d{3}|20\d{2})\b/g) ?? [])]
    .map((value) => Number.parseInt(value, 10))
    .filter((year) => (birthYear !== null && year < birthYear) || (deathYear !== null && year > deathYear))
  if (contradictingYears.length) errors.push(`생몰과 모순되는 연도 ${contradictingYears.join(',')}`)
  return errors
}
...
async function main() {
  ...
  if (RESEARCH || DEEP_RESEARCH) {
    console.log('외부 조사 첨부는 폐기됐다. --research/--deep-research는 무시한다.')
  }
