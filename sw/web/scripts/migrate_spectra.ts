...
// Function to generate spectrum using OpenAI
async function generateSpectrumForCeleb(nickname: string, title?: string, bio?: string) {
  const prompt = `
...
`;
  ...
  try {
    ...
    console.error(`Error generating spectrum for ${nickname}:`, error);
    return null;
  }
}

async function migrateSpectra() {
  console.log("Starting Spectrum Migration Process...");
  ...
  for (let i = 0; i < celebs.length; i++) {
    ...
    const spectrumData = await generateSpectrumForCeleb(celeb.nickname, celeb.title, celeb.bio);
    ...
    if (spectrumData) {
      ...
      const { error: upsertError } = await supabase
        ...
        .upsert({
          celeb_id: celeb.id,
          persona: spectrumData, // Storage boundary: physical JSONB column remains `persona`.
