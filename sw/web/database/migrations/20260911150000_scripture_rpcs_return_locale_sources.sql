-- 시대별·선정 서재 RPC가 언어 행의 sources를 함께 돌려준다.
-- 화면이 표시용 제목 행([no-ko]/[no-en] 배지)을 공용 함수(flattenLocales)로 판정하기 위해서다.
-- 반환 컬럼이 늘어나므로 create or replace로는 바꿀 수 없어 drop 후 재생성한다.

drop function if exists public.get_chosen_scriptures(text, integer, integer);
drop function if exists public.get_scriptures_by_era(text, text, integer, integer);
