/**
 * 세력도감(Faction) 음성 파일명·경로 규칙 — 단일 원천(SSoT).
 *
 * remotion 렌더·음성 스크립트(`src/compositions/Faction/voice-names.ts` re-export)와
 * web-bo(`src/lib/faction-voice.ts` re-export)가 이 모듈을 공유한다.
 * BookRecommend voice-names의 "정렬 가능한 0패딩 명명"을 따른다.
 * 순수 문자열 함수만 둔다 — 프레임워크·fs 의존 금지.
 */
