...
> **최종 실측 체크: 26.07.30** — 카드 자산 5장과 `/rest` 등록 코드를 대조했다. 기억·시대의 초상·유랑은 구현만 보존하고 공개 진입을 주석 처리했다. 현재 허브에는 배경 자산이 연결된 게임 4종만 노출된다.
...
`memory-card.webp`는 기억 게임용으로 납품된 파일명을 바꾸거나 복제하지 않고 시대의 초상 카드와 전체화면 배경에서 재사용한다. 비공개 기억 게임 코드도 같은 경로를 참조하므로 파일은 유지한다.
...
- **세 게임 개발자 모드 노출(26.07.31)**: 유랑·기억·시대의 초상은 `dev: true`로 표시되어 로컬 개발 서버나 `?dev=1`에서만 카드가 뜬다. 카드 라벨에 `· 개발 중`이 붙고, 미공개 게임 자료는 개발자 모드일 때만 조회한다.
