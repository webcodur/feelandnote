...
const readIntroduction = cache(unstable_cache(
  async (isbn: string | null, locale: 'ko' | 'en', source: BookIntroductionSource, sourceUrl: string | null, legacyFallback: boolean) => {
    const result = await fetchBookIntroduction({ isbn, locale, source: legacyFallback ? null : source, sourceUrl })
    return pickIntroForLocale(locale, [result.description])
  },
  ['book-introduction-selected-source-v1'],
...
export async function getBookIntroduction(
  ...
  return readIntroduction(normalizeBookIsbn(isbn), language, source, sourceUrl ?? null, legacyFallback)
}
