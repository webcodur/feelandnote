/*
  파일명: /components/features/user/explore/hub/SpectrumExtremeGrid/SpectrumExtremeGrid.tsx
  기능: 극단의 기록가 그리드
  책임: spectrum 4그룹 탭 전환 + 2x2 수평 카드. 축별 고유 컬러 + 차순위 확대.
*/ // ------------------------------
...
import { GROUPS, AXIS_COLORS } from "../../spectrumAxis";
import SpectrumQuickViewModal from "../SpectrumQuickViewModal";
...
interface SpectrumExtremeGridProps {
  entries: SpectrumExtremeEntry[];
}

export default function SpectrumExtremeGrid({
  entries,
}: SpectrumExtremeGridProps) {
  const locale = useLocale();
  const [activeTab, setActiveTab] = useState(0);
  const [selectedCard, setSelectedCard] = useState<{ entry: SpectrumExtremeEntry; isOpposing: boolean; color: string } | null>(null);
  ...
  const handleCardClick = (entry: SpectrumExtremeEntry, isOpposing: boolean, color: string) =>
