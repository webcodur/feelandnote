...
/**
 * 셀럽 DB 연결 배지 — 팩션 인물 발화 항목의 배지와 **같은 규격**이다.
 * 대조 창구(/api/celebs/exists)와 판정 규칙(✓DB / ⚠없음 / 미연결 / 신화)을 팩션에서 그대로 가져왔고,
 * 지금은 이 컴포넌트 자체를 팩션 발화 항목도 그대로 재사용한다.
 *
...
/**
 * 이 배지는 인물의 `slug`·`mythical` 두 값만 본다. 그래서 담화 전용 인물 타입을 물지 않고
 * **그 두 값만 있는 최소 모양**을 받는다(팩션 인물·담화 인물 둘 다 이 모양을 만족한다).
 */
type BadgeTarget = { slug?: string; mythical?: boolean }
...
export function useCelebExists(cast: BadgeTarget[]): { existing: Set<string>; loaded: boolean } {
...
export function CelebBadge({ speaker, existing, loaded }: Props) {
  ...
  if (speaker.slug) {
    return <span title={`연결 키(${speaker.slug})가 DB에 없음 — 미등록이거나 slug 오기`} className="shrink-0 rounded bg-warning/10 px-1 text-[10px] font-bold text-warning-text">⚠ 없음</span>
  }
