...
**음성 파이프라인 단일 진입점**: 1단계(`pnpm voice`, 유료 API) 실행 후 **`/voice-sync <에피소드명>`** 스킬 한 번 호출로 2~4단계(whisper → analyze → reconcile-check 조건부 → sub 생성 → sub 검증) 자동 일괄 처리. 각 단계를 따로 호출하지 않는다.
