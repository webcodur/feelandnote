...
export function SegmentTimingPanel({
  ...
  return (
    <div className="border border-border/30 rounded p-2 bg-bg-card/30 text-[11px]">
      <div className="flex items-center justify-between mb-1">
        ...
        <button
          onClick={handleCopy}
          className="px-2 py-0.5 text-[11px] text-accent border border-accent/40 rounded hover:bg-accent/10 transition-colors"
          title="시간표를 텍스트로 복사"
      ...
      <table className="w-full text-[11px] tabular-nums">
        ...
        <tbody>
          {layout.entries.flatMap((e, i) => {
            const rows = [
              <tr key={`seg-${i}`} className="border-t border-border/20">
            ...
            if (layout.revealAfterHook && i === 0) {
              rows.push(
                <tr key="reveal" className="border-t border-border/20 bg-bg-card/30">
                  <td className="pr-2 text-text-secondary">--</td>
                  <td className="pr-2 text-accent" title="훅 다음 인트로 — chime + 인물 · 책 등장">
                    인트로
                    <span className="ml-1 text-text-dim text-[11px]">chime + 등장</span>
                  </td>
          ...
          <tr className="border-t border-border/20 bg-bg-card/40">
            <td className="pr-2 text-text-secondary">--</td>
            <td className="pr-2 text-text-primary" title="마지막 엔드 카드 (인물 · 책 정보 표시)">
              로고
              <span className="ml-1 text-text-dim text-[11px]">
            ...
            <td className="pr-2 text-right">
              <div className="flex items-center justify-end gap-1">
                ...
                />
                <span className="text-text-dim text-[11px]">s</span>
                {!isLogoAuto && (
                  <button
                    type="button"
                    onClick={() => onChangeLogoDurationSec(undefined)}
                    className="text-text-secondary hover:text-red-400 text-[11px]"
