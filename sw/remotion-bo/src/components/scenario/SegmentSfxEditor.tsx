...
export function SegmentSfxEditor({ sfx, files, basePath, onChange }: {
  ...
  return (
    <div className="grid grid-cols-[100px_1fr] gap-2 pb-1.5">
      <div />
      <div className="space-y-1 px-2 py-1 rounded bg-bg-card/40 border border-border/30">
        <div className="flex items-center gap-2 text-[10px] text-text-secondary">
          <span className="font-semibold">SFX</span>
          {files.length > 0
            ? <button type="button" onClick={add} className="text-accent hover:underline">+ 추가</button>
            : <span className="text-text-dim">soundeffect/ 폴더가 비어있다</span>}
        </div>
        {list.length > 0 && (
        ...
        {list.map((s, idx) => (
          <div key={idx} className="grid grid-cols-[1fr_60px_110px_50px_55px_55px_55px_24px] gap-1.5 items-center text-[11px]">
            <input
              ...
              className="bg-bg-main border border-border rounded px-2 py-1 focus:outline-none focus:border-accent"
            ...
            <input
              ...
              placeholder="0"
              className="bg-bg-main border border-border rounded px-1 py-1 text-center focus:outline-none focus:border-accent"
            ...
            <select
              ...
              className="bg-bg-main border border-border rounded px-1 py-1 focus:outline-none focus:border-accent"
            ...
            <input
              ...
              className="bg-bg-main border border-border rounded px-1 py-1 text-center focus:outline-none focus:border-accent disabled:opacity-40"
