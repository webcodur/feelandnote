...
/**
 * 실존·전승이 함께 다뤄지는 인물(celeb_reality='BOTH')의 채점 근거 — 실존 정보로만 매긴다.
 *
 * 단군왕검·주몽·길가메시·홍길동처럼 실존 코어에 전승층이 두껍게 얹힌 인물은 전승을 근거로
 * 쓰면 점수가 무한히 부풀려진다. 곰이 사람이 됐다는 서술이나 축지법은 그 인물의 능력이 아니다.
 * 사료가 실제로 뒷받침하는 층에서만 매기고, 그 층이 얇으면 얇은 대로 남긴다.
 */
export const BOTH_REALITY_EVIDENCE_RULE =
  'celeb_reality=BOTH 인물은 실존 인물의 정보에 근거해서만 채점한다. 동시대 기록·실록·비문·고고학처럼 '
  + '사료가 뒷받침하는 층만 근거로 쓰고, 신화·전설·후대 문학의 서술은 점수 근거에서 뺀다. '
  + '초자연적 능력(도술·변신·신의 혈통)은 어떤 축의 근거도 되지 못한다. '
  + '사료 층이 얇으면 점수를 지어내지 말고 그 얇음이 드러나게 중립 근처에 두며, reason에는 '
  + '확인된 사료 사실만 적는다. 전승에서 끌어온 서술을 reason에 쓰지 않는다.'

export const SCORING_PRINCIPLES: readonly string[] = [
  '16축 각각에 대해 그 인물의 근거를 찾아 정리하고, 정리한 근거대로 점수를 낸다.',
  CRIMINAL_HISTORY_RULE,
  BOTH_REALITY_EVIDENCE_RULE,
...
export const SCORING_PROCEDURE: readonly string[] = [
  ...
  '1-1단계 — 층 가르기(BOTH만): 수집한 사실을 사료가 뒷받침하는 층과 전승층으로 가른다. 전승층은 여기서 버리고 이후 단계에 넘기지 않는다.',
