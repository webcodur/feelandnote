...
`content_id`·`isbn`이 없거나 검색 순번 `idx`만 적힌 옛 선택 자료는 거부한다. 픽션 원전은
...
기존 원전 감사에서 현재 판본이 축약·분권이거나 판매 상태가 나쁘면 그 상품을 억지로
...
판본·상품 표는 DB 트리거가 관련 작품과 원전 캐시를 자동으로 비운다. 트리거 밖의 자료를
