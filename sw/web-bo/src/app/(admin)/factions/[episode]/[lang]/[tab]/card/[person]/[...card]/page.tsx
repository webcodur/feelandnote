...
export default async function FactionPersonCardDeepLinkPage({ params }: { params: Promise<Params> }) {
  ...
  if (!FACTION_EDIT_LANGS.has(lang) || !FACTION_EDIT_TABS.has(tab)) {
    redirect(`/factions/${folderToParam(name)}/ko/info/card/${encodeURIComponent(personName)}/${tail}`)
  }
  if (tab !== 'info') {
    redirect(`/factions/${folderToParam(name)}/${lang}/info/card/${encodeURIComponent(personName)}/${tail}`)
  }
