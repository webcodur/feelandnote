...
> **최종 실측 체크: 26.07.29** — 열린 0 / 조사 완료 -1 규격과 Light 회수 경로 반영
...
- 조사 전·선별 전·보류 상태는 모두 `0`이다. 없다는 뜻이 아니라 **열린 상태**다.
- 첫 콘텐츠가 등록되는 순간부터 실제 `user_contents` 개수(`1 이상`)를 표시한다.
- 정식 조사 종료 조건을 충족했는데도 유효한 콘텐츠가 없을 때만
  `profiles.content_research_status='confirmed_empty'`로 바꾸며 표시값은 `-1`이다.
- 빠른 선별에서 자료가 적어 보이거나 검색 1회에 실패한 것은 `-1`의 근거가 아니다.

기존 Light는 과거 작업을 버리고 처음부터 조사하지 않는다.

| 기존 상태 | 작업 |
|---|---|
| 콘텐츠 보유 | 등록분 감사 → full 승격 |
| 감상여정에 괄호형 작품명 있음 | 작품 후보와 출처만 표적 검증 |
| 감상여정은 있으나 괄호형 작품명 없음 | 비정형 작품명 추출 → 표적 검증 |
| 활성인데 감상여정 없음 | 처음부터 조사 |
| 비활성·감상여정 없음 | 영향력·자료 존재 가능성만 선별해 `queued` 또는 `deferred` |
...
검색 종료와 `confirmed_empty` 확정은 같은 말이 아니다. `-1` 확정에는 다음
중 하나가 추가로 필요하다.

- 신규·무단서 인물: 위 단계별 검색과 시대·언어별 보충 검색을 완료했고 유효 작품 0건
- 기존 감상여정 보유 Light: 감상여정에서 추출한 작품 후보를 전부 검증했고,
  표기 변형·원어 제목·동명이인 오매칭까지 확인했으나 유효 작품 0건

비활성 무단서 인물의 10초 선별은 어느 조건에도 해당하지 않는다.
