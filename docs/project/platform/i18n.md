...
> **최종 실측 체크: 26.08.10** — 회원·셀럽 물리 분리 뒤 `celebs`·`celeb_contents` 번역 열과 사용자 웹 정적 문구를 다시 대조했다.
...
| 남은 작업 | 개발 중인 게임 UI는 26.07.27 사용자 지시에 따라 이번 배포에서 제외 |
...
| 테이블 | 추가 컬럼 |
|--------|----------|
| `celebs` | `title_en`, `bio_en`, `cultural_journey_en`, `virtual_monologue_en`. `consumption_philosophy_en`은 레거시 호환 열이며 `quotes_en` 컬럼은 없음 |
...
| 항목 | 규모 | 완료 |
|------|------|------|
| celebs (title, bio) + 명언 | 867명 × 3 | ✅ (명언 정본은 이후 `celeb_dialogues.lines_en.quote`로 이관) |
| celebs (nickname_en 보충) | 37명 | ✅ |
| celebs (현재 `cultural_journey_en`) | 867명 | ✅ |
...
- CelebForm: nickname_en, title_en, bio_en, quotes_en, cultural_journey_en
  - `quotes`/`quotes_en`은 폼 입력 필드명일 뿐 `celebs` 컬럼이 아니다. `actions/admin/celebs.ts`가 이 값을 `celeb_dialogues.lines.quote`·`lines_en.quote`에 기록한다(SSoT).
...
| `sw/web/src/proxy.ts` | 수정 | next-intl middleware + 인증 세션 쿠키 통합, `api/` 제외 |
...
- `proxy.ts`에서 next-intl middleware와 `updateSession` 인증 쿠키 통합
...
→ **완료**(26.03). naver_book 2,364건 전량 verified, 한글/CJK 잔존 0건.
...
- [x] `proxy.ts` (next-intl middleware + 인증 세션 쿠키 통합)
...
- [x] Header.tsx: `useTranslations()`, nav 라벨
...
- 스킵: (auth)/layout (robots only), lab/* (영어 전용)
...
> 26.07.27: 개발 중인 게임은 사용자 지시에 따라 커밋·배포 대상에서 제외했다. 공용 메시지 키가 준비돼 있어도 게임 실행 코드 완료로 간주하지 않는다.
...
- `sw/web-bo/scripts/celeb/i18n-backfill.mjs`: 관리형 인물 데이터의 KO→EN 누락 보강
- `sw/web-bo/scripts/celeb/dialogue-repair.mjs`: KO/EN 대사 JSON 키 구조 교정
- `sw/web-bo/scripts/celeb/readings-translate.ts`: 읽어보기 3필드의 해시 가드·재개 가능한 2단계 번역

#### 26.08.04 읽어보기 영문 검증 스냅샷

- `celeb_explanations` 2,670행의 `plain_text_en`·`interpretive_title_en`·`interpretive_text_en` 전량 완료
- 공개 RLS 1,732행에서 세 영문 필드 각각 1,732/1,732, 누락 0
- 전수 품질 검사: 한중일 문자 잔존·문단 불일치·원문 숫자 누락 0
- 정적 감사: KO/EN 각 3,611개 메시지 키, 정적 번역 호출 2,398건, 오류·경고 0
- `james-hetfield`, `peter-thiel` KO/EN × 1440px/390px 8화면 오류 0

#### 26.08.27 전수 한영 짝 점검 스냅샷

- `audit-celeb-data --all`: 오류 0·경고 0. 이 회차 채움 — 관계 노트 note_en 766, 소비 철학(문화 여정 alias 포함) 93, 세력 설명 22, 외부 관계명 58(Wikidata 27 + 번역 31), 대사 키 구조 불일치 2(kimbal-musk·dj-seo의 lines 내 `_en` 여분 키 제거). 후속 사람 검수에서 관계 노트 49건의 주어 누락·직역투, 소비 철학 2건, 세력 설명 5행, 외부 관계명 1건을 교정했다.
- 감사기 판정 정비: `'미작성'` 자리표시자는 한국어 데이터 부재로 간주하고, fiction 티어의 KO 전용 대사는 번역 누락으로 세지 않는다(celeb-i18n.md 규칙 준수). celeb_explanations에 실제 누락 없음.
- `audit-static`: 오류 0, 경고 7(maintenance 독립 페이지·게임 UI — 대상 밖). ContentIntroModal·IntroBadge·RecentContentsSection·ReviewCard·서가 베스트 칩 라벨·SpectrumSection·AxisLibraryPanel·FactionShowcase의 인라인 문구·로케일 분기를 메시지 번들로 이관(tsc·eslint 통과).
- 관계 노트 백필을 위해 `i18n-backfill.mjs`에 relation_note 도메인을 추가하고, 명시적 주어가 있는 완전한 영문 문장을 요구했다.
- 정적 감사: KO/EN 각 3,793개 메시지 키, 정적 번역 호출 2,596건.
- `jamukha`·`kimbal-musk` KO/EN × 1440px/390px 8화면은 route·locale·SEO·앵커·가로 넘침 오류 0. 경고 14건은 테스트 환경의 Google 광고 DNS 실패 8건과 전체 주소를 다른 수단으로 확인할 수 있는 긴 출처 URL 줄임 6건이다.

#### 26.08.04 게시판 locale 적용 스냅샷

- 자유게시판·피드백·공용 댓글에 `locale` 필드와 `ko|en` 제약을 적용했다. 기존 자유글 3건·피드백 1건은 `ko`, 영문 UGC는 0건으로 시작한다.
- 공지 5건의 `title_en`·`content_en`을 백필하고 두 영문 컬럼을 필수값으로 전환했다. 관리자 공지 폼도 한영 동시 입력을 요구한다.
- 목록·상세·작성·댓글·홈 자유게시판을 요청 locale로 분리하고 게시판 날짜도 KO/EN 형식으로 나눴다.
- 정적 감사: KO/EN 각 3,633개 메시지 키, 정적 번역 호출 2,427건, 오류·경고 0.
- 로컬 HTTP 실측: 공지·자유게시판·피드백의 KO/EN 6개 URL 모두 200, `<html lang>` 일치. 영문 공지는 영문 제목을 표시하고 영문 자유게시판·피드백에는 기존 한국어 UGC가 노출되지 않는다.
...
위 8,826건은 26.03.02 당시 스냅샷이지, 이후 생성 행을 자동으로 보장하는 제약이 아니다.
음악 후보를 Apple 최종 콘텐츠로 확정하며 생긴 `celeb_contents` 255행에 한국어
`review`만 들어간 결손을 발견해 `review_en` 255/255를 추가 백필했다. 이어서 테이블 전량을
페이지네이션하자 별도 신규 행 127건(BOOK 58·VIDEO 65·MUSIC 4)이 더 드러나 모두 영문화했다.
이 회차의 증분은 총 382건이며, 26.08.10 live 최종값은 `review`가 있는 12,915행 중
`review_en` 보유 12,915·공백 0이다. 새로 쓴 382행은 한국어 원문 복사·한글 잔존·대체문자·
독립 `Source:` 라벨·첫 문장 인물명 누락도 모두 0이다.

추가 MUSIC 4행은 감상문만 번역하지 않고 Apple 원곡을 확정해
`previewUrl`·Apple 링크·KO/EN `content_locales`까지 함께 완결했다. 이 절의 12,915 수치는
감상문 locale(`review_en`) 스냅샷이며, 콘텐츠 제목·저자 메타의 `content_locales` 커버리지와는
별도다.

신규 음악 등록 스크립트는 한국어 `evidence`에 `--review-en`이 없으면 iTunes 조회나 DB 쓰기
전에 중단한다. 번역 완료 수는 누적 상수가 아니므로, 대량 적재 뒤에는
`.agents/skills/audit-web-i18n/scripts/audit-celeb-data.mjs`로 증분 결손을 다시 확인한다.
...
| celeb_contents 8,825건 번역 비용 | 우선순위 분류, 단계적 처리 |
