...
export function SegmentInsertBar({ atIdx, onInsert }: {
  ...
  return (
    <div className="group/ins flex items-center justify-center gap-2 py-1 px-2 my-1 rounded bg-bg-card/30 hover:bg-accent/5 transition-colors">
      <span className="text-[11px] text-text-secondary/40 group-hover/ins:text-text-secondary/70 transition-colors select-none">여기에 추가</span>
      <button
        type="button" onClick={() => onInsert(atIdx, 'quote')}
        title="이 위치에 인용 구간 삽입"
        className="px-2 py-0.5 text-[11px] text-accent/80 border border-accent/40 rounded hover:bg-accent/10 hover:border-accent hover:text-accent cursor-pointer transition-colors"
      >+ 인용</button>
      <button
