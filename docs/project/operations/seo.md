...
일일 직접 신청과 현재 도달점은 [Google 일일 색인 신청](../../continuous/google-indexing.md)이 쥔다. URL별 검사·접수·할당량 제한 결과는 [신청 데이터](../../../data/seo-index-inspection-20260909.json)에 보존한다. 접수는 재크롤·색인 완료가 아니다. [Google 재크롤 요청 안내](https://developers.google.com/search/docs/crawling-indexing/ask-google-to-recrawl)

남은 검증은 최신 배포 이후 한영 탐색·대표 인물의 수집 본문 확인, 브랜드 질의별 노출 페이지 비교,
미색인 인물 표본의 미수집·수집 후 제외 구분이다. 탐색의 직접 링크·HTML 경량화·정렬 개편은 구현한 개선이며 검색 노출 해결의 증거가 아니다.
...
서버 조회·캐시 수정 `8dd8f2b6`는 09-09 10:05 KST 운영에 반영하고 Cloudflare HTML 캐시를 비웠다. 공개 URL 7곳의 새 배포 ID·HTTP 200·canonical·index/follow 검증은 서버 배포 검증이며 SEO 해결 판정이 아니다. 운영 결과는 [Oracle 사용자 웹 운영](../platform/external-services.md#oracle-사용자-웹-운영)이 쥔다.
...
YouTube 채널로 서재탐방·세력도감을 발행한다. 외부 신뢰도를 백링크로 쌓으려 하지 말고 영상과 인물
...
1. 7월 15일 이전까지 Google은 셀럽 서가가 서버 HTML에 없는 버전, 콘텐츠 링크가 robots에 막힌 버전, 일부 허브가 canonical=홈을 신고하는 버전을 장기간 보았다. 교정 후에도 홈페이지 마지막 크롤은 그보다 이르다. 당시 검색 접근 결함은 이 문서의 「검색 노출 약 2% 규모」 절에, 광고 심사 판단은 [AdSense 운영](adsense.md)에 둔다.
...
- Search Console `직접 조치`와 `보안 문제`는 각각 `감지된 문제 없음`, `삭제 > 임시 삭제 항목`은 `지난 6개월 동안 제출된 요청 없음`이다. 그 시점에 보고된 수동 조치·보안 문제·임시 삭제 요청은 없었다.
...
08-10 브랜드 교정 이후에도 회복이 없어 다음 네 항목을 조사했다. 노출 감소·사이트맵 비중만으로 Google 내부의 제외 사유를 확정할 수는 없다.

### 1. 4월 장애 보고와 검색 노출 감소
...
검색 노출이 있던 사이트가 4월부터 감소한 것은 확인된다. 장애 시점과 겹치지만, 당시 URL별 색인 이탈 시점과 원인까지 이 집계로 확인할 수는 없다.
...
보고한 디시인사이드 글 1건뿐이었다. 당시 표본의 통로는 확인했지만 이 상태만으로 크롤 지연의 원인을 특정할 수 없다. 이후 변경·장애가 있으면 최신 응답과 Google의 크롤 기록을 다시 대조한다.

### 3. 작품 페이지가 사이트맵 URL의 79%를 차지했다
...
이 비중은 제출 URL 수이며 실제 크롤 요청·예산 점유율은 아니다. UUID 주소 자체도 색인 제외의 근거가 아니다. 당시 확인한 본문 문제는 다음과 같다.
...
4. 한 줄 정의 필드·스니펫 개선([`celeb-01-02-profile-intro.md`](../celeb/celeb-01-02-profile-intro.md))은
   위 셋이 끝난 뒤에 착수한다. 구글이 방문하지 않는 상태에서 문구를 다듬어도 노출 자리가 없다.

작품 페이지는 이때 삭제·noindex하지 않고 사이트맵 제출 대상에서만 뺐다. **이 판단은 2026-08-25에 뒤집혔다** — 아래 「네이버 실측과 작품 상세 noindex 전환」절을 따른다.
...
3. 하루 노출이 2월~3월 수준(월 수백)으로 돌아오는지 7·14·28일에 비교한다. 회복되지 않으면 크롤·색인 상태와 검색 노출을 다시 대조한다. 외부 링크만 남은 변수라고 단정하지 않는다.

## 네이버 실측과 작품 상세 noindex 전환 (2026-08-25)

사이트맵 제외(08-14)만으로는 이미 색인된 작품 URL이 빠지지 않는다. 네이버 웹 탭을 직접 질의해
작품 페이지가 유입을 만들지 못하면서 인물 페이지 자리를 가져간다는 것을 확인하고 noindex로 바꿨다.

### 실측

| 질의 유형 | 표본 | 결과 |
|---|---:|---|
| 작품명 단독(`앵무새 죽이기`, `공정하다는 착각`, `몰몬경` 등) | 5 | 자사 노출 **0** |
| 작품명 + 의도(`… 감상`, `… 서평`) | 3 | 자사 노출 **0** |
| `필앤노트 {인물명}` | 16 | 본인 인물 페이지 **3**, 그 자리를 채운 작품 페이지 **23** |
| `{인물명} 추천 책` | 3 | 색인된 인물은 정확히 적중, 미색인 인물은 0 |

- 작품 상세는 작품명 질의에서 한 건도 잡히지 않는다. 서점·출판사·나무위키에 같은 소개문이 있고
  주소가 UUID라 어휘 신호가 없다는 08-14 판정이 네이버에서도 그대로 확인됐다.
- 반대로 브랜드 질의에서는 작품 페이지가 인물 페이지를 밀어낸다. 즉 신규 유입은 못 만들면서
  이미 서비스를 아는 사람이 치는 검색의 자리만 잠식한다.
- 인물 페이지 미색인은 생성 시기와 무관하다(1월 생성 빌 게이츠·일론 머스크도 미노출, 7월 생성
  인물은 노출). 네이버가 3,059명 중 극히 일부만 들고 있다.
- `/celeb/chey-tae-won`은 페이지 title 원문으로 검색해도 0건이다. 200·`index, follow`·canonical
  일치·사이트맵 등재 상태이므로 페이지 결함이 아니라 미색인이다.

### 조치

1. **작품 상세 전량 noindex** — `sw/web/src/app/[locale]/(main)/content/[contentId]/page.tsx`.
   종전에는 감상문이 하나라도 있으면 색인 허용이었는데, 그 감상 피드가 회원 감상과 셀럽 감상을
   합치므로 셀럽 감상 한 줄이 붙은 **10,215개**(한·영 20,430 URL)가 색인 대상이었다. 이제 조건 없이
   `index: false, follow: true`다. 페이지는 그대로 열려 있고 내부 링크로 닿는다.
2. **인물 URL 전량 재통지** — 「IndexNow」절 참조.

noindex는 검색엔진이 그 URL을 다시 크롤해야 반영된다. 색인에서 빠지기까지 수 주가 걸리며 통지로
앞당길 수 없다. 반영 여부는 `필앤노트 {인물명}` 질의에서 작품 페이지가 줄고 인물 페이지가
올라오는지로 확인한다.

## 외부 통로와 접힌 링크 (2026-09-03~04)

08-25까지의 조치는 사이트 안쪽에 집중했다. 외부 유입을 늘리기 위해 아래 통로도 열었다. 외부 링크 유무만으로 크롤 예산이나 색인 여부를 단정하지 않는다.

### 접힌 링크는 없는 링크다

`/library/curated`는 목록 96개를 아코디언과 모달 뒤에 두어 HTML 링크가 **2개**였다. 카테고리 탭도
`onClick` 상태 전환이라 선택된 탭의 내용만 실렸다. 사람 눈에는 96개가 다 보이는 화면이지만 크롤러가
받는 HTML에는 2개뿐이다. 아코디언을 항상 그려지는 카드로 펴고 탭을 `<Link href>` 로 바꿔 **링크 2 →
96, 탭 8**로 늘렸다. 규칙은 「내부 링크 통로」가 쥔다.

### 크롤러가 받는 500은 색인 이탈이다

`/library/popular` 가 `member_contents` 를 통째로 세다 문 시간(57014)을 넘겨 화면째 죽었다. 그
화면이 쓰는 것은 고른 작품들의 감상 인원뿐인데 전체를 집계하고 있었다. 대상 `contentIds` 만 넘기고
집계 RPC를 500건씩 나눠 부르게 고쳤다(`actions/library/profession.ts`, `actions/library/helpers.ts`).
DB 스키마는 건드리지 않았다.

### 네이버를 외부 통로로 세운다

한국에서 「OO 추천 책」은 네이버에서 검색되고, 네이버는 외부 사이트보다 자기 블로그를 위에 올린다.
2020년부터 쌓인 계정(글 217·방문 32만)에 인물 안내글 59편을 예약해 사이트 인물 페이지로 링크를
내보낸다. 운영 규칙·양식·자동화는 [`blog-naver-book.md`](../../continuous/blog-naver-book.md)가 전부 쥔다.

### 색인이 붙는 도메인을 빌린다 (2026-09-05)

자사 검색 노출이 적은 상황에서 외부 검색 유입을 늘리기 위해 티스토리에
「필앤노트 시네마」를 열었다. 영화 감상 3,208건은 네이버 책 블로그가 다룰 수 없는 덩어리이고,
티스토리는 색인이 빨리 붙는다. 목적은 링크 가중치가 아니라 **검색 결과에서 자리를 차지해 사람을
사이트로 보내는 것**이다. 운영 규칙은 [`blog-tistory-cinema.md`](../../continuous/blog-tistory-cinema.md)가 쥔다.

소속을 밝히고 독자에게 읽을 가치가 있는 글을 발행한다. 소속 공개 자체가 링크 스팸이나 저가치 대량 콘텐츠에 대한 면제 조건은 아니다. [Google 스팸 정책](https://developers.google.com/search/docs/essentials/spam-policies)

### 수동 조치·보안 문제 확인

09-05와 09-09 UI에서 직접 조치·보안 문제 모두 「감지된 문제 없음」을 확인했다. 이는 수동 조치가 없다는 근거이며 자동 시스템의 평가까지 배제하거나 「권위가 원인」이라고 확정하는 근거는 아니다. Google은 자동 시스템과 수동 검토를 모두 사용한다. [Google 스팸 정책](https://developers.google.com/search/docs/essentials/spam-policies)
...
외부 링크뿐이고 그것을 네이버 블로그가 친다([`blog-naver-book.md`](../../continuous/blog-naver-book.md)).
...
## 색인된 페이지도 순위에서 빠진다 (2026-09-15)

`/celeb/otto-von-bismarck`은 09-13 크롤·색인됨·canonical 일치이고 제목이 「오토 폰 비스마르크의 책 감상 기록 | 필앤노트」인데,
「필앤노트 비스마르크」 검색에 나오지 않고 최근 3개월 노출이 0이다. 09-15 UI에서도 직접 조치·보안 문제는 「감지된 문제 없음」이었고,
Search Status Dashboard에 9월 진행 중인 공식 업데이트도 없다. 미색인만의 문제가 아니라 색인된 페이지도 사이트 단위 자동 평가로
순위가 눌린 상태로 본다. 원인은 미확정이다. 같은 날 최근 24시간에는 인물 페이지 15개가 평균 41위로 노출돼 전면 배제는 아니었다.

**구글 전용 noindex로 인물 페이지를 거르지 않는다(09-15 검토 후 기각).** 조회 상위·구글 노출 인물만 남기는 허용 명단과
감상 기록 0건 조건을 모두 검토했다.
- 구글이 품질을 판단하는 재료는 실제 수집한 페이지다. 인물 페이지 대부분은 방문 전이라 막아도 평가가 바뀔 근거가 없다.
- noindex는 권장이 아니라 차단이다. 오래 두면 재방문이 줄어 신뢰가 오른 뒤 구글이 넓힐 자리를 스스로 닫는다.
- 감상 기록 수는 인물 페이지의 가치 기준이 아니다. 신화·전설 인물은 인물 분석이 본체다.

네이버·티스토리 블로그는 감상 배경 원문을 옮겨 실어 사이트 페이지와 20자 조각 기준 7~22%가 겹친다(09-15 표본 2편).
구글이 방문하지 않는 사이트 페이지의 자리를 빼앗는 구조는 아니므로 블로그 중단 사유로 삼지 않는다.
...
아래는 당시 검색 접근 결함과 교정 근거다. 광고의 현행 심사·게재 판단은 [AdSense 운영](adsense.md)이 쥔다.
