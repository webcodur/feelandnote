...
> 최종 갱신: 2026-07-20
> 대상: 아바타 미등록 인물 **129명** (DB 실측, CELEB 전체 1,692명)
...
⚠️ **`_refs`를 일괄 참고 자료로 단정하면 안 된다.** 일리아스 편 `_refs`는 실측 결과 전부 정면 얼굴 완성본이었다. 반대로 파일명만 보고 `_refs`를 완성 개인샷으로 오판한 적도 있다. **경로만으로 단정하지 말고 이미지를 열어 확인한다.**

## 분류표

| 그룹 | 인원 | 자산 |
|------|-----:|------|
| **A** 완성 개인샷 보유 | 38 | 우리 그림 |
...
※ 진수는 B와 D드라이브 양쪽에 있어 A-4로 별도 기재(위 합계에는 B로 계상).
...
## A-1. 올림포스 신 편 7명 · `factions/Gods-Greek/01-olympus/`

이미 등록돼 있다(2026-07-16). 단 **소스가 개인샷인지 얼굴본인지 미확인**이며 `Gods-Greek/_refs/01-olympus-new/`에도 얼굴본으로 보이는 파일이 있다. 교체 검토 시 `_refs` 쪽을 먼저 연다.

제우스 `1/zeus.png` · 헤라 `1/hera.png` · 포세이돈 `2/poseidon.png` · 아테나 `3/athena.png` · 아레스 `3/ares.png` · 아폴론 `4/apollo.png` · 아프로디테 `5/aphrodite.png`

중복·버전 없음.

## A-2. 알렉산드로스 편 10명 · `factions/Path-of-Kings/01-macedonia/1/`
