/**
 * 세력도감 사진 폴더 정리 — 만들기·이름변경·삭제·파일 이동·탐색기로 열기.
 *
...
export async function POST(req: Request) {
  ...
  try {
    switch (action) {
      case 'create': {
        const folder = await createFolder(FACTIONS_DIR, ep, body.folder ?? '')
        return NextResponse.json({ ok: true, folder })
      }
      case 'rename': {
        const r = await renameFolder(FACTIONS_DIR, ep, body.folder ?? '', body.name ?? '')
        return NextResponse.json({ ok: true, ...r })
      }
      case 'delete': {
        await deleteFolder(FACTIONS_DIR, ep, body.folder ?? '')
        return NextResponse.json({ ok: true })
      }
      case 'move': {
        const r = await moveImage(FACTIONS_DIR, ep, body.from ?? '', body.toFolder ?? '')
        return NextResponse.json({ ok: true, ...r })
      }
      case 'open': {
        // 에피소드 폴더(folder 를 주면 그 하위)를 OS 파일 탐색기로 연다
        const target = path.normalize(path.join(episodeDirOf(FACTIONS_DIR, ep), ...safeRelSegs(body.folder ?? '')))
