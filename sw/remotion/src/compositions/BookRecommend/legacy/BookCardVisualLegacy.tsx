...
import { SECTION_LABEL_H } from '../sections/BookCardVisual/PaginatedSection'
import { BookCardHero } from '../sections/BookCardVisual/BookCardHero'
import { CinematicPanel } from './CinematicPanelLegacy'
import { FONT } from '../fonts'
import {
  CONTEXT_QUOTE_GAP, QUOTE_CONTEXTAFTER_GAP, f, FPS,
} from '../timing'
...
type Props = {
  ...
  /** 토막별 시작 프레임 (각 구간 시작 기준) — 다토막 음성 타이밍 이어붙이기에 사용 */
...
export const BookCardVisual: React.FC<Props> = ({
  ...
  // --- 타이밍 ---
  // 다토막 책: voice-sync가 summary/context를 여러 토막(D01b, D01b2…)으로 나눠 저장하고
  // 각 토막은 자기 음원 기준(0초부터) 타이밍을 갖는다. 본문은 전체 토막을 한 흐름으로
  // 보여주므로, 각 토막 타이밍을 토막 시작 프레임만큼 초 단위로 밀어 이어붙여 전체 구간
  // 타이밍을 복원한다. 단일 토막 책은 오프셋 0이라 기존과 동일하게 동작한다.
  // 토막 중 하나라도 타이밍이 없으면 전체를 버리고(undefined) 선형 진행률로 폴백한다.
  const mergePartTimings = (
    keyFn: (i: number, part: number) => string,
    parts: string[],
    partStartsF: number[],
  ): VoiceTimingSegment[] | undefined => {
    const merged: VoiceTimingSegment[] = []
    for (let p = 0; p < parts.length; p++) {
      const segs = timings?.[vnTimingKey(keyFn(index, p))]
      if (!segs || segs.length === 0) return undefined
      const offSec = (partStartsF[p] ?? 0) / FPS
      for (const s of segs) {
        merged.push({
          ...s,
          start: (s.start ?? 0) + offSec,
          end: (s.end ?? 0) + offSec,
          subTimings: s.subTimings?.map(t => t + offSec),
        })
      }
    }
    return merged.length > 0 ? merged : undefined
  }
  const summaryParts = bookFieldParts(book.summary, book.summaryParts)
    ...
    <div style={{ position: 'absolute', inset: 0, opacity: fadeOut }}>
      ...
      {bodyOp > 0 && isSquare && (
        <div style={{
          ...
          display: 'flex',
          // 가로 모드: 좌측 이미지 / 우측 텍스트 (행 순서 반전)
          flexDirection: portrait ? 'row' : 'row-reverse',
          // 가로 모드 상단 여백을 살짝 키워 상단 상태표시줄과 본문이 겹치지 않게 한다
          padding: portrait ? `${PAD_TOP}px 40px ${PAD_BOTTOM}px` : `104px ${CINEM_PAD}px ${PAD_BOTTOM}px`,
          gap: portrait ? 16 : 24,
        }}>
          ...
          {!portrait && (
            <div style={{ flexShrink: 0, height: '100%', aspectRatio: '1 / 1', opacity: posterOp, position: 'relative' }}>
              <CinematicPanel episodeName={episodeName} book={book} sLabelContext={sLabelContext} imageTransitions={imageTransitions} />
              {/* 배경연출 이미지 위 좌상단 표지 — 겹치는 구간이라 살짝 투명 처리 */}
              <div style={{
                position: 'absolute', top: 14, left: 14, height: SQ_POSTER_H * 0.78, aspectRatio: '2 / 3',
                borderRadius: 8, overflow: 'hidden', zIndex: 2, opacity: 0.86,
                boxShadow: '0 10px 32px rgba(0,0,0,0.55), 0 0 20px rgba(200,164,110,0.08)',
