      ...
      <div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex rounded-md overflow-hidden border border-border text-xs">
            <Link href={koHref}
              className={`px-2 py-0.5 font-semibold ${!isEn ? 'bg-blue-500/20 text-blue-400' : hasCounterpart ? 'text-text-dim hover:text-text-secondary' : 'text-text-dim/30 pointer-events-none'}`}>
              KO
            </Link>
            <Link href={enHref}
              className={`px-2 py-0.5 font-semibold ${isEn ? 'bg-green-500/20 text-green-400' : hasCounterpart ? 'text-text-dim hover:text-text-secondary' : 'text-text-dim/30 pointer-events-none'}`}>
              EN
            </Link>
          </div>
        </div>
