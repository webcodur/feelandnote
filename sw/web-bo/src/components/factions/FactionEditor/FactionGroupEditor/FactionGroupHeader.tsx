      ...
      <header className="relative z-10 select-none rounded-t-lg p-2.5" style={{ backgroundColor: color }}>
        <div className="flex h-48 min-w-0 w-full items-stretch gap-2 overflow-x-auto scrollbar-hide">
          <section
            ...
            className="flex h-full shrink-0 overflow-hidden rounded-md border bg-black/15 shadow-inner"
          ...
          >
            <button
              ...
              className="flex shrink-0 flex-col items-center justify-center gap-1 border-r border-black/25 bg-black/20 px-2.5 font-mono hover:bg-black/40 focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-white"
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-between gap-2 border-l border-black/20 p-2">
              {/* 상단 1행: 큼직한 로고 2개 나란히 + 2*2 조작 아이콘 & 배지 */}
              <div className="flex h-[5.5rem] min-w-0 items-center justify-between gap-2.5">
                <div className={`grid h-full grid-cols-2 gap-1.5 ${disabled ? 'opacity-40 saturate-50' : ''}`} aria-label="세력 로고">
                  <button
                    type="button"
                    onClick={event => { event.stopPropagation(); setLogoOpen(true) }}
                    {...logoDropProps}
                    className={`group relative aspect-square h-full overflow-hidden rounded-md border bg-bg-card/30 p-0.5 ${logoDragOver ? 'ring-2 ring-accent' : 'hover:bg-black/20 hover:brightness-110 hover:shadow-sm'}`}
                  ...
                  <button
                    ...
                    {...logoImgDropProps}
                    className={`group relative aspect-square h-full overflow-hidden rounded-md border bg-bg-card/30 p-0.5 ${logoImgDragOver ? 'ring-2 ring-accent' : 'hover:bg-black/20 hover:brightness-110 hover:shadow-sm'}`}
                ...
                <div className="flex shrink-0 flex-col items-end justify-between gap-1.5 self-stretch">
                  <div className="flex min-w-0 items-center gap-1">
                    {disabled ? <span className="shrink-0 rounded border border-danger/40 bg-danger/20 px-1.5 py-0.5 text-[9px] font-semibold text-danger-text">영상 제외</span> : null}
                    {group.longformOnly && !disabled ? <span className="shrink-0 rounded border border-accent/40 bg-accent/10 px-1.5 py-0.5 text-[9px] font-semibold text-accent">롱폼</span> : null}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    <button
                      type="button"
                      onClick={onMoveUp}
                      className="flex h-7.5 w-7.5 items-center justify-center rounded-md border border-border bg-bg-main/70 text-text-secondary hover:bg-bg-hover"
                      title="세력 위로 이동"
                      aria-label="세력 위로 이동"
                    >
                      <ChevronUp size={14} />
                    ...
                    <button
                      type="button"
                      onClick={() => onChange({ ...group, disabled: disabled ? undefined : true })}
                      aria-pressed={disabled}
                      className={`flex h-7.5 w-7.5 items-center justify-center rounded-md border ${disabled ? 'border-accent bg-accent/10 text-accent' : 'border-border bg-bg-main/70 text-text-secondary hover:bg-bg-hover'}`}
                    ...
                    <button
                      type="button"
                      onClick={onDelete}
                      className="flex h-7.5 w-7.5 items-center justify-center rounded-md border border-danger/50 bg-black/10 text-danger-text hover:bg-danger/20"
                      title="세력 삭제"
                      aria-label="세력 삭제"
                    >
                      <Trash2 size={14} />
