/*
  파일명: /components/features/library/sections/EraSection.tsx
  기능: 불후의 명작 섹션 (통합)
...
import type { EraLibrary, LibraryResult } from "@/actions/library";
import { getChosenLibrary, getEraContents } from "@/actions/library";
...
interface Props {
  initialEraData: EraLibrary[];
  initialChosenData: LibraryResult;
...
function ContentSection({
  ...
  const te = useTranslations("library.page.eraPage");
  const tempty = useTranslations("library.page.empty");
  return (
    <div>
      {/* 작품 라벨 */}
...
function EraInfo({ era, isAllEra = false, topCelebsAcrossAllEras = [], totalCelebCount = 0, totalContentCount = 0 }: EraInfoProps) {
  const t = useTranslations("library.page.eraPage");
...
export default function EraSection({ initialEraData, initialChosenData, topCelebsAcrossAllEras }: Props) {
  ...
  const fetchData = (tab: TabValue, category: CategoryFilter, targetPage: number) => {
    startTransition(async () => {
      if (tab === "all") {
        const result = await getChosenLibrary({
