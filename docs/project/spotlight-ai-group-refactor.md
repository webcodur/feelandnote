# 스포트라이트 「AI」 상위 그룹 개편 계획서

> **최종 실측 체크: 26.07.16** — 실 DB로 `celeb_tags` 전량(40행) 대조: `parent_id` 컬럼 부재·그룹 헤더 8행(sort 1~8, 배정 0명)·맨해튼 단독(sort 9)·`mapping-ai` 삭제·`Mistral` 개명 모두 확인. `constants/spotlightGroups.ts` 그룹 8개·자식 구성 대조. **구현 서술은 코드·DB와 일치**하며, 낡은 것은 아래 "스키마 변경 권한" 배경 서술 하나였다(교정함). UI 3곳 렌더 동작은 화면으로 확인하지 않았다.

/ explore/spotlight 태그 목록을 계층화하고, AI 회사 태그를 상위 그룹 하나로 묶는다. 겸사겸사 스포트라이트 AI 태그 인물을 faction 최신 로스터에 맞춰 보강하고 faction 쪽 결손을 메운다.

작성 2026-07-05. 상태: **구현 완료** (아래 "진행 결과" 참조).

## 진행 결과 (2026-07-05)

구현 당시 Supabase 스키마 변경 권한(MCP·관리 토큰)이 막혔다고 판단해 `parent_id` 컬럼 방식(후보 A) 대신 **코드 상수 방식**으로 구현했다. 그룹 소속은 `sw/web/src/constants/spotlightGroups.ts`가 단일원천이며, 그룹 헤더는 `celeb_tags`에 일반 태그 행으로 존재한다.

> **정정(2026-07-16)**: 위 "권한이 막혔다"는 전제는 **더 이상 사실이 아니다.** 이 시점에 DDL이 정상 동작한다(마이그레이션 `drop_unused_remotion_images_and_dialogue_backup`(20260715171752)로 테이블 2개 DROP 성공). 따라서 코드 상수 방식을 강제하던 제약은 해소됐고, `parent_id` 컬럼 이관은 **재검토 여지 있음**이다. 다만 이관 여부는 판단 사항이라 여기서 결정하지 않는다(코드 상수 방식도 정상 동작 중이며, 실익·이관 비용은 별도 검토 대상). 현재 정본은 여전히 `spotlightGroups.ts`다.

- **DB(REST)**: AI 그룹 + 7개 그룹 태그(제국과 군주·난세의 영웅·사상의 계보·혁명과 건국·예술의 시대·자수성가와 혁신·시련을 넘어) 생성. `mapping-ai` 삭제. `Mistral AI`→`Mistral`. faction 20명 스포트라이트 태그 배정. 최상위 정렬 = 그룹 8개(sort 1~8) + 맨해튼 단독(9).
- **그룹 구성**: AI(11)/제국과 군주(4)/난세의 영웅(2)/사상의 계보(2)/혁명과 건국(3)/예술의 시대(4)/자수성가와 혁신(3)/시련을 넘어(2), 맨해튼 단독.
- **코드**: `getFeaturedTags`가 그룹 헤더를 포함하고 자식에 `parentSlug` 부착. 공용 유틸 `spotlightGrouping.ts`.
- **디자인(섹션 헤더형)**: 컬렉션(`SpotlightIntroView`)은 그룹을 전체폭 구분 헤더 + 펼치면 자식 카드 그리드로 렌더(여러 그룹 동시 펼침, AI 기본 펼침). 드로어·시트도 그룹 헤더 톤 통일. 최상위엔 그룹·무소속만, 하위는 펼쳐야 노출.
...
- AI 회사 태그 11개(OpenAI·Anthropic·Google DeepMind·xAI·Meta·Mistral AI·Hugging Face·DeepSeek·Thinking Machines·SSI·AI 선구자들, `sort_order` 27~37)가 역사·문화 테마 태그와 같은 평면에 나열돼, 컬렉션 화면(`SpotlightIntroView`의 4열 그리드)이 AI 카드로 뒤덮여 산만하다.
- `AI의 지도를 그리다`(slug `mapping-ai`, sort 1)는 예전 태그로 삭제 대상이다.
- 스포트라이트 AI 태그의 배정 인물이 faction「AI-Supremacy」로스터보다 뒤처져 있다. faction엔 있는데 태그엔 없는 인물이 **20명**이다.
- faction 쪽은 Hugging Face 세력만 미완이다: 팀 3명 중 2명이 비활성(disabled), 22개 세력 중 유일하게 단체사진이 비어 있다.
...
3. 스포트라이트 목록 UI를 「그룹 카드 접기 → 펼치면 자식 태그」 2단 구조로 개편한다.
4. faction의 20명을 스포트라이트 AI 태그에 반영(DB 보강)한다.
...
- `SpotlightIntroView.tsx` (주 그리드, `:60-61`): 부모 없는 태그 + 그룹 카드를 먼저 렌더, 그룹 카드 클릭 시 자식 11개 인라인 확장.
- `SpotlightTagDrawerDesktop.tsx` (`:86-87`): 그룹 헤더 + 자식 들여쓰기.
- `SpotlightTagSheetMobile.tsx` (`:93-94`): 동일.
- `FeaturedSpotlight.tsx`: 현재 `activeTagIndex`(number) 인덱스 선택 모델. 그룹 행이 배열에 섞이므로 **평탄화 인덱스 유지**(그룹 행은 선택 대상에서 제외, 자식만 선택 가능)로 최소 변경. slug replaceState 로직은 자식 slug 유지라 그대로.

### 3-4. faction 인물 → 스포트라이트 태그 보강 (20명)
...
| 3 | 목록 UI 3곳 그룹 접기/펼침 + FeaturedSpotlight 선택 모델 | `tsc`·`eslint`, dev 화면 |
...
- **C. 그룹 카드 클릭 UX** — 제안: 인라인 확장(URL 자식 slug 유지). 그룹 전용 페이지(`/explore/spotlight/ai`)는 필요 시 후속.
...
- 목록 UI: `sw/web/src/components/features/landing/{SpotlightIntroView,SpotlightTagDrawerDesktop,SpotlightTagSheetMobile,FeaturedSpotlight}.tsx`
