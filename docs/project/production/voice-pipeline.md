...
pnpm voice:faction       팩션 합성 (faction-tts.ts)
pnpm voice:faction-align 팩션 정렬 (faction-align.ts)
pnpm faction:durations-pull  음원 길이 → DB 반영
...
- **엔진**: Gemini 무료 키 일관. `--engine elevenlabs`는 팩션에서 거부된다.
- **보이스**: 인물별 배정값(`voiceSpeaker`/`quoteSpeaker`)이 데이터에 있다 — 정책이 아니라 데이터.
- **청크**: LLM 자동 분할 대신 사람이 DB에 `quoteChunks`를 끊어 둔다. `faction-align`이 덩어리별 시각을 `data.timing.p<N>.<lang>.json`에 쓴다.
- **저장**: `public/factions/<에피>/voice/*.wav`. 파일명 규칙은 shared `faction-voice-names.ts`가 쥔다 — `F{gi+1}C{ci+1}P{pi+1}-quote.wav`, personIndex는 그룹 내 로컬 인덱스.
- **길이 계약**: `faction:durations-pull`이 실제 음원 길이를 DB `quoteDuration`에 반영 — 어긋나면 화면에서 목소리가 잘린다.
- **미리듣기**: web-bo `api/faction/voice/{gemini,gemini-v3,elevenlabs}/preview` — 합성 코어는 `web-bo/src/lib/gemini-tts.ts`·shared 엔진 상수.
...
| 팩션 음성 파일명 규칙 | `packages/shared/src/lib/faction-voice-names.ts` | `Faction/voice-names.ts`(재export)·`web-bo/src/lib/faction-voice.ts` |
| Gemini 합성+키 순회(스크립트) | `sw/remotion/scripts/voice/lib/gemini-engine.ts` | `2-synthesize/engines.ts`·`faction/engine.ts`·`book-person/tts.ts` |
...
- `web-bo/src/lib/discourse-types.ts`·`faction-types.ts` — 씬·타입 정의 미러. 음성 파이프라인 상수가 아니라 타입이라 이번 통합 대상에서 제외했다. 통합 시 shared 이관 검토 대상.
