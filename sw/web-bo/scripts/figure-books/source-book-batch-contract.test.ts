...
test('작품 정체성·본문 범위·판본 ISBN을 엄격히 파싱하고 상품 링크 입력을 받지 않는다', () => {
  const parsed = parseFigureBookManifest(publishedInput())
  ...
  assert.throws(() => parseFigureBookManifest({
...
test('reviewedDistinctContentIds는 누락·현재 후보가 아닌 ID·비검토 후보를 허용하지 않는다', () => {
  ...
  const incompletePlan = buildFigureBookPlan(incomplete.manifest, incomplete.resolved, incomplete.catalog)
  ...
  const unmatchedPlan = buildFigureBookPlan(unmatched.manifest, unmatched.resolved, unmatched.catalog)
...
test('reviewedDistinctContentIds의 text ID도 동일 작품 후보로 차단한다', () => {
  ...
  const plan = buildFigureBookPlan(fixture.manifest, fixture.resolved, {
...
test('같은 ISBN이나 같은 figureBook 작품 후보는 검토 목록으로도 waive할 수 없다', () => {
  const isbnFixture = reviewedDistinctJttw(JTTW_CANDIDATE_IDS)
  isbnFixture.catalog.contents[0] = {
    ...isbnFixture.catalog.contents[0],
    external_id: JTTW_NEW_KO_ISBN,
  }
  const isbnPlan = buildFigureBookPlan(isbnFixture.manifest, isbnFixture.resolved, isbnFixture.catalog)
...
test('범위 메타가 없는 기존 비완역 후보는 명시적인 reuseContentId 전까지 중단한다', () => {
  ...
  const ambiguous = buildFigureBookPlan(manifest, resolved, catalog)
  assert.equal(ambiguous.action, 'conflict')
  assert.match(ambiguous.conflicts.join('\n'), /no stored work identity/)

  const explicit: FigureBookManifest = { ...manifest, reuseContentId: CONTENT_ID }
  const reviewed = buildFigureBookPlan(explicit, resolved, catalog)
  assert.equal(reviewed.action, 'reuse')
  assert.deepEqual(reviewed.localeChanges.map((change) => change.kind), ['update'])
  assert.deepEqual(reviewed.contentUpdate?.metadata.figureBook, {
    workIdentity: manifest.work.identity,
...
test('explicit reuse는 기존 figureBook의 NULL만 채우고 비-NULL 충돌값은 덮지 않는다', () => {
  const raw = publishedInput() as Record<string, unknown>
  const manifest = parseFigureBookManifest({ ...raw, reuseContentId: CONTENT_ID })
  const resolved = buildResolvedSourceBookRegistration(manifest, {
    ko: edition('kakao_book', KO_ISBN),
    en: edition('openlibrary', EN_ISBN),
  })
  const catalog: BookCatalogSnapshot = {
    contents: [storedContent({
      ...
      metadata: {
        ...
        figureBook: { workIdentity: 'homer/iliad', editionKind: null, textScope: null },
      },
    })],
  ...
  assert.equal((plan.before.content?.metadata?.figureBook as Record<string, unknown>).workIdentity, 'homer/iliad')
})

test('explicit reuse upgrades legacy per-field source markers to verified source URLs', () => {
  const raw = publishedInput() as Record<string, unknown>
  const baseManifest = parseFigureBookManifest(raw)
  ...
  const automatic = buildFigureBookPlan(baseManifest, resolved, catalog)
  assert.equal(automatic.action, 'conflict')
  assert.match(automatic.conflicts.join('\n'), /ko\.sources\.thumbnail differs/)

  const explicitManifest = parseFigureBookManifest({ ...raw, reuseContentId: CONTENT_ID })
  const explicit = buildFigureBookPlan(explicitManifest, resolved, catalog)
...
test('apply SQL은 외부 검색 없이 짧은 단일 트랜잭션·service_role·stale guard·exact readback을 사용한다', () => {
  ...
  assert.match(sql, /INSERT INTO public\.figure_book_contents/)
  assert.match(sql, /INSERT INTO public\.figure_book_editions/)
...
test('text content ID를 명시한 explicit reuse SQL은 metadata identity 백필도 text 비교로 수행한다', () => {
  ...
  const plan = buildFigureBookPlan(manifest, resolved, {
  ...
  assert.ok(plan.contentUpdate?.metadata.figureBook)
...
test('대상 locale의 legacy non-object sources는 explicit reuse update에서도 원형을 보존한다', () => {
  ...
  const explicitManifest = parseFigureBookManifest({
    ...raw,
