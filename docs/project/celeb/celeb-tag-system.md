...
> **최종 실측 체크: 26.08.09** — 데이터 단일원천 구조는 아래 「26.08.03 단일화」 절이 최신이며, 장르 계층은 바로 아래 「26.08.09 장르 조정」을 함께 따른다. 그 이전 실측은 26.07.27(개편 절)·26.07.16(DB 스키마 전수, 서버 액션 4종, `components/features/landing/` 컴포넌트 7종, 타입, 라우트, 백오피스 경로).

## 26.08.09 장르 조정

- `X제국`(`x-empire`)을 최상위 그룹으로 신설하고, `스페이스X`·`테슬라`·`X (트위터)`·`뉴럴링크`·`측근과 자금`·`보링 컴퍼니`·`황제` 7개 테마를 그 아래로 옮겼다.
- `틸 유니버스`(`thiel-universe`)는 방산·데이터·플랫폼·벤처·정치를 함께 관통하므로 `특집`의 자식이 아니라 최상위 독립 태그로 분리했다. `특집`(`special-features`)은 빈 예약 분류로 남겼다.
- 26.08.09 DB 실측은 상위 그룹 14개·전체 태그 151개다.
...
### ④ 대분류 14개 · 태그 151개 (26.08.09 실측)

축이 뒤섞여 있던 묶음을 분야 기준으로 다시 세웠다. **인공지능 · 기술과 과학 · 산업 · 권력 · 사상과 신념 · 미술 · 음악 · 문학과 영화 · 스포츠 · 이면 세계 · X제국 · 틸 유니버스 · 특집 · 신화와 이야기 · 삶의 궤적.**
...
- 「X제국」은 머스크 계열 7개 테마를 묶는 전용 자리다. 「틸 유니버스」는 방산·데이터·플랫폼·벤처·정치가 한 인물의 자본과 인맥으로 연결되는 독립 축이다. 「특집」은 향후 한 사람이나 한 사건을 통째로 파는 기획을 위한 빈 예약 분류다
...
- 26.08.09 DB 실측 최상위 15개: `ai`(11) / `technology-and-science`(15) / `business-and-industry`(26) / `power-and-war`(22) / `thought-and-conviction`(4) / `visual-arts`(3) / `music`(11) / `literature-and-film`(2) / `sports-legends`(6) / `shadow-world`(8) / `x-empire`(7) / `thiel-universe`(0) / `special-features`(0) / `myth-and-fiction`(17) / `paths-of-a-life`(4)
