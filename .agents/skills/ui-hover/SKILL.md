...
| 카드 | hover 시 배경·글자색 강조 (위로 뜨는 `-translate-y`·확대 `scale`은 넣지 않는다) |
| 칩·태그 | 선택/hover 강조 |
| 리스트 행 | hover 하이라이트 |
| 탭·토글 | 활성 상태 전환 |
...
// ❌ 이동 — 위로 뜸·확대는 금지 (색 강조로 대신)
<button className="hover:-translate-y-0.5 hover:scale-105">
```

> **이동(translate/scale) 지양**: hover 시 요소가 위로 뜨거나 커지는 위치·크기 변화는 넣지 않는다. 텍스트 금색 전환처럼 **색·밝기 강조**로 반응을 표현한다.
