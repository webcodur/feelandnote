...
**핵심**: 팩션은 의미분할(chunk) 단계가 없다. 의미 덩어리는 `data.json` 의 인물 `quoteChunks`(발화 호흡 단위로 끊은 덩어리)가 담당하며, 이 스킬은 그 덩어리에 발화 시각만 입힌다.

## 사전 조건

- `public/factions/<에피소드>/data.json` 존재. 대사 인물에 `quote`(통대사) + `quoteChunks`(발화 의미 단위 덩어리) 채워짐.
  - `quoteChunks` 가 비어 있으면 통대사 1덩어리로 폴백한다(페이지 전환 없음). 점등·페이지를 원하면 `quoteChunks` 를 발화 호흡 단위로 끊어 둔다.
