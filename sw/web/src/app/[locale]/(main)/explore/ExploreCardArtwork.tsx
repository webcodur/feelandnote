/*
  파일명: /app/(main)/explore/ExploreCardArtwork.tsx
...
import { W, H, CX, GROUND } from "./artwork/shared";
import Ranking from "./artwork/Ranking";
import Spectrum from "./artwork/Spectrum";
import Myth from "./artwork/Myth";
import Faction from "./artwork/Faction";
...
const TINTS: Record<string, { color: string; glow: number }> = {
  ranking: { color: "#b8434e", glow: 0.26 },
  spectrum: { color: "#5fbfa8", glow: 0.18 },
  myth: { color: "#6f7ad8", glow: 0.26 },
  faction: { color: "#b8573a", glow: 0.2 },
};

const ARTS = { ranking: Ranking, spectrum: Spectrum, myth: Myth, faction: Faction } as const;
...
export default function ExploreCardArtwork({ variant }: { variant: string }) {
  const { color: tint, glow } = TINTS[variant] ?? TINTS.ranking;
  const id = `explore-art-${variant}`;
  const gold = `url(#${id}-gold)`;
  const metal = `url(#${id}-metal)`;
  const Art = ARTS[variant as keyof typeof ARTS] ?? Ranking;
