...
import { findEpisodeDir, isNewLayout, parseEpisodeId } from '@/features/book-recommend/lib/server-utils'
...
export async function PATCH(
  ...
  if (!found) return NextResponse.json({ error: `episode not found: ${person}` }, { status: 404 })
  const newLayout = isNewLayout(found.dir)

  const written: string[] = []
  try {
    for (const loc of ['ko', 'en'] as const) {
      const fp = path.join(found.dir, newLayout ? `meta.${loc}.json` : `${loc}.json`)
