/*
  파일명: /components/features/scriptures/history/SheetMusic.tsx
  기능: abcjs 래퍼 컴포넌트
...
export default function SheetMusic({ abc, playable = false }: SheetMusicProps) {
  ...
  // 요소별 색상 계층 (add_classes: true 기반 abcjs CSS 클래스 활용)
  const NOTE_COLOR = "rgba(255,255,255,0.88)";   // 음표·기둥·잇단줄 — 밝은 화이트
  const STAFF_COLOR = "rgba(255,255,255,0.18)";   // 오선·마디선 — 은은한 배경
  const TEXT_COLOR = "rgba(255,255,255,0.55)";     // 텍스트(제목·박자·음이름)

  // abcjs 클래스명으로 오선/마디선 요소인지 판별
  ...
  return (
    <div className="relative">
      {/* 악보 렌더 영역 — 렌더 후 JS로 SVG 색상 직접 적용 */}
      <div
        ref={containerRef}
