...
type PersonaAxisKey = keyof typeof KO_PERSONA_AXIS_LABELS;

const AXIS_TOKEN_PATTERN = new RegExp(
  `\\b(${Object.keys(KO_PERSONA_AXIS_LABELS).join("|")})\\b(은|는|이|가|을|를|과|와)?`,
  "g",
);
...
/**
 * 한국어 설명문에 남은 persona 내부 축 식별자를 사용자용 축명으로 바꾼다.
 * 원본 데이터 정비 전에도 내부 스키마 이름이 화면에 노출되지 않게 하는 방어선이다.
 */
export function localizePersonaText(
  ...
  return text.replace(
    AXIS_TOKEN_PATTERN,
    (_match, rawKey: PersonaAxisKey, particle?: string) => {
      const label = KO_PERSONA_AXIS_LABELS[rawKey];
