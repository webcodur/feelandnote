...
description: 셀럽 avatar 이미지를 등록한다. 사진이 남은 인물은 실제 얼굴을 보존하고, 사진이 없는 고대·역사 인물은 인물별 제작본을 사용한다. 얼굴 크롭·R2 업로드·DB 갱신을 포함한다. "셀럽 이미지 채워줘", "avatar 자동 등록", "얼굴 사진 채워줘", "초상화 세팅" 등으로 호출.
---
...
- **사진이 남은 인물은 실제 얼굴을 보존한다.** 본인·소속기관·권위 매체의 사진과 대조해 다른 사람 얼굴을 막는다.
- **사진이 없는 고대·역사 인물은 제작본이 정상이다.** 명명 초상·P18이 없다는 이유로 결함 처리하지 않는다. QID와 프로필이 같은 인물인지, 다른 인물용 얼굴을 재사용하지 않았는지, 제작본의 시대·복식·품질이 맞는지만 본다. 초상·도상이 있으면 제작 참고로 쓸 수 있지만 필수 통과 조건은 아니다.
- **로컬 재료 얼굴은 용도를 구분한다.** 사진이 남은 인물에게 출처 불명 얼굴을 붙이지 않는다. 사진 없는 인물도 다른 인물용 얼굴을 이름만 바꿔 재사용하지 않고, 해당 인물을 위해 만든 완성본을 쓴다.
...
- **일반 웹 사진은 "Commons에 없을 때의 차선"이 아니다.** 공식 사이트·뉴스 기사·소셜 프로필 이미지가 더 크고 선명하면 Commons에 사진이 있어도 그쪽을 쓴다. 단건 스크립트에 직접 URL을 넣어 등록한다.
  > **왜 그런지는 실측으로 확인됐다(2026-08-21).** Wikidata에서 모델 1,374명을 뽑아 P18을 대량으로 받아 봤더니, 표본 판정에서 확대 없이 쓸 수 있는 것이 **42%**였다(나머지는 얼굴이 140~570px). 개별 검색분에서는 런웨이 전신샷(얼굴 30px)·뒤통수 사진·안경 쓴 스냅이 나왔고, 후보 10명 중 2명은 Commons에 쓸 사진이 아예 없었다. **Commons는 인물을 식별하는 곳이지 얼굴을 보여주는 곳이 아니다.** 근거는 `docs/project/celeb/celeb-avatar-spec.md` §0.3.
- **제작 실행은 사전 승인을 받는다.** 사진 없는 인물의 제작본 사용은 정상 정책이지만, 유료 이미지 생성은 사용자가 명시적으로 지시한 뒤 실행한다.
...
- `--identity-evidence <URL|fiction:SSoT>`: 모든 업로드 모드 필수. 사진이 남은 실존 인물은 얼굴을 확인할 수 있는 공식·기관·본인 페이지, 사진 없는 역사 인물은 QID나 권위 있는 인물 페이지, fiction은 원전·팩션 SSoT 경로를 쓴다. 역사 인물의 URL은 얼굴 증명이 아니라 프로필 식별 근거다.
...
사진 없는 고대·역사 인물의 `no_p18`은 자동 수집 경로의 실패일 뿐, 제작본을 쓰면 안 된다는 판정이 아니다.
...
**A. 신규 등록 직후 일괄**
```
npx tsx sw/web-bo/scripts/avatar/batch.ts --scan-db --offset 0 --limit 50
```
...
```
npx tsx sw/web-bo/scripts/avatar/upload.ts \
...
다른 사람 얼굴이나 다른 인물용 제작본이 붙어 아바타를 내린 프로필은 업로드 스크립트의 `PROVENANCE_QUARANTINED_SLUGS`에 격리한다. 파일명을 바꾸거나 임시 폴더로 복사해도 등록할 수 없다. 사진이 남은 인물은 실제 얼굴 일치, 사진 없는 인물은 QID와 인물별 제작 여부를 확인한 뒤 검역 목록을 명시적으로 해제한다.
...
- 동명이인·작품 표지·캐릭터·기념물 사진을 등록하지 않는다. 사진이 남은 인물은 본인 얼굴을 쓰고, 사진이 없는 인물은 그 인물을 위해 만든 제작본을 쓴다.
