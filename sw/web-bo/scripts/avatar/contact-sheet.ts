 ...
 *
 * 사용법 (sw/web-bo 에서):
 *   npx tsx scripts/avatar/contact-sheet.ts --out <출력폴더> [옵션]
 *
...
async function main() {
  ...
  if (!outDir) {
    console.error('사용법: npx tsx scripts/avatar/contact-sheet.ts --out <출력폴더> [--limit 36] [--offset 0] [--slugs a,b] [--seed x]')
    process.exit(1)
  }
