...
AI·기술 분야의 주요 인물을 **진영(세력)별로 묶어 보여주는 세로 영상** 시리즈. 인물 대사 음원과 화면 텍스트를 고정 길이 컷의 시간순으로 구성한다. 첫 에피소드는 `llm` — "AI를 만드는 사람들".

> **26.09.11 — 새 영상 제작을 중단했다.** 세력은 웹 세력도감에만 게시한다. 영상 없는 세력은 web-bo `/factions/themes/[tagId]` 테마 편집 화면에서 만든다. 이 문서의 영상 규격은 기존 편 유지용으로만 남는다. 기존 영상 자산·코드를 걷을지는 [`docs/todo/faction-video-stop.md`](../../../todo/faction-video-stop.md)에서 판단한다.

## 문서 구성

| 문서 | 책임 |
|---|---|
| 이 문서 | 시리즈 기획·엔진·데이터·완성 정의 |
| [`rules.md`](rules.md) | 제작 규칙과 반복 작업의 함정 |
| [`unification.md`](unification.md) | 본서비스·web-bo·Remotion 데이터 단일화 |
| [`video-clips.md`](video-clips.md) | 정지 화면을 영상 컷으로 확장하는 검토 |
...
통합 설계 SSoT는 [`unification.md`](unification.md)다(§4 경계 · §6 내보내기 · §7 음성 길이 · §8 web-bo 이식 · §9 폐기).
...
1. **기획·명단** — *어디서: 조사·웹 검색, 셀럽 파이프라인(web-bo `/celebs`).*
   진영별로 인물을 추리고 현직 여부를 웹으로 교차 확인한다(편성 원칙 1). 미등록 인물은 먼저 등록한다. 동명이인 주의. 등록 기준(상세 `docs/project/celeb/celeb-00-01-pipeline.md`):
   - 모든 인물은 `light`로 등록한다(콘텐츠 확보 시 `full` 승격). 관계 때문에 나오는 조연도 같은 기준을 쓴다.
   - **신화·전설·허구 속 존재**(신·영웅·괴물·서사 속 집단) → `celeb_reality='FICTION'`(basic 최소). 실존 코어에 전승층이 얹힌 인물은 `BOTH`다. 현행 셀럽 등록 경로로 기본 정보만 만든다. 얼굴은 없어도 된다. 인물 데이터에는 `mythical: true`를 함께 박는다.

   등록 여부는 편집기 인물 행 배지로 확인한다: **✓ DB**(실존 등록·연결) / **⚠ 없음**(키는 있는데 DB 부재) / **미연결**(키 없음) / **신화**(`celeb_reality`가 `REAL`이 아님, `mythical` 플래그 — DB 연결 시 초록).
...
4. **대사·수식어 작성** — *어디서: 편집기 정비 탭 인물 행. 쓰는 곳은 DB다.*
   대사(`quote`)·자막 덩어리(`quoteChunks`)·수식어(`epithet`)를 채운다. 신화·허구 인물 대사는 대표 원전과 기존 조사 자료를 근거로 쓴다. `quoteOrigin`은 출처 필수칸이 아니라 자유 메모칸이며 비어 있어도 정상이다.
...
9. **유튜브 업로드 · 세력도감 출간** — *어디서: 편집기 헤더의 「유튜브」·「출간」 패널.*
   유튜브는 아래 「유튜브 업로드」 절 참조. 출간은 세력도감(DB `celeb_tags`·`celeb_tag_assignments` + R2 이미지)에 반영한다 — 진단으로 막힌 항목을 확인하고, 미리보기(dry-run)로 변경 예정을 본 뒤 출간한다. 소개문은 채움 전용(도감에서 사람이 다듬은 글을 덮지 않음), 이미지는 해시 기반 멱등 업로드, 출간할 때만 운영 웹 캐시를 무효화한다. **상세는 `docs/project/apps/web-bo.md` 「세력도감」 절, 설계 SSoT는 [`unification.md`](unification.md) §4·§9.**
