...
> **최종 실측 체크: 26.08.11** — `persona` jsonb 구조·점수 범위와 용어 경계 대조
>
> **용어 경계:** 제품·문서·에이전트·사용자 대화에서는 이 16개 축을 **스펙트럼**이라 부른다. `celeb_persona` 테이블과 `persona` JSONB 열처럼 이미 배포된 DB 식별자만 레거시 내부명으로 유지하고, 그 밖의 개념명·제목·트랙명은 스펙트럼으로 통일한다.
...
담당 에이전트는 `celeb-5-spectrum`. 의존 트랙은 basic뿐이다.
...
**축 이름·정의·점수 범위·기준점은 코드가 정본이다** — `packages/shared/src/constants/celeb-spectrum-scale.ts`.
이 문서에 수치를 옮겨 적지 마라. 26.08.08 통합 이전에는 이 문서와 백오피스 상수 파일이 각각 다른 척도를 들고 있었고, 채점 담당은 문서만 읽어 다른 한쪽을 보지 못했다.

| 무엇 | 코드의 어디 |
|------|------------|
| 16축 키·한국어 이름 | `SPECTRUM_AXES`, `AXIS_LABELS` |
...
| "높은 지력을 보유" | 동어반복 |
| "여러 업적을 남김" | 추상적 |
| "일반적인 아이돌 수준의 댄스 체력" | 직군 기본값. 26.08.13 배치에서 20명에게 복제됐고 점수는 31~55로 갈렸다 |

### 근거문 중복 게이트

근거문은 **그 인물의 것**이다. 같은 그룹·같은 직군이라도 문장을 복사하지 않는다. 배치로 여러 인물을
한 번에 쓸 때 직군 기본값 문구를 돌려쓰는 것이 가장 흔한 실패였다 — 두 멤버의 16축 중 15축이 글자
그대로 같았고, 같은 문장에 34점과 36점이 붙어 문장이 점수를 설명하지 못했다.

반영 도구(`celeb:fill`)가 저장 전에 아래를 검사하고 걸리면 그 인물을 FAILED로 돌려보낸다. 판정 규칙과
허용값은 `sw/web-bo/scripts/lib/spectrum-reason-check.ts`가 정본이다.

| 결함 | 뜻 |
|------|-----|
| 같은 근거·다른 점수 | 같은 축에 같은 문장인데 점수가 다르다. 무력만 여성 보정 때문에 성별 안에서 비교한다 |
| 과도한 공유 | 같은 축의 한 문장을 허용 인원을 넘겨 공유한다 |
| 인물 복제 | 두 인물이 허용 축 수를 넘겨 같은 문장을 공유한다 |
| 기본값 문구(경고) | 「평균·일반·정보 부족·기록 없음」만 적고 행적이 없다. 반영은 되지만 감사 후보로 남는다 |

개인 근거를 끝내 못 찾은 축은 사실을 만들지 않는다. 대신 **무엇을 찾아봤고 무엇이 없었는지**를 그
인물의 사실(데뷔 연도·그룹·활동 기간)과 함께 적어 점수를 기준점과 견준다. "공개된 선행 기록 없음"
하나로 쉰 명을 덮지 않는다.

현재 DB의 중복 현황과 재조사 후보는 `pnpm celeb:audit:spectrum`이 뽑는다. 재채점은 패치를
`celeb:fill apply --replace-spectrum --only-slugs`로 덮어쓴다.
...
8단계 등급·하한선 규칙·배우 구간·여성 보정은 코드에 있다(`MARTIAL_GRADES`, `MARTIAL_FLOOR_RULE`, `PERFORMER_MARTIAL_BANDS`, `femaleMartialAdjust`, `FEMALE_MARTIAL_ADJUSTMENT_RULE`). 삼국지 인물은 `THREE_KINGDOMS_ANCHORS`를 따로 쓴다 — 게임 수치는 실존 인물보다 무장 무력이 전반적으로 높아 같은 자로 견주면 어긋난다.

**무력은 근력·민첩·지구력·강인함(부상 회복·고통 인내 포함)의 종합 추정치다.** 전장의 무인과 경기장의 선수를 같은 자로 잰다. 신체 활동 이력은 이 추정치의 증거이지 점수 자체가 아니다 — "금메달"이 아니라 "폭발적 파워·민첩" 식으로 번역하라. **시대·환경이 가한 수동 피해(피격·기아·학대·감염·전쟁 피해·포로 생활)는 본인 능동 역량이 아니므로 근거에서 뺀다 — 겪었다는 사실이 체력이 아니다.**

### 여성 무력 보정

무력 한 축에만 적용한다(능력 나머지·덕목·성향은 성별 무관). 규칙 전문은 코드의 `FEMALE_MARTIAL_ADJUSTMENT_RULE`이 정본이므로 여기에 수치를 옮기지 않는다. 요지만:

- **모든 여성에게 점진 보정을 적용한다.** athlete·commander·무술·댄서·실전 전투 등 신체 역량 입증 여성도 포함.
- 직군·이력으로 받은 무력 플러스(athlete 기본 80+, 댄서 구간 55~60 등)와 성별 보정은 **별개 차원**이다 — 직군 플러스는 산출 단계에서, 보정은 산출 후 한 번.
- 보정은 tapered: 산출 ≥50이면 -15, 50 미만이면 선형 감소(0점은 보정 0). 5점 하한 클램프. 상세 함수는 `femaleMartialAdjust`.
...
| 중상 후 복귀·지속 훈련 | 병약·만성 질환 기록 |
| 장기간 지속적 신체 훈련 | 단발성·한시적 활동 |
| 다종목 신체 훈련 (복싱+수렵+종군 등) | 단일 분야만 |
| 극한 환경 능동 극복 | 시대·환경 수동 피해 (근거 아님) |
...
- 확신이 없으면 일반값(50~55 하한)을 준다
- 신체 훈련·댄서·무용 이력이 확인되면 그 해당 구간을 준다 — 그 후 여성이면 보정을 적용한다
...
- `CRIMINAL_HISTORY_RULE`: 범죄·처벌 사실은 그 자체로 공정·성찰 등 어느 축도 직접 입증하지 않는다. 해당 축에 맞는 별도 행적이 없으면 중립으로 둔다.
- 추측으로 점수를 부풀리지 않는다
- 모든 점수는 행적 근거가 있어야 한다
- 기존 개별 컬럼(command, martial 등)에 직접 INSERT하지 않는다. **persona jsonb만 사용**

### 근거문에 쓰지 않는 것 (코드가 막는다)

판정 원문은 `sw/web-bo/scripts/lib/spectrum-reason-check.ts`이고, `celeb:fill`이 반영 전에 실행한다.
조사자는 제출 전에 `pnpm celeb:spectrum:check --file <패치>`로 같은 검사를 직접 돌린다.

| 판정 | 막는 것 |
|---|---|
| `private-info` | 질병·치료·식이·가족·열애·종교·MBTI. **생존한 연예 직군**에만 적용한다 — 역사·정치 인물에게는 가족·신앙·혼인이 공적 기록이라 가릴 대상이 아니다. 배역 준비로 몸을 만든 사실은 행적이므로 허용한다 |
| `proxy-credit` | 팬덤·팬클럽이 모아 낸 기부를 본인 행위로 적는 것. 명의가 본인이 아니면 근거가 아니다 |
| `length` | 15자 미만·40자 초과 |
| `floor` | 「확인 안 됨」류 문구에 공정·인자·절제·성찰·겸양 점수가 48~52 밖인 것. 근거가 없으면 깎지도 올리지도 않는다 |

기계가 못 잡아 사람이 보는 것 — 인물·작품 혼동과 동명이인, 그룹·회사 명의 행위를 개인 근거로 쓴 것,
게시물 해석으로 정치 성향을 매기는 것, 연도 없는 인상 서술.

---

## 작업 흐름

1. **기준점 확인**: `packages/shared/src/constants/celeb-spectrum-scale.ts`의 `SPECTRUM_ANCHORS`를 먼저 읽는다. **이 단계를 건너뛰면 회차마다 다른 자로 재게 된다.**
