/**
 * 세력도감 사진 창구 — 목록·업로드·삭제.
 *
 * 주소가 `/api/faction/media` 인 이유: 공용 사진 부품(`shared/bo/media`)이 `/api/{시리즈}/media` 로
 * 부른다. 시리즈 이름 `faction` 을 그대로 첫 토막에 두면 그 부품을 **한 줄도 고치지 않고** 쓴다.
...
export async function GET(req: Request) {
  ...
  try {
    if (url.searchParams.get('tree') === '1') {
      return NextResponse.json(await listImageTree(FACTIONS_DIR, ep))
    }
    return NextResponse.json(await listImages(FACTIONS_DIR, ep))
  } catch (e) {
...
export async function POST(req: Request) {
  ...
  try {
    if (ctype.includes('application/json')) {
      ...
      const saved = await saveImageFromUrl(FACTIONS_DIR, ep, url, basename ?? 'image')
    ...
    const saved = await saveImage(FACTIONS_DIR, ep, file.name, buf)
    return NextResponse.json({ ok: true, file: saved })
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : String(e) }, { status: 400 })
  }
...
export async function DELETE(req: Request) {
  ...
  try {
    await deleteImage(FACTIONS_DIR, ep, file)
