...
> **마지막 문서 대조: 26.08.10** — 저장소에 이미 있던 게임 문서의 위치, 책임, 문서에 적힌 공개 상태만 정리했다. 게임·시장 조사, 코드·DB·실화면 재검증, 신규 기획은 하지 않았다. 아래 상태표의 근거 문서가 마지막으로 실측된 날짜는 26.07.30~26.07.31이다.
...
아래 표는 현재 코드를 새로 검사한 결과가 아니다. 26.08.10에 기존 문서의 마지막 실측 표기를 한곳에 모은 색인이다. 공개 여부가 중요하면 상태 근거 문서를 열고, 코드까지 다시 대조한 뒤 갱신한다.

| 구역 | 게임 | 문서에 적힌 상태 | 상태 근거 |
|---|---|---|---|
| 공개 쉼터 `/rest` | 여명, 미궁, 패권, 천도 | 한 페이지에 마운트된 공개 게임 4종. 개별 페이지가 아니라 앵커로 진입 | [`architecture.md`](../project/platform/architecture.md#네비게이션), [`card-images.md`](card-images.md) |
| 개발자 모드 | 기억, 시대의 초상, 유랑 | 구현은 보존하며 로컬 개발 서버 또는 `?dev=1`에서만 카드 노출 | [`card-images.md`](card-images.md#6-적용-현황-260715-납품--260731-유랑-납품개발자-모드-노출) |
...
| 게임 | 전용 문서 | 범위 |
|---|---|---|
| 기억 | 전용 게임 규격 문서 없음 | 개발자 모드 노출과 카드 자산 보존 상태는 [`card-images.md`](card-images.md) |
