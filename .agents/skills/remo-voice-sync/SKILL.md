...
- **각 sub 청크 50자 이하.** 초과 시 파이프라인이 `sub 청크 과대` 경고로 자동 보고한다.
- 50자 초과 청크를 포함한 세그먼트는 Step 3에서 재분할 대상으로 다시 처리해야 한다.

#### 🎬 쇼츠 전용 규칙 (`--shorts <N>` 스코프)

쇼츠는 세로 화면에 자막이 한 줄씩 흐르므로 롱폼보다 조금 더 짧게 끊되, **너무 잘게 자르지 않는다**.

- **의미 단위 우선** — 어절 수보다 의미 단위 경계가 먼저. 절·구·부사어·관형구가 각각 한 덩어리.
- **분량 기준 4~6 어절.** 통사 결속(보조용언·지시사+체언·관형절+피수식어·관형구 체인+체언·의존명사구)이 길게 묶이는 경우 7~8 어절까지 허용.
- **25자 이하로 의미가 한 덩어리인 발화는 단일 sub 로 두는 것이 자연스럽다.** 굳이 쪼개지 않는다(예: "ICBM 한 발에 2천만 불." 같은 짧은 결속 발화는 한 덩어리 OK).
- 의미 결속을 깨면서 어절 수만 맞추지 않는다. 한 청크가 7~8 어절이어도 결속이 자연스러우면 그쪽이 정답.
