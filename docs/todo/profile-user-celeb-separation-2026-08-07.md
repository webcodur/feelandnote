...
> **상태: 종결(26.08.07).** 1~3단계 실행 완료, 4단계는 §6-6에서 목표를 다시 정의해 마무리했다. 다시 꺼낼 조건은 §6-6 말미에 적었다.
...
> **26.08.10 후속:** 이 문서는 26.08.07의 가짜 Auth 계정 제거와 계정 열 분리 이력이다.
> 당시 §6-6의 「`profiles` 유지」 결론은 26.08.09 재조사에서 다시 폐기됐다. 현행 구조와
> 남은 배포 게이트는
> `docs/todo/profile/profile-celeb-account-separation-improvement-2026-08-09.md`가 쥔다.
> 운영 DB에는 `member_profiles`·`celebs`와 관계별 전용 테이블, 새 RPC·트리거가 적용됐고,
> 앱 전환과 빌드 검증도 끝났다. 새 앱 배포 전이므로 레거시 제거 마이그레이션은 아직
> 적용하지 않았다. 이 이력 문서는 최종 DROP과 배포 후 검증이 끝날 때까지 TODO에 둔다.
...
4. **`users` 분리는 값이 싸진 뒤에 재판단.** "안 한다"가 아니라 "지금 값으로는 안 한다"다. 1~3단계로 얽힘이 풀렸으므로 남은 문제는 **계정 전용 열 다섯 개(`email`·`role`·`suspended_at`·`suspended_reason`·`last_seen_at`)가 2,759행 중 17행에만 의미 있다**는 것 하나다. 이걸 옮기는 비용의 대부분이 권한 규칙 55개였고, §6-5에서 그 비용을 걷어냈다
...
## 6-5. 실행 기록 — 권한 규칙 55개 정리 (26.08.07)

§3이 "`role`을 옮기려면 정책 55개를 동시에 고쳐야 한다"를 4단계의 최대 비용으로 꼽았다. 그런데 **그 55개가 하는 일을 그대로 하는 `is_admin()` 함수가 이미 DB에 있었고, 55개 중 아무도 부르지 않았다.** 전부 같은 문장을 복사해 갖고 있었다.
...
마이그레이션 `consolidate_admin_rls_checks_into_is_admin`으로 전부 `(SELECT public.is_admin())` 호출로 바꿨다. **동작은 바뀌지 않는다.**

**성능 함정을 함께 처리했다.** `is_admin()`이 `VOLATILE`이라 그대로 쓰면 **행마다** 호출된다. 기존 인라인 형태는 `( SELECT auth.uid() )`로 감싸 한 번만 계산되게 짜여 있었다. 함수를 `STABLE`로 바꾸고 호출부도 `(SELECT ...)`로 감싸 같은 성질을 유지했다.

**안전장치** — 마이그레이션이 스스로 검사하고 어긋나면 전체를 되돌린다.
- 치환 후에도 인라인 검사가 남으면 예외
- 대상 정책 수가 55가 아니면 예외
- 정책 이름·적용 범위(`PERMISSIVE`)·대상 롤·명령 종류를 원본에서 그대로 옮겨 재작성
...
|---|---|
| 남은 인라인 권한 검사 | **0** |
| `is_admin()`을 쓰는 정책 | 57 (새로 바꾼 55 + 원래 쓰던 2) · 18개 테이블 |
| 함수 변동성 | `STABLE` |
| 권한 검사가 아닌 `profiles` 참조 정책 3개 | **그대로.** `records`(2)·`user_contents`(1)의 "대상이 인물인가" 검사는 성격이 달라 손대지 않았다 |
| 다른 조건이 붙어 있던 정책 | 온전. `board_comments_select`의 공지·작성자·피드백 검사, `feedbacks_update_own_or_admin`의 `PENDING` 조건 모두 보존 |
| 동작 시험 | 관리자 `true` · 일반 회원 `false` · 비로그인 `false` |

**이 정리가 남긴 것**
- 같은 판정이 55벌 복제된 상태가 사라졌다. 판정 기준이 바뀌면 이제 함수 하나만 고친다
- `role`을 다른 테이블로 옮기는 비용이 **정책 55개 → 함수 1개**로 줄었다. 4단계를 다시 판단할 때의 셈이 달라진다

## 6-6. 4단계 최종 판단 (26.08.07) — 목표를 다시 정의하고 종결

### 원래 4단계는 실행 불가능한 형태였다

1판과 2판 §6이 적은 "회원 17명을 `users`로 옮긴다"는 **비싼 게 아니라 성립하지 않는다.** 감상·팔로우·점수·활동기록 같은 축은 인물과 회원이 함께 쓰고, 그 축의 열 하나(`user_contents.user_id`, `follows.follower_id` 등)가 두 부모 테이블을 동시에 가리킬 수 없다. 회원이 `profiles`를 떠나는 순간 그 17명을 가리키던 513행이 끊긴다.

**따라서 "행을 옮긴다"는 갈래는 폐기한다.**

### 진짜 목표는 「계정 정보가 인물 기록에 섞이지 않는 것」이었다

그 목표에 이르는 길은 둘이다.

| 갈래 | 하는 일 | 비용 | 연결 재작업 |
|---|---|---|---|
| **(a) 열을 옮긴다** | `email`·`role`·`suspended_at`·`suspended_reason`·`last_seen_at`을 `user_accounts`(17행)로 분리. `profiles.id`는 공용 식별자로 유지 | 코드 약 10파일 + 마이그레이션 1 | **없음** — 공용 축은 그대로 `profiles.id`를 가리킨다 |
| **(b) 열은 두고 오염을 DB가 막는다** | 인물 행에 계정 값이 못 들어가게 제약 | 코드 0 | 없음 |

(a)의 비용 대부분은 `role`을 읽는 7곳이었는데, §6-5에서 권한 판정을 `is_admin()` 하나로 모아 그 부담이 크게 줄었다. **(a)는 이제 언제든 싸게 할 수 있다.**

### 결정: (b)를 실행하고 여기서 종결한다

마이그레이션 `forbid_account_fields_on_celeb_profiles`로 제약을 걸었다.

```sql
CHECK (profile_type <> 'CELEB'
       OR (email IS NULL AND coalesce(role,'user') = 'user'
           AND suspended_at IS NULL AND suspended_reason IS NULL
           AND last_seen_at IS NULL))
```

동작 시험: 인물에게 정지 표시를 걸려는 UPDATE, 관리자 권한을 주려는 UPDATE **둘 다 차단**된다. 기존 데이터 위반 0건.

**(a)를 지금 하지 않는 이유.** 실제로 났던 사고는 회원 정지 기능이 인물에게 실행된 것 하나였고(손웅정), 그 실패 경로는 제약으로 **불가능**해졌다. 남은 이득은 모델이 깔끔해지는 것인데, 회원이 17명이고 늘 조짐이 없는 상태에서 코드 10파일을 바꿀 값이 아니다. 회원 쪽이 실제로 자라면 그때 (a)를 하면 되고, §6-5 덕분에 그 비용은 이미 낮다.

### 착수 조건 — 다음에 (a)를 꺼낼 때

아래 중 하나라도 해당하면 (a)를 실행한다.

- 회원 수가 세 자리로 늘어난다
- 회원 전용 열이 새로 필요해진다(알림 설정·구독 등급 등)
- 회원 전용 화면이 `profiles`를 훑느라 인물 2,742행을 걸러내는 부담이 눈에 띈다

**그 전까지 이 문서는 종결 상태다.**

## 6-7. 실행 기록 — 계정 정보 분리 완료 (26.08.07)

§6-6이 "(a)는 지금 안 한다"로 닫았으나, **반창고(제약)로 굳히는 것보다 고치는 게 맞다**는 판단으로 (a)를 실행했다. §6-5에서 권한 판정을 함수 하나로 모아둔 덕에 비용이 이미 낮았다.

### 왜 사람은 안 나눴나

"같은 스키마 두 테이블을 UNION으로 합쳐 읽고, 인물이 두 쪽을 오간다"는 그림도 가능하다. 다만 **외래키는 두 부모를 동시에 가리킬 수 없어** 공용 축(감상·팔로우·점수)이 갈 곳을 잃는다. 부모 테이블을 따로 두면 풀리지만, 정작 원하는 "오가는 유연함"은 **안 나눌 때 더 싸다** — 줄이 움직이지 않고 `profile_type` 하나만 바뀐다. 그래서 사람은 `profiles` 한 곳에 두고 **계정만** 뗐다.

### DB

| 마이그레이션 | 내용 |
|---|---|
| `create_user_accounts_table` | `user_accounts` 신설. 회원 17행 이관(값 불일치 0). `id`는 `profiles.id` 참조 |
| `add_account_status_to_user_accounts` | 회원 정지를 `profiles.status`(인물 공개 상태)에서 분리 |
| `point_is_admin_at_user_accounts` | `is_admin()`이 계정 테이블을 본다. **정책 55개가 이 함수를 부르므로 함수 하나만 바꿔 전부 따라왔다.** SECURITY DEFINER라 되돌림(재귀) 없음 확인 |
| `handle_new_user_creates_account_row` | 가입 시 사람 기록과 계정 기록을 함께 만든다 |
| `drop_account_columns_from_profiles` | `email`·`role`·`suspended_at`·`suspended_reason`·`last_seen_at` 제거. 호환 뷰 `profiles_compat`을 같은 보안 설정(`security_invoker`, 읽기 전용 권한)으로 재생성 |

`profiles.status`는 남겼다 — 인물 공개 상태라 계정과 다른 축이다.

### 코드 (18파일, 커밋 `1a4d0299`)

관리자 진입 판정 9곳, 회원 계정 화면, 신고 처리·상세, 회원 목록, 기록 화면, 대시보드, 내 정보 조회.

### 🔴 배포 사고와 교훈

첫 배포가 **빌드에서 실패했다.** 인물 콘텐츠 개수 함수의 인자가 3→2로 줄었는데 **호출부만 커밋되고 함수 본체가 빠져 있었다.**

- **로컬 타입체크가 못 잡는다.** 타입체크는 작업본을 보고 배포는 커밋된 것을 빌드한다. 작업본에는 함수가 이미 고쳐져 있어 계속 통과했다.
- 고친 뒤 푸시하기 전에 **커밋된 상태로** 호출부 8곳을 훑었더니 `sw/web/src/actions/home/getCelebs.ts`가 아직 3인자였다. 이걸 안 잡았으면 두 번째 실패였다.
- **규칙: 커밋 직후 `git show HEAD:<파일>` 기준으로 정합성을 확인한다.** 작업본 검사만으로 배포를 판단하지 않는다.

수정 커밋 `a9cf2132`(시그니처·시험) · `150aa74a`(잔여 호출부).

### 검증 (열 제거 후)

| 확인 | 결과 |
|---|---|
| `profiles`에 남은 계정 열 | **0** (열 50 → 45) |
| 사람 / 계정 | 2,759 / 17 · 계정 없는 회원 0 |
| 감상 기록 | 11,722 · 무손실 |
| 인물 상세 조회 · 호환 뷰 | 정상 (뷰 2,759행, `security_invoker` 유지) |
| 권한 정책 | 59개가 `is_admin()` 사용 |
| 타입체크 | sw/web · sw/web-bo 통과 |
| 운영 동작 | 배포 후 사용자 확인 완료 |

### 남은 것

- `sw/web/src/types/supabase.ts`가 낡았다(제거된 5열이 아직 적혀 있고 `user_accounts`가 없다). 브라우저 클라이언트만 이 타입을 쓰고 해당 열을 읽는 코드가 없어 지금은 영향이 없다. **다음에 타입을 다시 뽑을 때 함께 갱신한다.**
