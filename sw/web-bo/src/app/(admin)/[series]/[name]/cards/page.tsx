...
export default function CardsPage() {
  ...
  useEffect(() => {
    document.title = `${episode?.host?.nickname ?? name} 카드 — Feel & Note BO`
  }, [episode, name])
        ...
        {cards.map((card, i) => (
          <div key={`${mode}-${bookIdx}-${ratio}-${i}`} className="flex-none flex flex-col items-center justify-center cursor-pointer group" onClick={() => handleCardClick(card)}>
            <div className="relative">
              <div className="absolute inset-0 z-10 group-hover:ring-4 ring-accent/50 rounded-[10px]" title="클릭하여 편집 영역으로 이동" />
              <Player
                component={BookCard}
        ...
        <div className="p-8 max-w-4xl mx-auto space-y-12 pb-32">
          ...
          {activeEditorId === 'edit-number' && (
            ...
            <div className="grid grid-cols-2 gap-5 p-6 rounded-2xl bg-[var(--color-bg-card)] border border-border shadow-sm">
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">값 (Value)
                <input value={overrides?.numberCard?.value ?? ''} onChange={e => updateOverride(['numberCard', 'value'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder={String(scriptBase?.books?.length ?? '')} />
              </label>
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">단위 (Unit)
                <input value={overrides?.numberCard?.unit ?? ''} onChange={e => updateOverride(['numberCard', 'unit'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder="권의 책" />
              </label>
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">설명 (Description)
                <input value={overrides?.numberCard?.desc ?? ''} onChange={e => updateOverride(['numberCard', 'desc'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder={`${scriptBase?.host?.nickname}의 서재`} />
              </label>
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">태그 (Tag)
                <input value={overrides?.numberCard?.tag ?? ''} onChange={e => updateOverride(['numberCard', 'tag'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder="서재 탐방" />
              </label>
            </div>
          </div>
          )}
          ...
          {activeEditorId === 'edit-intro' && (
            ...
            <div className="grid grid-cols-2 gap-5 p-6 rounded-2xl bg-[var(--color-bg-card)] border border-border shadow-sm">
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">이름 (Nickname)
                <input value={overrides?.host?.nickname ?? ''} onChange={e => updateOverride(['host', 'nickname'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder={scriptBase?.host?.nickname} />
              </label>
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">직함 (Title)
                <input value={overrides?.host?.title ?? ''} onChange={e => updateOverride(['host', 'title'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder={scriptBase?.host?.title} />
              </label>
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium col-span-2">인물 설명 (Bio)
                <textarea value={overrides?.host?.bio ?? ''} onChange={e => updateOverride(['host', 'bio'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary h-24 focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner leading-relaxed" placeholder={scriptBase?.narrator?.celebIntro} />
              </label>
              <label className="flex flex-col gap-2 text-sm text-text-dim font-medium col-span-2">핵심 철학 (Philosophy)
                <textarea value={overrides?.host?.philosophy ?? ''} onChange={e => updateOverride(['host', 'philosophy'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary h-24 focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner leading-relaxed" placeholder={scriptBase?.host?.featuredQuote || scriptBase?.host?.philosophy} />
              </label>
            </div>
          </div>
          )}
          ...
          {activeEditorId?.startsWith('edit-book-') && (
            ...
            {books.map((b, i) => activeEditorId === `edit-book-${i}` && (
              <div key={i} id={`edit-book-${i}`} className={`space-y-5 p-6 rounded-2xl border shadow-sm scroll-mt-10 ${selected.includes(i) ? 'border-border bg-[var(--color-bg-card)]' : 'border-border/40 bg-[var(--color-bg-card)] opacity-40 grayscale-[20%]'}`}>
                ...
                <div className="grid grid-cols-1 gap-5">
                  <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">카드 표시용 단축 제목 (Title)
                    <input value={overrides?.books?.[i]?.title ?? ''} onChange={e => updateOverride(['books', String(i), 'title'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner" placeholder={b.title} />
                  </label>
                  <label className="flex flex-col gap-2 text-sm text-text-dim font-medium">감상배경 / 추천이유 (WHY READ)
                    <textarea value={overrides?.books?.[i]?.context ?? ''} onChange={e => updateOverride(['books', String(i), 'context'], e.target.value)} className="p-3 rounded-xl bg-[#ffffff] border border-border text-text-primary h-40 focus:border-accent focus:ring-1 ring-accent outline-none shadow-inner leading-relaxed" placeholder={b.contextMain} />
                  </label>
                </div>
              </div>
            ))}
