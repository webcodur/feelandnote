 ...
 *
 * faction_people의 `is_person=true` 행은 자연인 또는 하나의 이름·행위 주체를 가진 개별 허구 인물만 담는다.
 * `is_person=false` 서사 항목에는 이 가드를 적용하지 않는다. 회사·조직·제품·기계·기체·부대·종족·듀오는
 * 개인샷으로 만들지 않고 faction_groups와 미디어 문맥으로 관리한다. 완전한 의미 판별기는 아니며,
 * 과거 사고에서 확인된 명백한 비인물 입력을 인물 진입점에서 막는 가드다.
 */
