      ...
      <div className="bg-bg-main border border-border/60 rounded p-2 flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <select value={engine} onChange={e => setEngine(e.target.value)}
            ...
            className="bg-white border border-slate-300 text-slate-950 font-black rounded px-2.5 py-1 text-xs outline-none focus:border-accent shadow-sm cursor-pointer">
          ...
          <input placeholder="특정 대상 ID (예: book-0-title)" value={only} onChange={e => setOnly(e.target.value)}
            className="bg-white border border-slate-300 text-slate-950 font-bold rounded px-2.5 py-1 text-xs w-48 outline-none focus:border-accent placeholder:text-slate-400 shadow-sm" />
          <label
            title={alignEnabled
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 rounded text-xs font-black bg-accent text-white hover:bg-accent-hover shadow-sm">
            전체 생성
          </button>
          <button
            ...
            className="px-3 py-1.5 rounded text-xs font-black bg-white border border-slate-300 text-slate-900 hover:border-accent hover:text-accent hover:bg-slate-50 shadow-sm">
            누락분만 생성
          </button>
          {hasShorts && (
            <button
