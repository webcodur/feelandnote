...
| 한국어 화면에 뜨는 작품 / 영문 화면 | 3,447권 / 3,151권 (쿠팡 활성 상품이 붙은 작품 305권) |
...
**2회차 판본 대조는 476건 중 383건까지 왔다(2026-09-07).** 통과분으로 파트너스 링크 157건을 만들었고(`_picks-r2-chunk-*.json` → `pick.mjs`, 20건씩 전경 실행), 상품 붙은 작품이 149권에서 305권이 됐다. 남은 93건은 두 프로필이 모두 차단돼 새 프로필에서 `verify-edition.mjs … 383 476`으로 이어간다. `pick.mjs`는 코덱스 경로처럼 제목만 남긴 상품명도 현재 상품명의 앞부분과 같으면 같은 상품으로 본다.
