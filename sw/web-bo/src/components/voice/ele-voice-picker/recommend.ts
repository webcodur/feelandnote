...
/**
 * 보이스 추천 순위 매기기 — 세력도감 인물과 인물 상세가 함께 쓴다.
 *
 * 「이 인물에게 어떤 목소리가 어울리나」를 판단하는 규칙(권력자형·연구자형·해커형…)은 화면이
 * 달라도 같다. 다른 것은 인물을 설명하는 말을 어디서 긁어오느냐뿐이라, 그 말 뭉치(subjectText)만
 * 받아 채점한다. 부르는 쪽이 자기 데이터에서 문장을 만들어 넘긴다.
 */
...
type BuildRecommendationArgs = {
  /** 인물을 설명하는 말 뭉치 — 이름·직군·소속·대사 등을 부르는 쪽이 이어 붙여 넘긴다 */
  subjectText: string
  voices: EleVoiceLike[]
...
/** 인물 설명 조각들을 채점용 한 줄로 잇는다 — 부르는 쪽이 자기 데이터로 만든다 */
export function joinSubjectText(parts: (string | null | undefined)[]) {
