...
export const NARRATIVE_CANDIDATE_SCHEMA_VERSION = 1 as const
...
export type NarrativeProfileSnapshot = {
  id: string
  slug: string
...
export type FigureBookLocaleSnapshot = {
  locale: string
  title: string | null
  creator: string | null
  description: string | null
...
export type FigureBookAssignmentSnapshot = {
  content_id: string
  ...
  locales: FigureBookLocaleSnapshot[]
}

export type FigureBookSnapshot = {
  fingerprint: string
  assignments: FigureBookAssignmentSnapshot[]
}
...
export type NarrativeCandidateEvent = {
...
export type NarrativeCandidateDeletion = {
  id: string
  reason: string
}

export type NarrativeCandidate = {
  schema_version: typeof NARRATIVE_CANDIDATE_SCHEMA_VERSION
  slug: string
  celeb_id: string
  ...
  source_snapshot: FigureBookSnapshot
  anchor_source_ids: string[]
  source_selection_reason: string
  events: NarrativeCandidateEvent[]
  deletions: NarrativeCandidateDeletion[]
  quality_notes: {
...
export type NarrativeCandidateSeed = {
  schema_version: typeof NARRATIVE_CANDIDATE_SCHEMA_VERSION
  slug: string
  celeb_id: string
  celeb_reality: 'FICTION' | 'BOTH'
...
export function fingerprintNarrativeProfile(profile: NarrativeProfileSnapshot): string {
  return hashValue({
    id: profile.id,
    slug: profile.slug,
...
function validateSourceSnapshot(value: unknown, errors: string[]): value is FigureBookSnapshot {
  if (!isRecord(value)) {
  ...
  const snapshot = value as FigureBookSnapshot
...
export function validateNarrativeCandidate(value: unknown): {
  ...
  const candidate = value as NarrativeCandidate

  if (candidate.schema_version !== NARRATIVE_CANDIDATE_SCHEMA_VERSION) {
...
export async function fetchFictionProfileBySlug(
  db: DatabaseClient,
  slug: string,
): Promise<NarrativeProfileSnapshot | null> {
  ...
  return data as NarrativeProfileSnapshot | null
}
...
export async function loadFigureBookSnapshots(
  db: DatabaseClient,
  celebIds: string[],
): Promise<Map<string, FigureBookSnapshot>> {
  ...
  const snapshots = new Map<string, FigureBookSnapshot>()
  for (const celebId of celebIds) {
    const snapshotAssignments = (assignmentsByCeleb.get(celebId) ?? [])
      ...
      .map((assignment): FigureBookAssignmentSnapshot => {
...
export async function loadFigureBookSnapshot(
  db: DatabaseClient,
  celebId: string,
): Promise<FigureBookSnapshot> {
  return (await loadFigureBookSnapshots(db, [celebId])).get(celebId) ?? {
