...
function Switch({ checked, onChange, activeColor = 'bg-accent' }: { checked: boolean, onChange: (c: boolean) => void, activeColor?: string }) {
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent focus:outline-none ${checked ? activeColor : 'bg-white/20 border-black/30 shadow-inner'}`}
    >
      <span
        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 ${checked ? 'translate-x-4' : 'translate-x-0'}`}
      />
    </button>
  )
}
...
const CTRL = 'bg-white border border-slate-300 rounded px-1.5 py-0 h-[22px] w-full cursor-pointer text-slate-950 font-black focus:border-accent focus:bg-slate-50 focus:outline-none shadow-sm'
      ...
      <div className="grid grid-cols-12 gap-x-2 gap-y-2 items-center text-sm font-bold">
        ...
        {seg.role === 'celeb' ? (
          <Cell colSpan={4} label="출처">
            <input
              className="text-amber-950 font-black bg-white border border-slate-300 rounded px-1.5 py-0 h-[22px] focus:border-accent focus:bg-slate-50 focus:outline-none flex-1 w-full shadow-sm"
        ...
        {isOverrideActive && (
          <Cell colSpan={12} label="오버라이드" title="이 구간 보이스/스타일 제미니 강제 오버라이드. 입력하면 엔진과 무관하게 제미니로 우회 생성됩니다.">
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              <GeminiVoiceSelect value={seg.geminiVoice ?? ''} onChange={next => updateSegField(idx, 'geminiVoice', next || undefined)} placeholderLabel="보이스 ID (미지정)" className="bg-white border border-slate-300 rounded px-1.5 py-0 h-[22px] w-32 focus:border-accent focus:bg-slate-50 focus:outline-none text-blue-900 font-extrabold font-mono shadow-sm" />
              <input value={seg.style ?? ''} onChange={e => updateSegField(idx, 'style', e.target.value || undefined)} placeholder="제미니 스타일 (예: 속삭이듯)" className="bg-white border border-slate-300 rounded px-1.5 py-0 h-[22px] flex-1 min-w-0 focus:border-accent focus:bg-slate-50 focus:outline-none text-slate-950 font-bold italic placeholder:text-slate-400 shadow-sm" />
