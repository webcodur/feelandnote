...
실측: 저장소에 `.env` 파일이 하나도 없고 셸에도 DB 연결 환경값이 없다. **실제 조회는 로컬에서 100% 실패한다.** 유저가 직접 만져보려면 표본 데이터가 있어야 한다.

그래서 데이터 경로를 둘로 만든다.

1. **실제 조회** — 서버 액션이 Oracle 운영 DB를 읽는다. 운영 환경의 정답 경로다.
...
- 디스크에 이미 있는 실제 프로젝트 데이터를 표본 재료로 써도 된다: `sw/remotion/public/episodes/<인물>/`.
...
7개 게임의 `fixture.ts`에 하드코딩된 인물 표본 314항목을 당시 실 DB(현 `celebs`로 이관된 활성 CELEB 1,736명)와 기계적으로 전수 대조했다. 현재 DB 정본은 `celebs`다.
