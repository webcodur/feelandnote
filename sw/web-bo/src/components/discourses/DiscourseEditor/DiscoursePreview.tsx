 ...
 * 길이 산식은 근사치로 베끼지 않고 **렌더의 buildCues 를 그대로 불러 쓴다**
                ...
                return (
                  <div key={i} className="space-y-1 rounded-md bg-bg-main/40 p-2" style={{ borderInlineStart: `3px solid ${color}` }}>
                    <div className="flex items-center gap-1.5">
                      ...
                      {!t.duration && <span className="rounded bg-warning/15 px-1 text-[10px] font-semibold text-warning-text" title="음성이 없어 글자 수로 길이를 어림잡았습니다">추정</span>}
                    </div>
                          ...
                          return (
                            <div key={ci} className="flex items-center gap-1">
                              {img && (chSrc
                                ...
                                : <span className="h-5 w-5 shrink-0 rounded bg-danger/15" title="연결된 사진을 찾을 수 없습니다" />)}
