...
이 수치를 곧바로 “얇은 페이지 77.1%”라고 부르면 안 된다. 상세 페이지에는 네이버·TMDB·IGDB·Spotify에서 가져온 소개·줄거리·트랙 정보도 붙고, 구글은 최소 글자 수나 감상문 건수 기준을 공개하지 않는다. 다만 사이트가 직접 더한 감상·출처 설명이 한 건의 짧은 외부 자료 요약뿐인 페이지가 많다는 뜻이다. 라이브 균등 표본 24건에서도 19건이 감상문 1건이었고, 일부 음악 상세 페이지는 메뉴·푸터를 제외한 본문이 281~365자였다.
