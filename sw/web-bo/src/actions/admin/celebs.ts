...
// #region Celeb Works (창작물)
export interface CelebWorkRow {
  id: string
  celeb_id: string
...
export async function getCelebWorksAdmin(
  celebId: string,
  page = 1,
  limit = 20,
  workType?: string,
): Promise<{ works: CelebWorkRow[]; total: number }> {
  const supabase = createAdminClient()
  const offset = (page - 1) * limit
  ...
  if (error) {
    console.error('getCelebWorksAdmin 에러:', error)
    return { works: [], total: 0 }
  }

  return { works: (data ?? []) as CelebWorkRow[], total: count ?? 0 }
}

export async function addCelebWork(input: {
  celebId: string
  ...
  const { error } = await supabase.from('celeb_works').insert({
    ...
    search_keyword: input.searchKeyword || null,
  })
...
export async function updateCelebWork(input: {
  id: string
  title?: string
  titleEn?: string | null
  role?: string
  ...
  if (input.titleEn !== undefined) updates.title_en = input.titleEn
  if (input.role !== undefined) updates.role = input.role
  if (input.description !== undefined) updates.description = input.description
  if (input.descriptionEn !== undefined) updates.description_en = input.descriptionEn
  ...
  if (input.searchKeyword !== undefined) updates.search_keyword = input.searchKeyword
  if (input.contentId !== undefined) updates.content_id = input.contentId

  const { error } = await supabase.from('celeb_works').update(updates).eq('id', input.id)
...
export async function deleteCelebWork(id: string) {
  const supabase = createAdminClient()

  const { error } = await supabase.from('celeb_works').delete().eq('id', id)
