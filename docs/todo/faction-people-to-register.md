...
>
> 🔴 **명언 출처 재검증이 남았다.** 작업 도중 세션 웹 검색 한도(200회)가 소진돼 **1차 자료 대조를
> 끝내지 못했다.** 40건 중 출처가 확인된 것은 잣코(워싱턴포스트)·하렐·몬세구르·해먼드·아이작먼(위키백과
> 직접 인용)·다간·알렉산더 정도이고, 나머지는 업적 기반 창작(룰북 §6.2 D 허용)이거나 **널리 알려진
> 발언이지만 이번에 원문을 열어 확인하지 못한 것**이다. 후자에 해당하는 것은 도너번·윌리엄 보잉·
> 엘리 코헨·벡위드·쩡위췬·왕촨푸·베테유·바이락타르·플레이터·맥레이븐·베토 오로크다.
> 검색 한도를 올린 뒤 이 목록만 대조하면 된다.
> 폰 브라운의 '로켓은 완벽했다, 엉뚱한 행성에 떨어졌을 뿐'은 1차 출처가 없는 전승이라 **이미 교체했다.**
>
> **남은 일**
...
### B-5. 위대한 해커들 — 국가의 군단 (great-hackers-state)
...
| 대상 | 인원 | 메모 |
|---|---|---|
| 가장 어두운 시간 (churchill-darkest-hour) | 61 | 처칠·루스벨트·스탈린·롬멜·주코프 등 2차대전 지휘관 전원. 전부 유명 인물이라 등록 가치가 높다 |
