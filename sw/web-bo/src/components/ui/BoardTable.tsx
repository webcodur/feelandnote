...
/**
 * 관리 목록 화면의 공용 표 부품 — 세력도감 테마·담화·랭킹·인물 도서 목록이 같이 쓴다.
 *
...
export type BoardRowTone = 'default' | 'group' | 'child'

const TONE: Record<BoardRowTone, string> = {
  default: '',
...
export function BoardTableCell({
  align = 'left',
  className = '',
  colSpan,
  style,
 ...
 */
export function BoardTableSection({
  colSpan,
  title,
  note,
  level = 1,
...
/** 아무것도 없을 때의 한 줄 */
export function BoardTableEmpty({ colSpan, children }: { colSpan: number; children: ReactNode }) {
  return (
    <tr>
 ...
 */
export function BoardTableBadge({
  color,
  icon,
  title,
...
export function BoardTableCount({ value, icon, title }: { value: number; icon?: ReactNode; title?: string }) {
  return (
    <span
      title={title}
