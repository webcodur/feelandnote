...
> §2.2~2.4는 당시 스키마 설명이며, 현행 여부를 다시 대조하지 않았다. 현재 계약은 `docs/project/data/db-core.md`를 기준으로 하고 이 문서는 차이 조사 후보로만 사용한다.

> ✅ **마이그레이션 완료 (2026-03-06)** — Phase 1(DDL+이관), Phase 2(앱코드 전환), Phase 3(레거시 DROP) 모두 완료.
> 아래는 설계 당시 문서. 마이그레이션 완료 당시에는 `content_locales` 테이블을 유일한 로케일 데이터 소스로 정리했다.
...
→ **`docs/archive/data/en-book-data-quality.md`** 참조 (진단, 수정 프로세스, sources 스키마, 썸네일 수집 규칙, API 사양, 이력 전부 포함).
