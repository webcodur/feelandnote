...
const BASE_ROW: FictionSourcePurchaseOptionRow = {
  edition_id: 7,
  content_id: 'work-1',
  locale: 'ko',
  title: '일리아스',
...
test('요청 locale은 구매 플랫폼 하나로 고정한다', () => {
  assert.equal(getFictionSourcePurchasePlatform('ko'), 'coupang')
  assert.equal(getFictionSourcePurchasePlatform('en'), 'amazon')
  assert.equal(getFictionSourcePurchasePlatform('ja'), null)
})
...
test('인물별 등장 설명은 요청 언어 값만 사용한다', () => {
  ...
  assert.equal(getFictionSourceCharacterDescription(assignment, 'ko'), '한국어 등장 설명')
  assert.equal(getFictionSourceCharacterDescription(assignment, 'en'), 'English appearance description')
  assert.equal(getFictionSourceCharacterDescription({
    description: '한국어 등장 설명',
    description_en: null,
  }, 'en'), null)
})
