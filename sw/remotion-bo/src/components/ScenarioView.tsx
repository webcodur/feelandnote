...
function LongformView({ episode }: { episode: EpisodeData }) {
...
function ShortsView({ episode }: { episode: EpisodeData }) {
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className={`text-xs font-semibold ${ROLE_COLORS[seg.role] ?? 'text-text-secondary'}`}>
                {seg.role === 'celeb' ? episode.host.nickname : ROLE_LABELS[seg.role] ?? seg.role}
              </span>
...
export function ScenarioView({ episode }: { episode: EpisodeData }) {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {view === 'longform' ? (
        <LongformView episode={episode} />
      ) : (
        <ShortsView episode={episode} />
      )}
    </div>
  )
}
