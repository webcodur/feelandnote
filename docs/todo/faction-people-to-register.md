...
- `--image-url` : 본인·소속기관·공식/권위 매체 등 신원이 확인되는 웹 이미지
- `--image-file` : 독립 신원 근거를 확보한 로컬 완성본. `_재료`·`서비스_재료`·`_refs`는 거부된다
...
### 폐기된 방식: `_refs`를 셀럽 아바타로 직접 등록

```
sw/remotion/public/factions/<편>/_refs/            ← 편에 따라 하위 폴더로 나뉘기도 한다
```

이 폴더는 영상 생성용 얼굴 참조 재료이지 신원 자료가 아니다. 파일명이 한국어 인물명이어도
그 인물의 실제 얼굴이라는 보장이 없으며, 셀럽 아바타 입력으로 직접 쓰면 안 된다.

실제 사고로 제베와 방연의 참조 이미지가 해시까지 동일했고(`790ab07a`), 제베 서비스 아바타까지
잘못 등록됐다. 제베 이미지는 2026-07-30 DB·R2에서 제거했다. 동시대 초상이나 외모 기록이 없는
제베는 임의 얼굴로 다시 채우지 않는다. 실존 인물은 공식·기관·본인 페이지 또는 인물명이 확인되는
역사 초상·도상으로 독립 대조한 뒤에만 등록한다.
