...
function FeatureCard({ href, icon, title, gradient }: FeatureCardProps) {
  return (
    <Link
      href={href}
      className={`group flex flex-col items-center justify-center p-5 rounded-2xl text-center transition-all duration-300 hover:scale-105 no-underline ${gradient}`}
    >
      <div className="w-12 h-12 mb-3 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center transition-transform group-hover:scale-110">
        {icon}
      </div>
      <h3 className="text-lg font-bold text-white mb-1">{title}</h3>
      <div className="inline-flex items-center gap-1 text-white/70 group-hover:text-white transition-colors">
