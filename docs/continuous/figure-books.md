...
| 한국어 화면에 뜨는 작품 / 영문 화면 | 3,446권 / 3,217권 (쿠팡 활성 상품이 붙은 작품 345권) |
| 관계 | 7,985건 — 등장 3,787 · 창작 3,114 · 연관 1,084 |
...
**2회차 판본 대조 476건을 끝냈다(2026-09-07).** 일치 195·불일치 176·ISBN 미노출 90. 통과분으로 파트너스 링크 197건을 만들었고(`picks-from-verify` → 20건 청크 → `pick.mjs` 전경 실행), 상품 붙은 작품이 149권에서 345권이 됐다. 검토 상품이 검색 결과에서 사라진 13건은 비워 두었다. 차단된 프로필은 새 `--user-data-dir`로 바꾸면 바로 풀린다 — 하루에 프로필 셋을 썼다. `pick.mjs`는 코덱스 경로처럼 제목만 남긴 상품명도 현재 상품명의 앞부분과 같으면 같은 상품으로 본다.
