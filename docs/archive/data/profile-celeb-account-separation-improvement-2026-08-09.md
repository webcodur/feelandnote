...
>
> 상태: **완료(26.08.10) — 새 앱 운영 배포, 레거시 제거, 타입 재생성, 배포 후 검증까지 종료**

## 26.08.10 실행 상태

### 운영 DB에 적용한 최종 전진 마이그레이션

다음 여덟 파일은 운영 DB에 순서대로 적용했다.
...
- `20260809184517_retire_legacy_profile_domain.sql`
...
여덟 번째 파일은 새 앱 배포 뒤 호환 트리거·옛 FK·혼합 테이블·`profiles_compat`·`profiles`와
`contents.user_count`를 제거하고 `content_recommendations.member_content_id`만 남겼다.
...
커밋 `ffbf2c43`을 `main`에 배포했고 Vercel의 `feelandnote`·`feelandnote-bo` Production
배포가 모두 성공한 뒤 마지막 제거 마이그레이션을 적용했다. 적용 직전 원본·분할 행 수를
다시 대조했고, 커밋된 파일 전체의 원격 `BEGIN → DROP/검증 → ROLLBACK` 시험도 13.32초에
재통과했다. 실제 적용은 마이그레이션 이력 기록과 같은 트랜잭션에서 11.16초에 끝났다.

적용 뒤 운영 DB와 PostgREST에서 다음을 확인했다.

- `profiles`·`profiles_compat`와 옛 혼합 관계 7종이 모두 제거됨
- 현행 DB의 `profile_type` 열, 레거시 함수 참조, 레거시 정책 참조가 각각 0건
- 새 도메인 외래키의 미검증 상태 0건
- 회원 17명, 셀럽 2,966명, 회원 감상 34건, 셀럽 감상 12,563건 보존
- 회원→회원 팔로우 2건, 회원→셀럽 팔로우 13건, 회원 방명록 1건 보존
- `contents.user_count`와 추천의 옛 `user_content_id` 제거, `member_content_id` 유지
- 파생 점수·소셜·로그·알림 테이블에 대한 회원 직접 쓰기 권한 없음
- 새 `celebs`·`member_profiles`·`member_contents` REST 조회는 200, 은퇴한 `profiles`·
  `user_contents` 조회는 404

Supabase CLI 2.113.0으로 `sw/web/src/types/supabase.ts`를 다시 생성했다. 은퇴한 테이블 선언과
`profile_type` 토큰은 0건이다. 최종 타입으로 `pnpm build:web`, `pnpm build:bo`, Remotion
TypeScript 검사를 다시 통과했다. 운영 `https://feelandnote.com`의 홈·인물 상세·사이트맵·
피드·새 SEO 이미지 라우트도 모두 200을 반환했다.

공개·등급 필터 뒤 출생 사건 한 건을 DB에 바로 넣던
`sw/web-bo/scripts/auto-seed-birth.mjs`는 전원 모집단과 최소 품질 규칙을 모두 어겨 은퇴했다.
타임라인 본문 대량 생성은 이 구조 전환과 별도 작업이며, 남은 도구는 `celebs`를 조회한다.

### 완료 판정

운영 배포·레거시 제거·타입 재생성·배포 후 검증까지 끝났다. 이 문서와 26.08.07 이력은
`docs/archive/`로 옮기고 `AGENTS.md` TODO 색인에서 제거한다.
...
26.08.10에는 다음 여덟 마이그레이션을 추가 적용했다.
...
새 앱의 Production 배포와 구버전 종료를 확인한 뒤
`20260809184517_retire_legacy_profile_domain.sql`을 적용했다. 옛 혼합 테이블과 호환 변경
포착은 운영 DB에서 제거됐고, 물리 분리와 `profile_type` 제거가 끝났다.
...
원격 전체 스키마를 새 빈 DB에 한 번에 재현하는 squash/baseline 정리는 별도 유지보수로 남아
있다. 이는 기존 테이블의 값을 못 옮겼거나 새 물리 테이블이 0행이라는 뜻이 아니며, 회원·셀럽
물리 분리의 미완료 항목이나 배포 blocker로 세지 않는다.
...
실제 이행은 아래 세로 묶음의 추가→변경 포착→백필→읽기·쓰기 전환→검증→기존 구조
제거까지 마쳤다.
...
| 문서 | 처리 |
|---|---|
| `docs/archive/data/profile-table-separation-considerations.md` | 공용 부모 권고가 폐기된 선행 메모로 유지 |
| `docs/archive/data/profile-user-celeb-separation-2026-08-07.md` | 26.08.07 계정 분리 이력으로 보존 |
...
1. 최종 스키마 규칙은 `docs/project/` 문서에 반영했다.
2. 이 문서와 26.08.07 진행 문서는 완료 이력으로 `docs/archive/`에 옮겼다.
3. `AGENTS.md` TODO에서 이 작업을 제거했다.
4. 타임라인 결손 전수 감사와 생성 작업은 별도 TODO에서 이어간다.
