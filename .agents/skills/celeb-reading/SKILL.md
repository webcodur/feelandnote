...
3. `NULL`인 기존 글은 한영 두 탭을 먼저 읽는다. 통과하면 본문을 유지하고 `ai_reviewed`만 기록한다.
4. 실패한 인물만 신원 조사와 심화 조사를 실행한다. 조사 URL은 DB에 적재하지 않는다.
5. 한국어 초안, 한영 최종본, 의미 검수를 통과시키고 `ai_reviewed`와 함께 조건부 반영한다.
6. 혼합 표본을 직접 읽어 품질과 속도를 확인한 뒤 전량을 독립 릴레이로 처리한다.
...
수에 넣지 않고 `.tmp-celeb-reading/`의 실패 사유와 재개 명령을 보고한다.
