...
넓게 보여주는 형태로, `작품 소개 → 감상배경 → 기타 메타데이터` 순서로 읽는다. 감상배경은 현재 본문 열의 가로폭을 전부 쓰고
여섯 줄 높이 안에서 스크롤한다. 캐러셀·손가락 밀기는
...
목록 열은 본문과 같은 외곽·높이를 쓰며 처음과 끝은 서로 이어 순환한다. 모바일 상세는 바깥 패널의 여백을 되찾아
...
섹션 위계를 쓴다. 소개가 길면 제한된 높이 안에서 스크롤하며, 책 기본 정보는 넓은 화면에서
