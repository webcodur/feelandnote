...
> 이력 문서. 현행 규칙 아님. 작성 시점: 26.03. 실행 완료(figures/ranking/today/feed 라우트 및 navigation.tsx 반영됨).
> 당시의 용어 규칙과 대응표도 현재 기준으로 직접 인용하지 않는다. 현행 명칭은 `docs/project/platform/code-rules.md`와 `docs/project/service/`를 따른다.

> 아래는 당시 AI 에이전트가 순서대로 실행한 작업 목록이다.
