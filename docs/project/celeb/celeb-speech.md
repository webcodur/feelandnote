...
- 현재는 `lines.quote`만 작성한다. 영문은 사용자가 별도로 요청할 때만 다룬다.
...
웹·게임 대사는 버튼 한 번에 튀어나오는 **한 호흡의 반응**이다. 조사한 사실을 대사 안에 모두 설명하지 않는다. 인물의 짧은 판단과 행동만 남기며 별도 조사 기록이나 진행 문서를 만들지 않는다.
