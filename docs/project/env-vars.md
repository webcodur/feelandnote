...
> **최종 실측 체크: 26.07.31** — 저장소 내 `.env` 3종·`ga-credentials.json`·`.mcp.json` 전량 대조, 앱별 `process.env` 참조 실측

**이 저장소는 비밀값을 커밋하지 않는다.** `.gitignore`가 `.env`·`.env.*`·`ga-credentials.json`·`.mcp.json`·`**/credentials/`를 모두 제외한다.
...
| 4 | `ga-credentials.json` | 저장소 루트 | 유입 통계(GA4) 조회 불가. 웹 구동 자체는 됨 |
...
> Google Books 키가 15개나 남아 있는 이유는 폐기 전 한도를 늘리려 여러 개를 돌려쓰던 흔적이기 때문이다. 지우지는 않았으나 **되살리지 않는다**(`AGENTS.md` 공유 패키지 절).
...
4. 구글 서비스 계정 → 키 삭제 후 새 키 내려받아 `ga-credentials.json` 교체
