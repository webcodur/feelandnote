 ...
 * pnpm exec tsx scripts/celeb/timeline/prepare-fiction-candidates.ts --slugs=achilles,athena
 * pnpm exec tsx scripts/celeb/timeline/prepare-fiction-candidates.ts --slugs=absyrtus --allow-inactive
 */
...
import {
  ...
  loadFictionSourceSnapshots,
  type FictionCandidateSeed,
  type FictionProfileSnapshot,
...
async function loadActiveFictionProfiles(): Promise<FictionProfileSnapshot[]> {
  ...
  for (let from = 0; ; from += 1000) {
    ...
    const page = (data ?? []) as unknown as FictionProfileSnapshot[]
    profiles.push(...page)
...
function sourceTitle(snapshot: FictionSourceSnapshot, contentId: string, locale: string): string | null {
...
function writeSourceIndex(
  root: string,
  profiles: FictionProfileSnapshot[],
  snapshots: Map<string, FictionSourceSnapshot>,
) {
...
async function main() {
  ...
  for (const profile of profiles) {
    ...
    const seed: FictionCandidateSeed = {
      schema_version: FICTION_CANDIDATE_SCHEMA_VERSION,
      slug: profile.slug,
      celeb_id: profile.id,
      celeb_tier: 'fiction',
