...
후한 말 ~ 삼국 시대 인물 묶음을 시리즈와 직교한 축으로 관리한다. 현재 이 명단을 쓰는 시리즈는 book-recommend(서재 탐방) 하나다.
...
- **`yi-sun-sin`(이순신) 추가 금지** — 조선 인물이라 삼국지가 아니다.
- **`hell-bar`(저승 술집)를 현역으로 취급 금지** — 2026-07-16 폐기됐다(구현된 적 없음). 인물끼리 사상을 부딪치는 자리는 「가상 담화」(`docs/project/remotion/discourse/README.md`)가 대신하며, 담화는 아직 삼국지 인물을 쓰지 않는다. 기획 보존본은 `docs/archive/remotion/hell-bar/README.md`이며 참조 문서로 인용하지 않는다.
- **연의 캐릭터 vs 정사 인물 구분** — 슬러그는 같지만 인물 명단은 정사 기준이다. 명단 자체는 문제없으나 에피소드 작성 시 정사 기준을 지킨다.
