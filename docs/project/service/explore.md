...
검색·선택 영역은 PC에서도 가운데에 모으고 각 줄의 폭을 맞춘다. 두 모드 모두 페이지 번호를 바꿀 때 현재 스크롤 위치를 유지한다. 기관명은 두 줄 높이를 확보하고 한 줄일 때도 세로 가운데에 놓는다. 인물·기관 카드는 금빛 테두리·그림자·이미지 하단 빛·이름 칸 강조와 내부 이미지 확대를 `ExploreCard.styles.ts`로 공유한다.
