  ...
  return (
    <Button unstyled onClick={onClick} className="flex flex-col items-center group">
      ...
      {showProfession && user.title && (
        <span className="text-xs text-accent truncate max-w-full">{user.title}</span>
      )}
      {showProfession && user.profession && (
        <span className="text-[11px] truncate max-w-full">{getCelebProfessionLabel(user.profession)}</span>
      )}
    </Button>
  );
}
...
export function SimilarUserCard({ user, onClick }: { user: SimilarUserInfo; onClick: () => void }) {
  return (
    <Button unstyled onClick={onClick} className="flex flex-col items-center group">
      ...
      <span className="mt-2 text-xs font-medium text-text-secondary group-hover:text-accent truncate max-w-full">{user.nickname}</span>
      {user.overlap_count > 0 && <span className="text-xs text-accent font-serif">{user.overlap_count} Bonds</span>}
    </Button>
  );
}
    ...
    >
      ...
      <div className="flex items-center gap-3 text-left min-w-0 relative z-10">
        ...
        <div className="flex flex-col min-w-0">
          ...
          {subtext ? (
            <span className="text-[11px] text-accent font-serif font-black tracking-widest uppercase truncate">{subtext}</span>
          ) : (user.title || user.profession) ? (
            <div className="flex items-center gap-1">
              {user.title && <span className="text-xs text-accent font-serif truncate">{user.title}</span>}
              {user.title && user.profession && <span className="text-[11px]">|</span>}
              {user.profession && <span className="text-[11px] font-serif truncate">{getCelebProfessionLabel(user.profession)}</span>}
            </div>
          ) : null}
        </div>
      </div>
