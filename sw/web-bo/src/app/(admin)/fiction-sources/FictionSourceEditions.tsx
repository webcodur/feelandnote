...
import {
  deactivateFictionSourceProduct,
  replaceFictionSourceProduct,
  saveFictionSourceEdition,
  type FictionSourceEditionAdminItem,
} from '@/actions/admin/fiction-sources'

interface FictionSourceEditionsProps {
  contentId: string
  sourceTitle: string
...
function toEditionForm(edition?: FictionSourceEditionAdminItem): EditionForm {
  return edition
    ? {
...
function toProductForm(
  edition: FictionSourceEditionAdminItem | undefined,
  platform: 'coupang' | 'amazon',
): ProductForm {
...
export default function FictionSourceEditions({
  contentId,
  sourceTitle,
  editions,
}: FictionSourceEditionsProps) {
  ...
  const selectEdition = (edition: FictionSourceEditionAdminItem) => {
  ...
  const saveEdition = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        await saveFictionSourceEdition({
          ...editionForm,
          contentId,
        })
  ...
  const saveProduct = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        await replaceFictionSourceProduct({
  ...
  const deactivateProduct = () => {
    ...
    startTransition(async () => {
      try {
        await deactivateFictionSourceProduct({
          editionId: selectedEdition.id,
          platform: productForm.platform,
        })
