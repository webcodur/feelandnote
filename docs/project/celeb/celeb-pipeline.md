...
- 기존 0건 인물은 전부 `open`으로 둔다. 과거 조사 여부를 추측해 일괄 `-1`로 바꾸지 않는다.
...
기본 등록은 `content_research_status='open'`으로 시작한다. 감상 여정은 웹
리서치 기반으로 작성하되, 콘텐츠 가능성을 영구히 닫지 않는다. 이후 작품
단서가 확보되면 콘텐츠를 등록하고 감사를 거쳐 full로 승격한다.

기존 Light 0건은 다음 순서로 회수한다.

1. 콘텐츠가 이미 있는 Light → 등록분 감사 후 full 승격
2. 활성 + 감상여정에 명시 작품 있음 → 명시 작품만 표적 검증
3. 활성 + 감상여정 있음 → 비정형 작품명 추출 후 검증
4. 활성 + 감상여정 없음 → 처음부터 조사
5. 비활성 + 감상여정 있음 → 위와 같이 기존 작품 단서를 먼저 회수
6. 비활성 + 감상여정 없음 → 영향력·자료 존재 가능성만 빠르게 선별
7. 비활성 빠른 선별에서 단서가 없으면 `deferred` 또는 `open`으로 남긴다. `confirmed_empty` 금지

운영 화면은 web-bo `/celebs/content-research`다. 위 경로는 현재 DB에서
자동으로 파생되므로 별도 명단 파일을 만들지 않는다.
