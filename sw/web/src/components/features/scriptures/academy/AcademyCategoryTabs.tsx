/*
  파일명: /components/features/scriptures/academy/AcademyCategoryTabs.tsx
  기능: 학당 카테고리 탭 (레이아웃용)
...
export default function AcademyCategoryTabs() {
  ...
  // pathname: /ko/scriptures/academy/video/lighting → segments[4] = "video"
      ...
      >
        {ACADEMY_CATEGORY_IDS.map((cat) => {
          ...
          return (
            <Link
              key={cat.id}
              href={`/scriptures/academy/${cat.id}/${firstCourse}`}
              className={`
