...
> **최종 실측 체크: 26.09.09 10:31 KST — 앞선 대거 색인 이탈 판정을 정정한다.** 09-04 Search Console 색인 목록의 82 URL을 공식 URL Inspection API로 다시 검사한 결과 모두 `PASS / Submitted and indexed`였다. 추가한 영문 홈도 색인됨이며, 빌 게이츠만 `Crawled - currently not indexed`다. 총 84개 중 83개 색인됨이라는 결과는 기존 색인 목록 중심의 표본이며 도메인 전체 색인률이 아니다. 같은 날 앞선 검사에서는 81개 미색인·1개 리디렉션으로 기록됐지만 재검사와 모순된다. 원응답과 재검사 결과는 [조사 데이터](../../../data/seo-index-inspection-20260909.json)에 함께 보존하며, 차이의 원인은 확인하지 못했다.
>
> 실제 Google 검색에서도 `필앤노트`의 첫 웹 결과가 홈페이지였고, `site:feelandnote.com` 첫 페이지에 홈·허브·인물 등 10개 결과가 나왔다. 홈·머스크·빌 게이츠를 한국어·영어 API로 각각 확인해도 앞의 둘은 색인됨, 빌 게이츠는 미색인으로 일치한다. 따라서 현재 사이트 전체 차단이나 기존 색인 목록 전체 이탈을 주장하지 않는다. 홈·머스크의 마지막 크롤은 각각 09-04 16:48:13 UTC, 09-08 14:03:45 UTC로 이번 서버 배포보다 앞서므로 배포가 색인을 복구했다고 해석할 수도 없다.
>
