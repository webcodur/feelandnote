 ...
 * 규격 SSoT: docs/project/celeb-avatar-spec.md §1
 * 판정 기준은 src/lib/avatar-geometry.ts 의 AVATAR_SPEC 하나만 쓴다 — 이 파일에 수치를 따로 적지 않는다.
 *
 * 사용법 (sw/web-bo 에서):
 *   npx tsx scripts/measure-avatar-geometry.ts <이미지폴더> [--json <출력경로>]
 *   npx tsx scripts/measure-avatar-geometry.ts --from-db [--offset 0] [--limit 200] [--json <출력경로>]
 *
...
function parseArgs() {
  ...
  if (!fromDb && !dir) {
    console.error('사용법: npx tsx scripts/measure-avatar-geometry.ts <이미지폴더> [--json <출력경로>]')
    console.error('       npx tsx scripts/measure-avatar-geometry.ts --from-db [--offset 0] [--limit 200]')
    process.exit(1)
  }
