...
> **최종 실측 체크: 26.07.30** — 화면 그룹·네비게이션 구성 + 인물 상세 테마 연결 실측
...
인물 상세의 색·배경·상단 문양은 인물마다 달라진다. fiction 여부와 직군으로 8개 계열을 고르고, 같은 계열에서도 slug 기반 색상·배경 변형을 적용한다. 결정 규칙과 `/lab/celeb-themes` 비교 화면은 `docs/project/celeb-detail-themes.md`가 단일원천이다.
