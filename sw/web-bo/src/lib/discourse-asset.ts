/**
 * 가상 담화 자산 경로 해석 — 서버 전용.
 *
 * 파일을 그대로 내주는 창구의 급소다. 라우트 안에 두면 검증할 방법이 없어서 따로 뺐다.
 *
 * ⚠ 이 앱의 진입 검사(`src/proxy.ts`)는 **경로가 이미지 확장자로 끝나면 아예 건너뛴다**
 *   (matcher 가 `.*\.(svg|png|jpg|jpeg|gif|webp)$` 를 제외한다). 팩션에서 실측으로 확인한 함정이다 —
 *   `/api/discourse/asset/x/y.png` 은 로그인 검사를 통과해 곧바로 라우트에 닿는다.
 *   그래서 라우트 자신이 사람을 확인하고(`guardDiscourseRoute`), 이 함수가 경로를 잠근다.
 *   둘 중 하나만 있으면 뚫린다.
 *
 * 두 겹으로 잠근다.
...
/** 내줄 수 있는 확장자만 */
export const DISCOURSE_ASSET_MIME: Record<string, string> = {
  '.png': 'image/png',
  '.jpg': 'image/jpeg',
  '.jpeg': 'image/jpeg',
  '.webp': 'image/webp',
...
export type DiscourseAssetResolution =
 ...
 *
 * @param root    public/discourses 절대경로
 * @param relPath 슬래시로 이은 상대경로(URL 디코딩은 호출 측이 마친 상태)
 */
export function resolveDiscourseAsset(root: string, relPath: string): DiscourseAssetResolution {
  ...
  const mime = DISCOURSE_ASSET_MIME[ext]
