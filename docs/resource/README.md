...
작업 참조 문서가 아니다. 프로젝트 규칙·규격의 근거로 쓰지 않는다.

## 문서 묶음

| 파일 | 내용 |
|---|---|
| [`ai-notes-index.md`](ai-notes-index.md) | AI 글쓰기·작업 메모 진입점 |
...
| 파일 | 내용 |
|---|---|
| `리모션 단축키.txt` | Remotion Studio 단축키 표 |
| `책 요약 예시.txt` | 책 요약 요청 프롬프트 템플릿 |
| `내 할 일.txt` | 화면·백오피스 개선 대기 목록 |
| `육안검사.txt` | 인물 사진을 눈으로 확인하고 남긴 문제 목록 |
| `보이스 에디터.txt` | 팩션 음성 ID 청감 메모와 편집 화면 개선 착안점 |
| `프롬프트_이미지생성시.txt` | 이미지 발주 요령 메모. 정본은 `docs/project/production/image-generation.md` |
| `프롬프트_전달.txt` | 화면을 보다 발견한 문제를 적어 넘기는 메모 |

`내 할 일.txt`·`보이스 에디터.txt`·`프롬프트_*.txt`는 커밋하지 않고 로컬에만 둔다.

> 어록 채굴물은 여기 두지 않는다. 소속 에피소드 폴더의 `quotes/`에 둔다 — `factions/<에피소드>/quotes/` 또는 `episodes/<시리즈>/<인물>/quotes/`.
> 기존 `언사/` 18종은 26.07.16에 각 소속으로 이관 완료했다.
